...
- 复核窗口 TTL 默认 14 天（dict_config 可配）：claim 到期无回归结果、无 admin 复核 → 自动回退 open 并记 conversion_record（防"认领驻车"——被认领的错误在研发未跟进时不再复发回流、也不再出现在 open 计数，等于人工关掉了该错误的线上告警）。**收敛 = 两时钟先到者（v3.5.4，R6）**：K 序列满 → 置 fixed（TTL 停钟）；本 TTL 先到 → 回退 open（K 观察中止、下轮 claim 重计），K 观察/回查推进不重置、不延长 TTL（§10.5）。
...
case 在该 run 无结果行（缺行）≠ na → 无终值，继续轮询至 claim TTL 超窗回退 open，不触发 needs_review（**v3.5.6 R-4：claimed case 缺行且 ∈ 该版 run 溢出 case 列表**——offline 只读面透出 cap newest-active-first 挤出 case（case_truncated → excluded_case_ids，§7.2/§11 补强）→ 判「该版本窗口外欠测」非修复失败 → claim 详情可诊断状态「该 case 连续 N 版未纳入回归（cap 窗口外），非修复失败」+ 前端引导（admin 人工 fixed / 提示 agent 错误量需治理）；不引入跨端保窗口；**v3.5.8 R-16：该「∈ 溢出列表」核对由 recheck 缺行时自动拉该版 run 比对 excluded_case_ids 完成**——∈ → 自动判「该版本窗口外欠测」标 claim 详情（不再依赖人工去 offline 只读面手工核对）、∉ → 人工兜底引导）
...
auto-fixed 判据 = **claim 固化 K（claim_k）+ claimed-case 级纯净判据**（v3.5.5 = Task #4-③ R-1/R-2，同步 solution_detail §7.6 v1.5 / offline error-backflow-phase2.md v0.6.2 注记；后续 R-3~R-24 修订演进见本条 v3.5.6~v3.5.9 内联标注 / §7.6 v1.6~v1.9）：
  **K = claim 固化 claim_k**：claim 时把 K 固化为 error_cluster.claim_k（值域 {1,2}、claim API 可选入参 k、缺省取全局默认 2 并写死固化值）；自 fix_version 起**连续 claim_k 个**（claim_k=1 = 单版纯净 pass 即 fixed）纯净可判版本 run 均无 fail → verify passed → fixed（closed_by=auto_regression）；任一版 fail → 即时 reopen 回 open（K 清零）；K 只读固化值不实时读全局键；观察期零推进可降、推进后锁死（v3.5.5）；na 行/缺行中断观察、不计数不终止，轮询至 claim TTL。**v3.5.8 R-17（候选版本序列锚）**：自 fix_version 起的候选版本序列以 offline「agent 已见版本」读面（manual/held_out runs distinct version，§10.4 R-5 数据源）取 ≥ fix_version 全版本序为准——中间版有发版但无 error run → 判缺行中断观察、不续缺环（杜绝 v1+v3 丢中间版的假连续 K 满 false-fixed）；该版 error run 由 R-3 差集迟到补建终态到达 → 补判推进
  ...
  **收敛终点 = 两时钟先到者**：K 满 → fixed（TTL 停钟）；claim TTL(14d) 先到 → 回退 open（K 观察中止、下轮 claim 重计），K 观察/回查推进不延长 TTL
```
