...
> 版本：v3.5.11（error-only 一期定稿；同步部署边界：仅 backend+frontend Docker、依赖走公共 infra 与公共 API 网关；Task #4 B 包 needs_review v1 源 = error-run 判定产物 + reentry 版本门控语义同步 + v3.5.4 R5-R7 auto-fixed 判据重构权威同步 + v3.5.5 R-1 claim_k / R-2 claimed-case 级纯净判据权威同步 + v3.5.6 R-3~R-10 修订包语义同步 + v3.5.7 R-18/R-19 高危 2 条语义同步 + v3.5.8 R-13~R-17 B 类 5 条语义同步 + v3.5.9 R-20~R-24 C/低危 5 条语义同步 + v3.5.10 **平台间契约反转（判定链由「online 主动拉 offline」反转为「offline 主动推 online」、online 判定链零出站）**语义同步，对应 solution_detail v1.23（v1.23 = 平台间契约反转第 1~3 刀落字（offline 主动 `POST /backflow/regression-results`、判定数据源改本 link 已落 `verify_run_record` 行集、整删轮询链与只读面）；v1.22 = T-3.6 结清纯口径修正；v1.11 = v1.10 语义 + 2026-09-08 阶段 1 尾项实现收口注记：前端栈 Vue 裁定/auth 最小闭环边界/trace 查询实测修正，无语义变更；v1.10 = v1.9 语义 + §14.5 X 系列集成异常/边界用例登记，亦无语义变更；栈措辞 React→Vue 已就地同步修补、不升版）+ v3.5.11 **R-7 改判权威同步（判据由「按 `reject_detail` 缺项分可愈性」改为「行为数据 = 该 link 历史重推次数」、范围收敛为单 link 路径；门控强度实为降档、批量侧标注已裁定不做）**，对应 solution_detail v1.23 就地订正注 + register R-7 改判记录）
...
> v3.5.3 变更：**Task #4 B 包语义同步（对齐 solution_detail v1.3，无新技术决策）**——needs_review v1 源由「claim 回归 infra-error 边缘触发」改定 = **error run 的 `na` 结果行**（case 级、reason 带 run_id+error_type；同 run 同 error_type 聚合单条 review 批、整批同动作处置），判分 na/会话快照不足/cc 文件型明确二期/D18 不回流、run 超时非源（TTL 兜底回退 open）；§10.4 状态机、§10.5 回查、§12.1 状态注、§14 P2 验收、§16 风险行同步；§10.5 reentry 补版本门控 = 等值+日期前缀序（同日前缀仅等值放行、同日不等值 count+1 提示不静默、跨日按日期前缀、非日期形态回退等值）。详见修订记录 v3.5.3 行。
> v3.5.4 变更：**Task #4 R5-R7 auto-fixed 判据重构权威同步 + D2 载体吸收（对齐 solution_detail v1.4 / offline error-backflow-phase2.md v0.6.1，消费语义、无 offline 机制改动）**——auto-fixed 判据 = **纯净 run + 跨版本稳定序列 K**（纯净可判 = error run 达终态 {completed, partial_failed, timeout, cancelled} 且 run 级 na_case==0，online 消费侧派生、不加纯净列不加 offline 字段；自 fix_version 起连续 K 个（默认 2、可配）纯净可判版本 run 均无 fail 才置 fixed、fail 即时 reopen、na 行/缺行中断观察不计数）；reason 值域扩 {na, unclean_run, reentry_same_version}（unclean_run = 含技术 na run 内 pass 行、同 (run,error_type) 聚单条批并入 na+pass；reentry_same_version claim 级单条）；K-TTL 两时钟收敛终点（K 满 → fixed 停钟 / claim TTL 先到 → 回退 open）；§7.2 补 verify_run_record / needs_review_batch 载体行；§10.4/§10.5/§12.1/§15/§16 状态注与风险行同步。详见修订记录 v3.5.4 行。
> v3.5.5 变更：**Task #4-③ R-1/R-2 auto-fixed 判据精化权威同步（对齐 solution_detail v1.5 / offline error-backflow-phase2.md v0.6.2 注记，消费语义、无 offline 机制改动）**——① **K 改 claim 级固化 `claim_k`**（error_cluster 增列、claim API 入参可选 k∈{1,2} 缺省取全局默认 2、claim 生命周期内零推进可降/推进后锁死、claim_k=1 = 单版纯净 pass 即 fixed、K 只读固化值不实时读全局键；dict_config 补 auto_fixed_k_default 键）；② **纯净判据由 run 级 `na_case==0` 下钻 claimed case 行 + na error_type 影响域分流**（环境级 circuit_open/interface_disabled/scheduler_unexecuted/http_client_error/pool_error → 触发 unclean_run；case 级 timeout/connect/sse_parse/http/no_done/body_too_large/missing_assertion/assertion_shape → 不牵连他 cluster pass）——unclean_run 触发窄化 + `needs_review_batch` 纯化仅 unclean_run 批 + **na 退批改 cluster 级单点** + 撞键裁定收窄；重 claim 同 fix_version 动态提示并入 R-5 范围。§7.2 载体行、§10.4/§10.5/§12.1/§15/§16 状态注与风险行同步。详见修订记录 v3.5.5 行。
> v3.5.6 变更：**Task #4 修订包 R-3~R-10 语义权威同步（对齐 solution_detail v1.6 / offline error-backflow-phase2.md v0.7；R-3/R-8 与 R-4/R-5 offline 只读面为 offline 机制改动、单独立项，solution 仅同步 online 消费语义）**——① **R-4 cap 溢出 case 回查可诊断**（§10.5 缺行 bullet /§10.4/§12.1）：claimed case 缺行且 ∈ 该版 run 溢出 case 列表（offline 只读面透出，R-3 差集补齐后旧版本也有 run）→ 判「该版本窗口外欠测（cap 挤出）」非修复失败 → claim 详情可诊断状态 + 引导 admin 人工 fixed / agent 错误量治理，不静默 14d 转圈、不引入跨端保窗口；② **R-5 claim 软校验**（§10.4/§12.1）：fix_version 提交前归一提示（trim/大小写/v 前缀）+ **∉ agent 已见版本 → 告警确认**（数据源 = offline「agent 已见版本」只读面，manual/held_out distinct version；非硬拦——豁免首版未终态/对账滞后）；版本活跃度动态 K 提示同源（R-1③ 兑现）；③ **R-7 invalidated 批量 requeue + 可愈性标注**（§10.3/§12.1 online 工具）：invalidated（online 现场内容缺）视图批量复位 assembled 供 pull，逐行复用守卫/防抖、per-row 结果，按缺项标可愈性（字段缺/词表空补齐即愈；版本不识别不愈默认禁勾强确认）；④ **R-9 needs_review_batch resolve 并发语义**（§10.4/§7.2）：resolve(reopen_cluster) 事务内逐引用 cluster CAS，非 claim 引用**语义化跳过**（已 open → skipped_already_open / fixed → skipped_fixed / 不可预期 → manual_review），批整体单事务置 resolved、conversion_record 记 per-cluster 结果、批级 CAS open→resolved 防双 admin；⑤ **R-10 input 截断 + reason 值域 3→4**（§7.2/§10.2/§10.4/§10.5/§12.1）：error_cluster 增 `input_truncated` 标记（input 实文超 8K 截断，link/payload 随带）；截断关联 run 结果不计 K 不置 verify passed → needs_review（reason=`input_truncated`，claim 级单条、不产批、同 reentry 通道）+ UI 提示「input 超长截断、复现证据不完整」；input_hash normalize 截断 4096 → 8192（对齐复现输入上限）；reason 值域 {na, unclean_run, reentry_same_version, input_truncated} 3→4 三文档一致。详见修订记录 v3.5.6 行。
> v3.5.7 变更：**Task #4-③ 高危 2 条 R-18/R-19 语义权威同步（对齐 solution_detail v1.7 / offline error-backflow-phase2.md v0.7.1；R-18 载体在 online 判定态、R-19 offline 仅标注修正零机制，solution 同步 online 消费语义）**——① **R-18 input 明文载体补断 + input_truncated 判定点下钻**（§7.2）：trace_judge_state（§6.1 step4 判定态）仅持久 `root_input_hash`、明文无载体的组装断点补列——判定态增 `input_snapshot_clean`（root request.input 脱敏截断≤8K 明文快照）+ `input_truncated`，**消费 worker root 到达同刻落库**（唯一见原始明文处，§6.1 step4 / detail §4.3）；error_cluster 开 cluster/窗口内回填**自判定态列复制**（不依赖 ES 回读承诺兑现，v3.5.1）；`input_truncated` 判定点由「cluster 首现/回填时判」**下钻到「消费 step4 root 到达时判」**（聚类/组装仅见 ≤8K 截断快照不可判，H7-1 联动修正，§7.2）；② **R-19 error_type 判据字面量对齐 executor 真值 + 补漏项**（§10.5/§7.2）：case 级判据清单短名 `connect/sse_parse/http` → executor 真值 `connect_error/sse_parse_error/http_error` + **补入 `no_usage`/`contract_error`**（executor.py 非 RETRYABLE、直接 na，offline phase2 §6.4 权威标注 v0.7.1 同步）——消除「名字错位 → 未知 error_type 默认从严环境级 → 假 unclean_run」系统性误伤；从严兜底回归「仅兜底非常态」。详见修订记录 v3.5.7 行。
> v3.5.8 变更：**Task #4-③ H7 批 B 类 5 条 R-13~R-17 语义权威同步（对齐 solution_detail v1.8；R-16/R-17 读面字段位置 + R-13 §6.2 同簇刷新复制留 Phase B 立项/核对，offline 零机制）**——① **R-13 input_truncated 判定位窗口化**（§7.2/§10.5）：截断标记语义 = **消费侧判定态的 cluster 当前判定位**——同键窗口内新现小输入纯净打点到达 → 位刷新清 0 → 后续小输入版 run 正常攒 K（同键首现截断不再毒化一生、escape 可达）；cluster 已 closed 不刷新（终态只读不翻案）；② **R-14 unclean_run 载体口径归一**（§10.4/§10.5）：unclean_run 只经 needs_review_batch 处置、cluster 不入 needs_review 态（claim 迁移 reason 剔除 unclean_run），批引 claim cluster **实时派生「被未决批挂起」标注**（join link_refs 免加列）、TTL 不豁免沿用既有兜底（resolve 时 R-9 skipped_already_open 幂等覆盖）；③ **R-15 双通道优先级 = input_truncated 优先**（§10.5）：同 run 双条件（截断证据 + 环境级 na）走截断单条、不并入 unclean_run 批（批聚合排除截断 cluster）；④ **R-16 缺行诊断自动核对**（§10.5）：缺行时 recheck 自动拉该版 run 比对 excluded_case_ids，∈ → 自动判「窗口外欠测」（不再依赖人工离线核对）；⑤ **R-17 K 序列版本锚 = versions 读面**（§10.5）：自 fix_version 起的候选版本序列以 offline「agent 已见版本」读面全版本序为准——中间版有发版但无 error run → 缺行中断（杜绝 v1+v3 假连续 false-fixed），R-3 差集迟到补建终态到达补判推进。详见修订记录 v3.5.8 行。
> v3.5.10 变更：**平台间契约反转语义同步（对齐 solution_detail v1.23 / 第 1~3 刀）**——判定链方向反转：**offline 主动推、online 零出站**。① **回查判定不再是 online 的拉取型 job**（原 `worker/recheck_job.py` 60s 轮询 offline `GET /runs`、`/runs/{id}/results`、`/agents/{a}/versions` 三读面，**已整删**）：offline 在回归 run 产出后 `POST /backflow/regression-results`（fire-and-forget、5s 超时、offline 侧 3 次重试 1s/2s/4s、**online 不重试**），online 在**接收端点内同事务**跑 `verify.judge_link` 落判定；`judge_link` 不再持 offline 客户端（判定数据源 = **本 link 已落的 `verify_run_record` 行集**（载荷 `raw_json` 原样留档）+ cluster 字段）。② **载荷契约变更**：**删** `bound_version_first_seen`（强制但零读点）、**增必填** `prev_terminal_version`（值可空 = 无前次终态 run；堵「单值水位无法枚举中间版本 → 跨缺版丢推送 → 假累积 K → 静默误判为已修」）+ `agent_latest_version`（offline 侧该 agent 已有终态 run 的最大版本，恢复发版水位：`V > agent_latest_version` = offline 还没跑到该版本 → 保持 `no_progress`；**它判不了「水位之内是否漏收了一笔」，后者由 `prev_terminal_version` 承载**）；`case_id` 重复 → 整单拒。③ **缺行中断判据改 agent 级**（`verify._agent_versions`，link→cluster 两跳；原 R-17「versions 只读面全版本序」与 R-4/R-16「excluded_case_ids 只读面」**读面均不存在**，相应作废/改锚，见 §10.5）。④ **判定安全网**：新增 `worker/rejudge_job.py`（60s、**纯本地、零 outbound**）——扫 `status=='claim'` ∧ 有现行 pending link 的簇，重放本 link 已落行补判一次，收三类「事件到达时未就绪」现场（claim 晚于推送 / 判定抛异常被降级 / 状态过渡窗口）；**job 数 = 6（judge_scan / cluster / assemble / claim_ttl / rejudge / rollup）**。⑤ **「结果未达」claim 判据**：claim 侧不再靠轮询超时判定——由 `claim_ttl_expire` conv 记录在 ∧ `fix_version` 非空 ∧ 现行 pending link 零 `verify_run_record` 判「结果未达」，含抑制集（未超窗 / 过期记录须晚于 `assembled_ts` / 空 `assembled_ts`）。详见修订记录 v3.5.10 行。
> v3.5.9 变更：**Task #4-③ H7 批 C/低危 5 条 R-20~R-24 语义权威同步（对齐 solution_detail v1.9；R-20 offline pre-scan 门禁 code 走 Phase B、R-21 root-late 补判通道 = online 机制、R-22/R-23 offline 收尾注记 + online 消费语义、R-24 online-only）**——① **R-20 空答语义定性**（§10.5/§11 注）：R-12 core 空答 fail 不动；复现 run 空答 fail = **leakage 空话术复发证据**（agent 无有效产出、与词表命中 fail 同属未修复、证据形态不同）——「空答不落 na、落 verifier fail」保持；pre-scan 显式立项 Phase B code_detail 实施门禁（owner = offline 实施）；② **R-21 root-late 补判通道**（§7.2/§10.2）：残 trace 按子节点判定（judged=1）后迟到的 root（root_ok 0→1、root_status ∈ {error,timeout}）**不重跑整 trace**——消费 step4 仅补一次 root 级候选（root_error_type 走 L1/L2 值域筛）→ 复用聚类（同键 count+1/补快照、异键开新簇、已 closed 跳过不翻案）；幂等 = `trace_judge_state.root_late_complement` CAS 标记（detail §4.3④/§4.4/§5.1⑩）；③ **R-22 收尾回填 na 统一 `scheduler_unexecuted`**（§10.5 注）：scanner 回收/orchestrator cancel/run 级超时收尾三路径回填 na 同源 error_type、「na case 必带 error_type」不变量全覆盖收尾路径、timeout 不作收尾回填原因（offline phase2 §6.3/§6.4）；④ **R-23 timeout 三层语义分界**（§10.5）：case 级 na 源 / run 终态 timeout 不直接触发 needs_review（B-3 兜底 = claim TTL 回退 open）/ 收尾回填 na（环境级）→ 关联 pass case 入 unclean_run 批——「run 超时非源」排除直接驱动、不排除环境级回填污染间接作用；⑤ **R-24 requeue guard 状态域精确化**（§10.4）：requeue 复位条件 = `cluster.status ∈ {open, claim, needs_review}`（fixed/inactive 禁、走 superseded+reopen）+ 锚点保护（不动 fix_version/claim_k/TTL）+ verify 兜底 = 仅 pending invalidated link（detail §7.4/§9.4）。详见修订记录 v3.5.9 行。
...
| D20 | online→offline 传输 | **pull 模式（脑暴定稿）**：online 只聚类/去重/组装**代表 badcase 的统一 payload（D19）**，不 push、offline 不直连 ES；**offline 定时任务调 online pull-API 拉取已组装 payload** → 入 offline 草稿态 → 激活决策仍归 offline（§10.3/§11 #5/§12 最小 API 面）。**反方向（复验结果回传）为 push（v3.5.10 契约反转）**：offline 产出回归结果后主动 `POST /backflow/regression-results`，online 不轮询、判定链零出站（§10.5） |
...
```
                    ...
                    └─────────┼────────────────────────────────────────┘
                              ...
                              │  ② offline 激活回写 + 复验结果 **主动推送**（v3.5.10 契约反转：offline 推、online 零出站）
                    agent-evaluation-offline（回归评测模式，§11）
                    定时拉取 payload → 草稿态 → 激活/人工确认 → 回归 run
                    → 复验结果 offline 主动 POST /backflow/regression-results → online 同事务判定 → 错误标「已复验」
```
...
```json
{
  ...
  "agent": "good-question",
  "agent_version": null,                 // agent 部署版本（SDK init 从环境注入，可空）；R5 判定版本锚定数据源
...
- **版本注入（R5 打点源，v3.4.5）**：SDK init() 读取部署环境 `AGENT_VERSION`（agent 侧构建时注入 image tag / git commit），写入每个事件 `agent_version` 字段——R5 判定版本锚定与聚类溯源的数据源（§10.5/§13.0 checklist #9）；未配置则该字段空、事件不拒收，但接入验收判不通过。
...
3. **脱敏复核**（§4.5）；
4. **trace 级累积态（决策 R1）**：partition=1 相对有序 → 消费侧维护 per-trace 累积缓冲（TTL=完成窗口）：request 根事件到达 → 标记该 trace 可判定；子节点 error 汇入该 trace 状态；**trace 完成才做 L1/L2 判定 + per-trace 归并（§10.2）**——SSE/多批/子节点分批到达下判定不缺完整 trace。**重建 = 判定态持久化（第三轮裁定）**：per-trace 累积态三要素（已见子节点含 error 汇总、root 是否到达、是否已判定）**落 MySQL 判定态表（TTL=完成窗口 + 宽限）**，重启/重平衡从持久化判定态重建缓冲、**不依赖 ES 回读**（offset 已过则已消费子节点无从回读，纯内存缓冲重平衡会静默漏判）；**残 trace（root 未到达但已有子节点 error）不悬挂等待——按已有子节点判定**，防超长 SSE 连接在缓冲 TTL 内不闭合导致漏判；判定完成记处理位点防重放重复建 case（§6.2）。**root-late 例外（v3.5.9 R-21）**：root 在判定态已 judged=1 后到达（残 trace 已按子节点 error 判过）→ **不重跑整 trace、不推翻已判**——本步对迟到 root 级触发一次补判候选（root_error_type 走 §10.1 L1/L2 值域筛，命中才产），复用 §10.2 聚类生命周期；幂等 = 判定态 `root_late_complement` 置 1 标记 CAS（§4.3④/§7.2/§10.2）。quality 信号二期才入累积态（v1 不采集）；
...
| `error_cluster` | 错误聚类（error_type、input_hash、首次/最近 ts、次数、**代表事件最小快照（含 input 实文，v3.5.1 扩存；超 8K 截断并标 `input_truncated`=1，v3.5.6 R-10；**v3.5.8 R-13：该标记 = 消费侧判定态的 cluster 当前判定位——同键窗口内新现 ≤8K 打点经 step4 判定位刷新清 0、closed cluster 不刷新**）**、状态 open/claim/fixed/inactive/needs_review、**claim 时填的修复版本 fix_version（判定锚定依据，§10.5）**、**claim 固化的 K（claim_k，v3.5.5）**、**生命周期代数 generation**——复发重开代数 +1，§10.2） |
| `error_case_link` | 错误 ↔ offline 回归用例关联（case_id、case_type、verify_status、**offline case 激活态 offline_status∈{assembled,draft,active,invalidated}（pull 传输，D20）**（assembled=online 已组装 payload 待 offline 拉取；draft=已拉取待激活——**v1 error 型收单即结构自检激活**（quality 型人工确认属二期 §4.4）；active=offline 已激活可进回归 run；invalidated=offline 驳回（**v1 = error 结构自检失败**；quality reject 软删 rejected 二期）回写失效，reopen 可再流转——online 侧冗余镜像 offline case 状态，供"待 offline 拉取/确认"筛选标记，§10.3/§10.5）、source trace_id、**建 case 时记录的触发 agent 版本（仅溯源：确认错误在缺陷版 V_obs 存在）**、**判定锚定版本 = 关联 claim 填的 fix_version**（决策 R5 修正——错误在已发布版发现、回归 run 绑待发布修复版，按触发版本取结果恒取空，§10.5）、**建 case 幂等键（payload_id，D19）**） |
| `trace_judge_state` | **消费判定态（§6.1 step4 持久化载体，重平衡/重启重建不回读 ES）**：per-trace 累积态三要素 + 已判定幂等集 + **`root_input_hash` + `input_snapshot_clean`（root request.input 脱敏截断≤8K 明文快照，v3.5.7 R-18）+ `input_truncated`（root 到达时判原始>8K 置 1，v3.5.7 R-18）+ `root_late_complement`（v3.5.9 R-21：root 在 judged=1 后迟到 → root 级补判已触发标记，root 到达事务内 CAS，§10.2）**——消费 step4 root 到达同刻落库、唯一见原始明文处；error_cluster 开 cluster/回填自本表复制快照与截断标（§7.2/§10.2/§10.3） |
...
| `verify_run_record` | 回归 run 单错级逐版本结果时间线（verify passed 的**跨版本稳定序列 K 载体**——K 满才 auto_regression 置 fixed，§10.5） |
| `needs_review_batch` | error run 判定产物批量聚合载体（**v3.5.5 纯化：仅承载 unclean_run 批**，reason 值域 {unclean_run}；同 (run,error_type) 至多单条、整批单动作处置；`na` cluster 级单点处置、`reentry_same_version` / `input_truncated`（v3.5.6 R-10）claim 级单条均不落本表；**resolve 并发语义（v3.5.6 R-9）：事务内逐引用 cluster CAS claim→open，非 claim 引用语义化跳过（skipped_already_open/skipped_fixed/manual_review），批整体单事务置 resolved、批级 CAS open→resolved 防双 admin、conversion_record 记逐 cluster 结果**，§10.4/§10.5） |
| `agent_credential` | agent 上报凭证（Kafka SASL/ACL 映射，加密存储 + 轮换，§12） |
| `dict_config` | 平台配置（兜底话术库、超时阈值、正文开关白名单、会话上下文轮数、聚类窗口等） |

> MySQL 一致性补强（六方向评审 + v3.4.5/v3.4.6）：`error_cluster` 去重键按类型分叉（§10.2：error=agent+interface+error_type 分类+input_hash；low_confidence=**证据签名** agent+interface+取数证据+断言态 → 归一 `signature_hash` 落键列，**二期启用，v1 无 low_confidence 故只有 error 键**）加**唯一索引、含生命周期代数 generation**——同代并发首现双写由唯一索引吸收（失败重试），复发重开（§10.2 superseded 后同键再现）代数 +1 开新行、不与同键已归档 cluster 冲突；`error_case_link` 带 source trace 幂等键防重试/重放重复建 case（§10.3 对账）；`error_cluster` 存**首次异常代表事件最小快照**（input_hash、**input 实文（脱敏 + 截断 ≤8K，v3.5.1 裁定扩存——供 error payload 组装直接取数、不依赖 ES 回读**，§10.3）、error_type、error_msg≤512、首次 trace_id、触发 agent 版本），供 ES ILM 过期后聚类处置仍有可辨原文（needs_review/人工处置场景）；快照 input 实文脱敏口径与正文同级（§4.5/§4.6），不因入 MySQL 放大 PII 留存。**input 截断标记（v3.5.6 R-10 + v3.5.7 R-18 判定点下钻）**：判定时点 = **消费 worker root 到达同刻**判打点原始 `len(input)>8K` → 落判定态 `trace_judge_state`（§6.1 step4 root 到达，**唯一见原始明文处**）`input_truncated` + 存脱敏截断 ≤8K 明文快照 `input_snapshot_clean`；error_cluster 开 cluster/窗口内回填**自判定态列复制**（聚类/组装仅见 ≤8K 截断快照，**不在聚类/组装判**——H7-1 曾标「cluster 首现判」已下钻，R-18）→ error_cluster/`error_case_link` 显式标 `input_truncated`（不用实长 8K 逼近判截断；link 随 D19 信封透传 offline 只读上下文、判定仍如实不因截断改判，H7-4），error run 复现输入源随带截断标记——claimed case 关联 run 截断证据不计 K（尾段依赖型错误复现输入 ≠ 真错误输入 → 终值不可信），转 needs_review（reason=`input_truncated`，§10.4/§10.5）。**v3.5.8 R-13（判定位窗口化）：该复制的 input_truncated = 消费侧判定态的 cluster 当前判定位——同键窗口内新现 trace 开簇/回填复制时以新现 trace 的 step4 判定态刷新（≤8K 清 0、>8K 置 1，单 trace 即刷、不攒批）；cluster 已 closed 不刷新（终态只读不翻案）——同键首现截断不再毒化后续小输入纯净版、escape 可达；截断历史留 needs_review(input_truncated) 产物 + 详情警示不抹（§10.5）。**
...
- `input_hash = sha256(normalize(input))`；normalize = strip + 折叠空白 + 截断 8192 字符（**v3.5.6 R-10：截断上限由 4096 → 8192，对齐复现输入截断上限 ≤8K——去重粒度不再小于复现粒度，消除 4K 前缀同/尾异长输入的键/快照漂移，§7.2 input_truncated**）；对话型取 input_turns（v1 无会话上下文摘要，摘要归一属二期——H7-20 清除 v3.4 残留）。
...
**root-late 补判（v3.5.9 R-21）**：残 trace 按子节点 error 判定（`judged=1`，§6.1 step4 判定态）后迟到的 root（root_ok 0→1、root_status ∈ {error, timeout}）**不重跑整 trace、不推翻已判结果**——仅对该迟到 root 级**补一次候选**（root_error_type 走 §10.1 L1/L2 值域筛，命中才产）复用本聚类生命周期：同键已有现行 cluster → 只计数/补快照；异键 → 新开 cluster generation+1 组装；同键已 closed（fixed/inactive）→ 跳过不翻案（终态只读）。幂等 = 消费侧判定态 `trace_judge_state.root_late_complement` 置 1 标记 CAS 防重放/多实例重复补（§7.2 trace_judge_state 行）。语义落点：detail §4.3④/§4.4/§6.1/§6.2。
...
每条 `error_cluster` 可 ignore（inactive）、**claim（人工认领标记已修复，进入"待复核"中间态，须填修复版本 fix_version + 说明，可选 K 参数（k∈{1,2} 缺省取全局默认）→ 固化为 claim_k，conversion_record 审计；fix_version 是 R5 判定锚定版本，缺失无法走 auto_regression 闭环，§10.5；v3.5.5：K 为 claim 固化值、观察期零推进可降推进后锁死；**v3.5.6 R-5 claim 表单软校验**：fix_version 提交前归一提示（trim/大小写/v 前缀，不改存储值语义）+ **∉ 该 agent 已见版本（**v3.5.10 改锚**：原数据源 = offline「agent 已见版本」只读面，**该只读面已随契约反转删除**；软校验降级为非硬拦提示——**改以「online 已收结果的版本集」为数据源**（载荷 `agent_latest_version` 承载 offline 发版水位，仅判「跑到没跑到」，判不了水面内漏收，后者由 `prev_terminal_version` 承载；详见 solution_detail §7.6 v1.23）；「fix_version 未见 → 提示」的目标语义不变）→ 告警确认**（展示已见版本、确认后才提交，非硬拦——豁免首版修复尚未终态/对账周期滞后合法场景）；重 claim 同 fix_version + 版本活跃度动态 K 建议同源提示（R-1③））**、查看已生成用例与复验状态。**置 fixed 需回归证据（该 case 自 fix_version 起连续 claim_k（claim 固化 K）个纯净可判版本回归 run 均无 fail，§10.5）或 admin 复核**——viewer 不能单方面 closed（决策 R4，防绕过 pending 回归门禁）。可 reopen。自动生成默认开启（组装 payload 供 offline 拉取、激活归 offline，D19/D20/§11 #5）。`needs_review`（v1 源 = claim 回归 error run 结果行的判定产物，reason 值域 {`na`、`unclean_run`、`reentry_same_version`、`input_truncated`（v3.5.6 R-10）}，§10.5；判分 na/会话快照不足/cc 文件型二期/D18 不回流、run 超时非源）由 cluster 属主 viewer 认领处置——**unclean_run（含环境级 na run 内 pass 行，v3.5.5 窄化）**同 run 同环境级 error_type 聚为单条 review 批（needs_review_batch，同 (run,error_type) 不重复建批）、整批同动作处置（补充证据回 open / 超时升级，§16 易用性登记；**v3.5.6 R-9：resolve 事务内逐引用 cluster CAS claim→open，非 claim 引用语义化跳过**——已 open（TTL/回归 failed 自然回退）→ `skipped_already_open` 幂等、fixed（K 满提前收敛）→ `skipped_fixed`、其余不可预期 → manual_review 返回明细；批整体单事务置 resolved、conversion_record 记逐 cluster 结果，批级 CAS open→resolved 防双 admin 并发——引用 cluster 漂移不再整批回滚死循环）；**na（cluster 级单点处置、退批 v3.5.5）与 `reentry_same_version`（claim 级单条）及 `input_truncated`（claim 级单条，v3.5.6）不产批**、走 cluster 级 needs-review-resolve，避免无主堆积。**v3.5.8 R-14/R-15**：unclean_run 只经批载体处置、批引 cluster **保持 claim、不入 needs_review 态**，cluster 详情/列表实时派生「被未决批 Bx 挂起」标注（join link_refs 免加列）；批引用期 cluster TTL 不豁免——到期自然回退 open、resolve 时 R-9 skipped_already_open 幂等覆盖；cluster 同现 input_truncated 证据（双条件同现）→ 走截断单条、**不并入 unclean_run 批**（批聚合排除截断 cluster）。
...
| claim | 认领填 fix_version | → fixed（auto_regression：该 case 自 fix_version 起**连续 claim_k 个（claim 固化 K，v3.5.5）纯净可判版本回归 run 均无 fail**，closed_by=auto_regression，§10.5）/ fixed（admin 复核通过，closed_by=admin_review）/ **open（回归 failed 或复核窗口 TTL 超时未复核 → 自动回退，错误重新暴露与计数）** / needs_review（该 fix_version 回归 run 结果行判定产物：na / reentry_same_version / input_truncated（v3.5.6 R-10）；**unclean_run 不入本态——只经 needs_review_batch 载体、批引 cluster 保持 claim 挂起（v3.5.8 R-14）**，§10.5） |
| needs_review | v1 源 = claim 回归 error run 结果行的判定产物，reason 值域 = {`na`（na 结果行；**cluster 级单点、退批 v3.5.5**）、`unclean_run`（含**环境级** na run 内 pass 行，v3.5.5 窄化；**v3.5.8 R-14：本态不驻——unclean_run 只触发建批、不使 cluster 入 needs_review 态**）、`reentry_same_version`（reentry 产物同版本 claim 的旧 run pass 行，claim 级单条）、`input_truncated`（复现 input 超 8K 截断、证据不完整，claim 级单条，v3.5.6 R-10）}；**unclean_run 同 run 同环境级 error_type 聚单条批（needs_review_batch）整批处置**（引用 cluster 非 claim → R-9 语义化跳过：skipped_already_open/skipped_fixed/manual_review，v3.5.6）、na/reentry/input_truncated 走 cluster 级单点（§10.5）；判分 na/会话快照不足/cc 文件型均二期/D18 不回流，**run 超时非源**（兜底 = claim TTL 超窗回退 open） | → open（补充证据/升级） |
| fixed | 置 fixed | → open（reopen：复发/误判） |
| inactive | ignore | → open（反悔/复发再现重新开） |

- 复核窗口 TTL 默认 14 天（dict_config 可配）：claim 到期无回归结果、无 admin 复核 → 自动回退 open 并记 conversion_record（防"认领驻车"——被认领的错误在研发未跟进时不再复发回流、也不再出现在 open 计数，等于人工关掉了该错误的线上告警）。**该回退路径同时是「结果未达」的判定现场（v3.5.10）**——claim 侧判「结果未达」= `claim_ttl_expire` 转换记录在 ∧ `fix_version` 非空 ∧ 现行 pending link 零 `verify_run_record`（含抑制集：未超窗 / 过期记录须晚于锚 `assembled_ts` / 空 `assembled_ts`），派生读字段 `result_gap_suspected` 供前端警示。**收敛 = 两时钟先到者（v3.5.4，R6）**：K 序列满 → 置 fixed（TTL 停钟）；本 TTL 先到 → 回退 open（K 观察中止、下轮 claim 重计），K 观察/判定推进不重置、不延长 TTL（§10.5）。
...
研发修复 → claim 时填 **fix_version**（待发布修复版本）→ offline 触发该 agent regression run（绑定该版本）
  → **offline 主动推送（v3.5.10 契约反转）**：offline 在回归 run 产出后 `POST /backflow/regression-results`（fire-and-forget、5s 超时、offline 侧 3 次重试 1s/2s/4s、**online 不重试**）→ online 在**接收端点内同事务**跑判定（**单错级，评审修正 + 决策 R5 锚定修正 + v3.5.10 契约反转**）。**判定数据源 = 本 link 已落的 `verify_run_record` 行集（推送载荷 `raw_json` 原样留档）+ cluster 字段**——不再是 online 轮询 offline `GET /runs` / `/runs/{id}/results` / `/agents/{a}/versions` 三只读面（该拉取链与 `offline_client` 已整删，online 判定链**零出站**）。以载荷内 `case_id` 对**锚定版本 = claim 填的 fix_version**：逐条判单错——错误在**已发布缺陷版 V_obs** 发现、回归 run 绑**待发布修复版 V_fix**，建 case 时记录的 trigger_version（V_obs）只能溯源"错误确存在于 V_obs"，**不能作回查锚点**（按 V_obs 取 run 恒取到缺陷版 run 或取不到）；回查取"绑定版本 = fix_version 的回归 run"（**不取"最新 run"**，防 v1.4/v1.5 先后触发拿错 run）；该 case 判定通过（**纯净 run + K 稳定序列，判据见下**）即该错 verify_status=passed → error_cluster 标 fixed（闭环）；与 run 级绿解耦（run 内他错仍红不阻塞本错 closed）
...
结果行 na（该 case 回归 run 判 na——infra 无法判定，判定素材缺失/复现失败等，带 error_type）→ 该 case 无终值：claimed cluster 置 needs_review（reason=`na`，**cluster 级单点处置、退批 v3.5.5**——na 不再聚合批；run 超时非源；兜底 = claim TTL 超窗回退 open）
case 在该 run 无结果行（缺行）≠ na → 无终值（**v3.5.10：不再「继续轮询」**——判定改由推送一次性事件驱动，缺行 = 本次推送未带本 case 结果，link 保持 pending 等下一版推送；**claim TTL 超窗仍兜底回退 open**，不触发 needs_review）。**原 v3.5.6 R-4（claimed case 缺行且 ∈ 该版 run 溢出列表 `excluded_case_ids` → 判「该版本窗口外欠测」）与 v3.5.8 R-16（recheck 缺行时自动拉该版 run 比对 `excluded_case_ids`）随 v3.5.10 契约反转整体作废**——`excluded_case_ids` 与 runs 只读面在反转后**不存在**、无从比对；缺行中断改由**载荷 `prev_terminal_version` + 本 link 已落行集**的 agent 级版本全集对照承载（见下「K 序列锚改锚」），claim 详情仍可提示「该 case 连续 N 版未纳入回归」供人工判断，但**不再有 excluded 核对这一步**
online 侧**无出站调用、不再有「回查失败 / 退避重试」路径**（v3.5.10）——推送失败由 **offline 侧重试（3 次 1s/2s/4s）** 兜底，再上层由 offline 侧 pull job / 告警承担；online 若长期未收到该 link 任何推送 → link 保持 pending，由 claim TTL 回退路径按「结果未达」（`result_gap_suspected`）在聚类详情提示（**online 不新增自身时钟**，§16）
auto-fixed 判据 = **claim 固化 K（claim_k）+ claimed-case 级纯净判据**（v3.5.5 = Task #4-③ R-1/R-2，同步 solution_detail §7.6 v1.5 / offline error-backflow-phase2.md v0.6.2 注记；后续 R-3~R-24 修订演进见本条 v3.5.6~v3.5.9 内联标注 / §7.6 v1.6~v1.9）：
  **K = claim 固化 claim_k**：claim 时把 K 固化为 error_cluster.claim_k（值域 {1,2}、claim API 可选入参 k、缺省取全局默认 2 并写死固化值）；自 fix_version 起**连续 claim_k 个**（claim_k=1 = 单版纯净 pass 即 fixed）纯净可判版本 run 均无 fail → verify passed → fixed（closed_by=auto_regression）；任一版 fail → 即时 reopen 回 open（K 清零）；K 只读固化值不实时读全局键；观察期零推进可降、推进后锁死（v3.5.5）；na 行/缺行中断观察、不计数不终止，轮询至 claim TTL。**v3.5.10 R-17 改锚（原「offline『agent 已见版本』versions 只读面」已随反转删除）**：自 fix_version 起的候选版本序列改以**本 link 已落 `verify_run_record` 行集**自建 ≥ fix_version 版本序，并以载荷 **`prev_terminal_version`（前次终态版本，值可空 = 无前次终态 run）**承载「中间是否跨过缺失版本」——单值水位无法枚举中间版，加该字段后跨缺版丢推送 → 判缺行中断观察、不续缺环（**防假连续 K 满 false-fixed 的目标不变**）；agent 级版本全集对照由 `verify._agent_versions`（link→cluster 两跳）承担；该版 error run 终态到达 → 补判推进（迟到结果由 rejudge_job 本地重放已落行兜底）
  **纯净可判（claimed case 级，v3.5.5 下钻）**：该 (agent,version) error run 达终态（status ∈ {completed, partial_failed, timeout, cancelled}）+ claimed cluster 对应 case 行为 pass + run 内无**环境级** na——判据锚点 = claimed case 行 + na error_type 影响域分流（环境/case 归类 = offline phase2 §6.4 权威标注，无标注新 error_type 默认从严）；纯净性由 online 从关联 run 的 na 行 error_type 消费派生、verify_run_record 不加纯净性列、不加 offline 字段，run 级 na_case==0 不再直接作判据
  **unclean_run（触发窄化 v3.5.5）**：含**环境级** na（circuit_open / interface_disabled / scheduler_unexecuted / http_client_error / pool_error 等）run 内 pass 行不置 passed → needs_review（reason=`unclean_run`，同 run 同环境级 error_type 聚单条批、不撞批键，见 §10.4/§7.2）；**case 级** na（timeout/connect_error/sse_parse_error/http_error/no_done/body_too_large/no_usage/contract_error/missing_assertion/assertion_shape 等；**v3.5.7 R-19：case 级清单字面量对齐 executor 真值 + 补入 no_usage/contract_error**，权威标注 = offline phase2 v0.7.1 §6.4）只说明该 case 无证据、**不牵连**他 cluster 已跑通 pass
  **reentry 产物**（generation>1）claim 同已完成版本 → 旧 run pass 属修复前证据 → needs_review（reason=`reentry_same_version`，claim 级单条不产批）
  **input 截断证据（v3.5.6 R-10）**：claimed case 关联 error_cluster 标 `input_truncated`（原始 input 超 8K 截断，复现输入 ≠ 真错误输入）→ 该版 run 结果**不计 K、不置 verify passed** → needs_review（reason=`input_truncated`，claim 级单条不产批、走 cluster 级单点同 reentry 通道）+ claim 详情提示「input 超长截断、复现证据不完整：建议人工复核或 agent 出小输入版本后重 claim」；后续小输入纯净版本 run 正常攒 K auto-fixed（截断防护在消费侧、offline 复现 pass 如实判不误判 na）。**v3.5.8 R-13：input_truncated = 消费侧判定态的 cluster 当前判定位**——同键窗口内新现小输入纯净打点（error 事件到达即按新现 trace 判定态刷新，不需等批/回填）→ 位刷新清 0、首现截断不再毒化同键后续纯净版（escape 可达）；cluster 已 closed 不刷新（终态只读不翻案）。**v3.5.8 R-15：同 run 同时现环境级 na → 该 case 仍走 input_truncated 单条、不并入 unclean_run 批（批聚合排除截断 cluster）**
  **收敛终点 = 两时钟先到者**：K 满 → fixed（TTL 停钟）；claim TTL(14d) 先到 → 回退 open（K 观察中止、下轮 claim 重计），K 观察/判定推进不延长 TTL
```
...
- **online 观察事实 reentry（v3.4.6）**：offline 判定"已复验/已修复"不是终局——online 保留该错误的**线上观察哨位**（按 error 去重键 agent+interface+error_type+input_hash 持续观察，与 §10.2 复发检测同源；L3 证据签名观察属二期 §4.4）：**错误在线上仍复发**（claim 声明 fix_version 已修复后仍再现）→ online 自动 reentry：新 cluster 重新 open + 经 conversion_record 标记"线上仍复发，claim 或回归失守"，提示研发核对 fix_version 是否真正覆盖线上路径；offline 侧对应地让该错误**回到待修复集**（§11 补强）。**版本门控（Task #4 B-5）**：仅当线上复发事件 `agent_version` 相对 `fix_version` 达「等值+日期前缀」序才开 reentry——两者均为被测 agent 自报版本字面量（不 parse tag 数值）：同 `YYYY.MM.DD` 日期前缀 → 仅**完全等值**放行、同日不等值按上线中**不开新 cluster**（⚠️ **批 50（#33）订正**：本处原写「只 `count+1`、不静默」，**与实现不符** —— 实测 `analyzer/cluster.py:199-205` 门控不过即 `return "blocked"`，**零落库**：不建簇、不 count、不 conv，该复发只在详情侧由判定态现算可见，见 solution_detail §7.5）；跨日按日期前缀字符串序；非该形态整体回退纯等值；同日 hotfix 复发走 reopen 人工路径。语义细节见实现基线 solution_detail §7.5。online = 观察事实权威（线上是否仍复发），offline = 验证事实权威（回归是否通过），二者互不替代、互相反馈。
- **判定守卫（v3.4.6，原「回查守卫」；v3.5.10 措辞随契约反转更新）**：**终态只读**——verify_status 已 passed/failed/superseded 的 link 不再被迟到推送改写（`judge_link` 入口 `verify_status != "pending"` → `no_progress` 固化；推送载荷不覆盖已定终态，只追加历史时间线）；**fix_version 守卫**——判定前校验 fix_version 存在且该版本结果已到达，版本号格式/存在性不合法 → pending + 提示，不静默判红（§7.2/§11 补强）。
- **闭环归属（边界）**：回归 run 的触发、版本绑定、与发布流程的门禁集成在 **offline 侧**（§11 #7，offline 改造方案范围）；online 负责生成用例集、**接收推送并判定** verify_status、展示回流看板 + 观察哨位 reentry。online 侧 P2 回流看板提供"该 agent 当前 open+claim 错误数 + 关联回归用例/复验状态"总览（claim 计入待闭环口径，与聚类列表一致）。
...
| 9 | 触发与版本绑定 | 回归 run 触发、版本绑定、发布门禁集成由 offline 侧定义（offline 改造方案范围）；online 依赖：组装统一 case payload（D19）落库供 pull-API 拉取（D20）、接收复验结果推送并判定 verify_status（按 fix_version，§10.5） |
...
> - **判定契约（单错级，修正 §10.5 全绿口径 + 决策 R5 锚定修正 v3.4.5 + v3.5.10 契约反转）**：`error_case_link` 建 case 时记录触发时 agent 版本（**仅溯源**）；**判定锚定版本 = claim 声明的 fix_version**（错误在已发布缺陷版发现、回归 run 绑待发布修复版——按触发版本取结果恒取错/取空，三方向独立确认的死锁）。**反转后 online 不再拉取 offline 结果**：offline 在回归 run 产出后 `POST /backflow/regression-results` 推送结果行（该 case pass 即 passed，与 run 级绿解耦——同 run 内他错修复不阻塞本错 closed），online 在接收端点内**同事务**逐条判单错。**offline 侧补齐（推送面）**：推送端点鉴权（预共享 secret / 服务账号）、载荷 schema（`case_id` / `case_type` / `pass_fail` / `version` / `run_id` / `prev_terminal_version` 等，`case_id` 重复 → 整单拒）、offline 侧失败重试（3 次 1s/2s/4s）与投递可观测。**原「online 侧 `list_runs` 按 agent+version+status 过滤 / `run_results` 返回 case_id+case_type+pass_fail」读面契约随反转作废**（该三读法与 `offline_client` 已整删）。
...
> - **quality 判分回写（决策 R5，二期）**：含 quality 的回归 case（二期），verifier 判分详情落 eval_result 后，**case 级 pass_fail 由 verifier 结果回写 run_results**（quality 达标线 → pass；na/error/不达标 → fail），使 §10.5 单错级判定对 quality 型 L3 用例同样生效。**error-only case（v1）的 pass_fail = executor 技术判定 ∧ verifier no_fallback（词表命中→fail）合成**——executor 判 pass 的 case 若 verifier 词表命中兜底话术仍判 fail，由 verifier 阶段在 run_results 落终值（不经 SCORING、不产 agent_score，§10.5 判定对 error 生效）。
...
> - **回归 run 与 manual 互斥槽（决策 R5：共存，不互斥）**：`create_run._MUTEX_STATUS`（pending/running）计数需评估——发布门禁触发回归时同 agent 常规 run 在跑会 409；**结论：回归 run 不占互斥槽、不参与 max_active_runs 计数，与 manual/held_out 共存**；代价是判定可用性窗口受常规 run 排队影响——online 判绿以 claim 的 fix_version 对应 run 的结果为准，跑哪个 run 属 offline 调度自由度。
> - **看板/清理豁免 + 回归 run 保留档（§17 #12，第三轮裁定）**：`/coverage` 与 gate stale_suites 的 case/suite 计数排除回归（否则门禁恒显"陈旧 suite"/覆盖率虚高）；`cleanup_rules` 按 agent+suite 保留 N=50 **只清 run 不清 case**（TestCase 从不被批量清理——#12 原"回归用例保留"描述失准，已改写为 **run 保留**）；对**回归 run 设独立保留档（pinned/豁免 cleanup）**，否则 R5 判定依据历史被清、闭环断；确需清理超老回归 run 时，其 pass_fail 终态先落归档快照（offline 侧），online 判定据此仍有判单错依据。
...
| 平台间 | offline ↔ online（pull 主导，D20） | offline 专用服务账号（evaluator 角色）+ 独立凭证，最小 API 面：**offline→online** pull-API（拉取 assembled payload，D19）+ case 激活/驳回态回写 + **offline→online 回归结果推送 `POST /backflow/regression-results`**（§10.5 单错级；v3.5.10 契约反转——原「online→offline 回归 run 结果回查」出站方向已删，**online 判定链零出站**）；**pull-API 契约加固（v3.5.1，安全评审）**：请求/响应显式带 `case_type` 白名单（v1 仅 `regression_error`）+ `schema_version`，非白名单 case_type 返回空集而非全量（防未来误组装流出）；响应 evidence 区二期字段（session_snapshot/retrieve_hit 等）v1 恒 null/缺省，offline 反序列化不因缺省误判为存在数据 |
...
| 角色 | 定位 | 权限范围 |
|---|---|---|
| viewer | 研发/负责人（排障与观测闭环） | 三个维度页面**查看** + 回流看板**人工操作**（ignore / **claim 认领（必填修复版本 fix_version + 说明，判定锚定依据）** / needs_review 处置，D11 研发闭环入口；置 fixed 需回归证据通过或 admin 复核，决策 R4；claim 后回归 failed 或超复核窗口 → 回退 open，§10.4）+ 正文查看 |
...
| 回流-错误聚类（/backflow） | cluster 列表/筛选（agent、接口、层、状态 open/**claim**/fixed/inactive/needs_review + **待 offline 拉取/确认**（关联 case offline_status=assembled/draft，§10.3））；代表 trace 与 input_hash；去重与生成计数；复验总览（open+claim 错误数 + 关联回归用例 verify 分布、**待修复集规模**（online 本地镜像推导，标注"近似 offline 权威集"，§11 补强 pending-fix））；**弃留墙入口（watchwall）【二期规划，v1 不提供】**：T3 弱信号（词表/agent 自报低置信、取数空弱但含存疑词）观察列表 + 抽样标注可转回流。**needs_review 状态注（Task #4 B-3 + v3.5.4 + v3.5.5 + v3.5.6 R-10）**：v1 源 = claim 回归 error run 结果行的判定产物，reason 值域 {`na`（na 结果行，infra 无法判定；**cluster 级单点、退批 v3.5.5**）、`unclean_run`（含**环境级** na run 内 pass 行）、`reentry_same_version`（reentry 产物同版本 claim 旧 run pass 行，claim 级单条）、`input_truncated`（复现 input 超 8K 截断、证据不完整，claim 级单条，v3.5.6）}；**unclean_run 同 run 同环境级 error_type 聚合单条批（needs_review_batch）整批处置**（引用 cluster 非 claim → 语义化跳过，v3.5.6 R-9）、na/reentry/input_truncated 走 cluster 级单点（§10.5）；C2 会话快照不足/cc 文件型/判分 na 均二期或 D18 不回流，run 超时非源（TTL 兜底）——筛选保留、缺省为空属正常非功能缺失 | viewer 查看 | P2 |
| 回流-聚类详情 | 关联回归用例（case_id/case_type/**offline 激活态 offline_status∈{assembled,draft,active,invalidated}**（assembled 标"已生成（待 offline 拉取）"/draft 标"已生成用例（待 offline 确认）"，§10.3）/verify_status、`source` 溯源、**该 case 历次回归 run 的 版本×pass/fail 时间线**）；conversion_record 审计时间线；**人工操作**：ignore、**claim 认领（必填 fix_version，可选 K k∈{1,2} 固化 claim_k，v3.5.5；**表单 fix_version 归一提示 + ∉ agent 已见版本 → 软校验告警确认 + 版本活跃度动态 K 建议，v3.5.6 R-5；v3.5.10：数据源 offline「agent 已见版本」只读面已随契约反转删除 → 该软校验降级为非硬拦提示，见 §10.4**）**、needs_review 处置（决策 R4：置 fixed 需该 link 回归 passed 或 admin 复核——admin 复核动作落本页，通过→fixed / 驳回→reopen，closed_by 记 admin_review）；**case 级操作（v3.5.1）**：对**未激活 case（offline_status∈{assembled,draft}）**可**人工 invalidated**（判无效注明原因、可 reopen，记 conversion_record）；**invalidated/rejected（error 自检失败）的 link 展示重推状态**——offline 侧重扫自愈中 / online 现场缺时由 **admin"重新组装/重推"**（invalidated→assembled 复位供 pull，复用 payload_id）；**批量重推 + 可愈性标注（v3.5.6 R-7）**：invalidated（online 现场内容缺）可筛选批量 requeue（按 agent/缺项过滤，勾选批量复位 assembled，逐行复用守卫/防抖、per-row 结果 requeued/skipped+原因回显），按 reject_detail 缺项标**可愈性**——字段缺/空词表补齐即愈可批量勾选、「版本不识别」不愈行默认禁勾强制确认（防无效反复重推）（**⚠️ 2026-09-11 改判**：同上——本行所述「批量重推」UI 整块**未做**，标注已改以行为数据判据落在**详情页单 link 重推**（`requeue_count≥2` 转双次强确认）；**批量路径已裁定不做（2026-09-11）**。详见 detail §9.3）；conversion_record 记批量操作（actor、筛选条件、逐行结果）；**active 后的 case 不提供人工 invalidate**（废弃走 superseded + reopen，防 offline/online 分叉，§10.5）；claim 行展示复核窗口剩余时间 + 固化 K（claim_k，v3.5.5）；超窗回退 open 提示；**claim 详情可诊断提示（v3.5.6）+ v3.5.10 更新**：R-4「该 case 连续 N 版未纳入回归（cap 窗口外欠测）非修复失败」+ 引导 admin 人工 fixed / agent 错误量治理提示（**v3.5.10：该诊断依赖的 `excluded_case_ids`/runs 只读面已随契约反转删除 → 自动判据作废，仅保留人工可读的「连续 N 版未纳入回归」提示**）；R-10「input 超长截断、复现证据不完整：建议人工复核或 agent 出小输入版本后重 claim」；**新增「结果未达」提示（v3.5.10）**：claim TTL 超窗且该 link 零 `verify_run_record`（派生字段 `result_gap_suspected`）→ 提示「该版本回归结果未达」 | viewer 可操作 + admin 复核 | P2 |
...
> 分两档：#1/#2/#3/#6/#7/#9 = **放量门禁**（维度 1/2 达标 + 版本溯源，任何 agent 放量前必过；#3 llm_call 覆盖同时支撑维度 2 的 LLM 级双指标；#9 版本注入支撑维度 1 溯源与 R5 判定锚定）；**#4/#8/#10 = 维度 3 开放验收（v1 error 侧）**，仅对申请/已开放维度 3 白名单的 agent 生效（作为 Step 2.4 加入白名单的前提），不入放量门禁（#10 词表覆盖度为 error 复验门禁前提，v3.5.1 增）；**#5 = 二期前置验收**（v1 无 quality 插桩，二期开放 L3 前补验）。
...
| 9 | 版本注入（R5 打点源） | SDK init 注入 `agent_version`（读 `AGENT_VERSION` env/构建注入，agent 侧 CI 保证），事件可溯源到部署版本——判定版本锚定与聚类溯源依赖，缺失即版本锚定无数据源 | §5.1/§4.1 |
...
│   │   ├── converter/      # offline 回归用例构造 + 调用 + 复验判定
│   │   ├── worker/         # 后台任务（聚类、用例组装、本地补判 rejudge、rollup；无出站轮询）
...
| **P2 回流闭环** | 维度 3 **error 回流（L1/L2，v1 定稿范围）**（gq/cs/sp）；offline 配套（§11 v1 error-only 子集，独立方案独立评审）；回归复验闭环 | 真实 LLM 错误自动生成回归用例且不重复；修复后该错误经回归 run **连续 claim_k 个（claim 固化 K）claimed-case 纯净可判版本 run 均无 fail → 标已复验**（run 级全绿口径已修正为单错级，auto-fixed = claim_k + claimed-case 级纯净判据，§10.5）；**降级劣化基础可见性验收（v1）**：制造一次 LLM 失败被业务兜底吸收（request 仍 ok），确认 trace 中 `request ok + llm_call error` 子节点红显、llm_call 失败率可见（埋点前提验收细目见 §13.0 checklist #4）；**needs_review（v1 源 = claim 回归 error run 结果行的判定产物：na / unclean_run / reentry_same_version / input_truncated（v3.5.6 R-10）；unclean_run（含环境级 na run 内 pass 行）同 run 同 error_type 聚合单批、na/reentry_same_version/input_truncated cluster 级单点（claim 级单条）；run 超时非源）不阻塞闭环**，处置路径见 §10.4/§10.5；**占比>50% 回炉补证据指标移入二期**（随 C2 会话型回归落地；v1 error 型复现不依赖会话历史） |
...
| error_cluster / 建 case 并发与重试非幂等（评审） | 中 | §7.2 去重唯一索引 + link 幂等键 + 处理位点（§6.2）；判定不因 cluster 归档跳过在途 case（防 case pass 而 cluster 永不 fixed）；仅人工 ignore（link 已 superseded）的 cluster 停判定 |
| needs_review 聚类无主堆积/无人认领（评审） | 中（v1 源 = claim 回归 error run 结果行的判定产物，reason 值域 {`na`、`unclean_run`、`reentry_same_version`、`input_truncated`（v3.5.6 R-10）}；**v3.5.5：unclean_run（含环境级 na run 内 pass 行）同 run 同 error_type 聚合单条批、na cluster 级单点（退批）、reentry_same_version claim 级单条、input_truncated claim 级单条（v3.5.6）**，run 超时非源；判分 na/会话快照不足/cc 文件型二期/D18 不回流；§10.4/§10.5/§12.1） | 属主 viewer 认领处置 + unclean_run 批整批同动作处置（引用 cluster 非 claim → 语义化跳过，v3.5.6 R-9）+ na/reentry/input_truncated cluster 级单点处置（补充证据回 open / 超时升级，§10.4）+ claim TTL 超窗兜底回退 open + K 序列（claim_k）满 auto-fixed（TTL 停钟，§10.5） |
...
| R5 判定无版本数据源/锚定取空（评审） | 中 | 事件 schema + SDK 注入 agent_version；判定锚定 claim.fix_version；fix_version 无对应 run → pending + 提示（§10.5/§11/§13.0 #9） |
...
| 12 | offline 回归 run 保留 | **回归 run 独立保留档**（已定，v3.4.5）：pinned/豁免 cleanup_rules——cleanup 删 run 不删 case（TestCase 从不被批量清理，原"回归用例保留"描述失准已改写为 run 保留）；确需清理超老回归 run 前先归档其 pass_fail 终态快照，判定仍有判单错依据 | 随常规清理（弃：判定依据历史被清，R5 闭环断） |
...
| 14 | 公共 API 网关拓扑与 SSO 透传（v3.5.2 部署边界派生）**【2026-09-14 拆半：#14a SSO 已定 / #14b 网关拓扑待 infra 回执】** | **#14a SSO（已定，2026-09-14）**：平台内部自持 JWT（HS256 access 15min + 不透明 refresh 7d、sha256 存 `user_session`）、自建登录页、**不依赖任何网关注入头** ⇒ **登录取代网关侧 SSO**（= 当前实现，零改动成本）。⚠️ **本定案带条件、非永久**：若 infra 后续强制前置 SSO，则按备选列重新定案——本行「已定」**不构成对备选方案的否决**。**#14b 网关拓扑（待 infra 回执，未定）**：公网网关终止 TLS + 域名（`{env}.` 参数化）；backend 仅接受网关转发来源；offline↔online 平台间走内网、不经公网网关 —— 四点提问单见 `docs/infra-access-matrix.md`「网关/SSO 定案提问单」 | 若 infra 强制前置 SSO → 需 infra 提供**带签名**的用户身份透传头、backend 验签（防伪造）；P0 上线前与 infra 敲定 |
...
| v3.5.3 | **Task #4 B 包语义同步（对齐 solution_detail v1.3 / offline error-backflow-phase2.md v0.5.1，无新技术决策）**：needs_review v1 源改定 = **claim 回归 error run 的 `na` 结果行**（case 级、reason 带 run_id+error_type，同 run 同 error_type 聚合单条 review 批、整批同动作处置）——原「仅 claim 回归 infra-error 边缘触发」弃用；判分 na/会话快照不足/cc 文件型明确二期/D18 不回流、**run 超时非源**（兜底 = claim TTL 超窗回退 open）；同步 §10.4 状态机（L428/L429 + 处置 intro）、§10.5 回查 code block 补 na/缺行分支、§12.1 needs_review 状态注、§14 P2 验收、§16 风险行；§10.5 reentry 补**版本门控 = 等值+日期前缀序**（同 `YYYY.MM.DD` 前缀仅完全等值放行、同日不等值只 count+1 且 fixed/claim 详情提示不静默、跨日按日期前缀串序、非日期形态回退纯等值；agent_version/fix_version 为被测 agent 自报字面量、不 parse tag；语义细节见 solution_detail §7.5） |
| v3.5.4 | **Task #4 R5-R7 auto-fixed 判据重构落字（权威同步 solution_detail v1.4 / offline error-backflow-phase2.md v0.6.1）+ D2 载体吸收，无 offline 机制改动**：①**auto-fixed = 纯净 run + 跨版本稳定序列 K**（§10.4/§10.5/§7.2）——auto-fixed 仅接受「纯净可判」run：error run 达终态（status ∈ {completed, partial_failed, timeout, cancelled}）且 run 级 na_case==0（纯净性由 online 从关联 run 的 na_case 消费派生、verify_run_record 不加纯净性列、不加 offline 字段）；自 claim fix_version 起**连续 K 个**（默认 2、dict_config 可配；K=1 兼容单版）纯净可判版本 run 均无 fail → verify passed → fixed（closed_by=auto_regression），任一版 fail 即时 reopen（K 清零）、na 行/缺行中断观察不计数、轮询至 claim TTL；②**reason 值域扩三源**（§10.4/§10.5/§12.1/§15 P2/§16）——`na`（na 结果行）沿用 batch、`unclean_run`（含技术 na run 内 pass 行，同 run 同 error_type 聚单条批、na+pass 并入不撞批键）、`reentry_same_version`（reentry 产物同版本 claim 旧 run pass 行，claim 级单条不产批）；③**K-TTL 两时钟收敛终点**（§10.4 TTL bullet）——K 满 → fixed（TTL 停钟）/ claim TTL 先到 → 回退 open（K 观察中止、下轮 claim 重计），K 观察/回查推进不延长 TTL；④**D2 载体吸收**（§7.2）——概览表补 `verify_run_record`（跨版本稳定序列 K 载体）+ `needs_review_batch`（reason 值域 {na, unclean_run} 聚合载体，同 (run,error_type) 至多单条、整批单动作处置）。承接 solution_detail v1.4（§7.6 auto-fixed 判据 blockquote / §5.1 verify_run_record+needs_review_batch DDL / §7.5+§8.4 reentry 联动 / §10.1 claim TTL） |
| v3.5.5 | **Task #4-③ R-1/R-2 auto-fixed 判据精化落字（权威同步 solution_detail v1.5 / offline error-backflow-phase2.md v0.6.2 注记，消费语义、无 offline 机制改动）**：①**K 改 claim 级固化 `claim_k`**（§10.4/§10.5/§7.2/§12.1）——error_cluster 增 claim_k 列、claim API 入参可选 k∈{1,2} 缺省取全局默认 2（dict_config 补 auto_fixed_k_default 键）并写死固化值、claim 生命周期内**零推进可降 / 推进后锁死**、claim_k=1 = 单版纯净 pass 即 fixed、K 只读固化值不实时读全局键；②**纯净判据由 run 级 na_case==0 下钻 claimed case 行 + na error_type 影响域分流**（§10.5/§7.2）——环境级 na（circuit_open/interface_disabled/scheduler_unexecuted/http_client_error/pool_error）→ unclean_run 触发、case 级 na（timeout/connect/sse_parse/http/no_done/body_too_large/missing_assertion/assertion_shape）不牵连他 cluster pass（归类权威 = offline phase2 §6.4 影响域标注，无标注新 error_type 默认从严）；③**unclean_run 触发窄化 + needs_review_batch 纯化仅 unclean_run 批 + na 退批改 cluster 级单点 + 撞键裁定收窄**（na 不再被并入批无辜批量 reopen）；④重 claim 同 fix_version 动态提示并入 R-5 版本活跃度查询范围（R-1 表单只留接口）。承接 solution_detail v1.5（§7.6 判定语义 v1.5 blockquote / §5.1 ④⑥⑦b DDL / §8.4 claim+review 端点 / §10.1 auto_fixed_k_default / §1.4/§12.1 登记） |
| v3.5.6 | **Task #4 修订包 R-3~R-10 语义权威同步（对齐 solution_detail v1.6 / offline error-backflow-phase2.md v0.7；R-3/R-8/R-4-5 offline 只读面 = offline 机制改动单独立项、solution 仅同步 online 消费语义）**：①**R-4 cap 溢出 case 回查可诊断**（§10.5 缺行 bullet /§10.4/§12.1）——claimed case 缺行且 ∈ 该版 run 溢出 case 列表 → 判「该版本窗口外欠测」非修复失败 → claim 详情诊断 + admin 人工 fixed / 错误量治理引导，不静默 14d 转圈、不跨端保窗口；②**R-5 claim 软校验**（§10.4/§12.1）——fix_version 归一提示 + ∉ agent 已见版本 → 告警确认（数据源 = offline「agent 已见版本」只读面，非硬拦）+ 版本活跃度动态 K 提示同源（R-1③ 兑现）；③**R-7 invalidated 批量 requeue + 可愈性标注**（§10.3/§12.1）——批量复位 assembled 供 pull、逐行守卫/防抖、per-row 结果、字段缺/词表空可愈 vs 版本不识别不愈禁勾强确认；④**R-9 needs_review_batch resolve 并发语义**（§7.2/§10.4）——逐引用 CAS + 非 claim 语义化跳过（skipped_already_open/skipped_fixed/manual_review）、批整体单事务置 resolved、批级 CAS 防双 admin；⑤**R-10 input 截断 + reason 值域 3→4**（§7.2/§10.2/§10.4/§10.5/§12.1）——error_cluster/error_case_link 标 `input_truncated`（>8K 截断）、截断关联 run 不计 K → needs_review（reason=`input_truncated` claim 级单条不产批）+ UI 提示、input_hash normalize 截断 4096→8192（对齐复现输入上限）；reason 值域 {na, unclean_run, reentry_same_version, input_truncated} 三文档一致。承接 solution_detail v1.6（§4.3/§5.1/§7.6/§8.4/§9.3 修订包落字） |
| v3.5.7 | **Task #4-③ 高危 2 条 R-18/R-19 落字（权威同步 solution_detail v1.7 / offline error-backflow-phase2.md v0.7.1）**：① **R-18 input 明文载体补断 + input_truncated 判定点下钻**（§7.2）——trace_judge_state 判定态仅持久 root_input_hash 的明文载体真空补列 `input_snapshot_clean`（脱敏截断≤8K 明文快照）+ `input_truncated`，**消费 worker root 到达同刻落库**（§6.1 step4 唯一见原始明文处）；error_cluster 开 cluster/窗口内回填自判定态列复制（兑现「不依赖 ES 回读」v3.5.1 承诺，修复静默组装断点）；判定点由「cluster 首现/回填时判」下钻「消费 step4 root 到达时判」（聚类/组装仅见 ≤8K 截断快照不可判，H7-1 联动修正）；② **R-19 error_type 判据字面量对齐 executor 真值 + 补漏项**（§10.5/§7.2）——case 级清单 `connect/sse_parse/http` → `connect_error/sse_parse_error/http_error` + 补 `no_usage`/`contract_error`（executor 非 RETRYABLE 直接 na），防短名错位被当「未知 error_type → 默认从严环境级」系统性假 unclean_run，从严兜底回归「仅兜底非常态」。承接 solution_detail v1.7（§7.6 影响域矩阵 + §5.1/§4.3 input_snapshot_clean/input_truncated DDL + §6.2 开 cluster/回填取数源） |
| v3.5.8 | **Task #4-③ H7 批 B 类 5 条 R-13~R-17 落字（权威同步 solution_detail v1.8；offline 零机制，R-16 读面字段位置 + R-13 §6.2 刷新复制留 Phase B 核对/立项）**：① **R-13 input_truncated 判定位窗口化**（§7.2/§10.5）——截断标记语义改「消费侧判定态的 cluster 当前判定位」：同键窗口内新现小输入纯净 trace 到达 → 开簇/回填复制时刷新清 0 → 后续版 run 可攒 K（escape 可达）；cluster 已 closed 不刷新不翻案；② **R-14 unclean_run 载体口径归一**（§10.4/§10.5）——unclean_run 只经 needs_review_batch 处置、cluster 不入 needs_review 态，批引 claim cluster 实时派生「被未决批挂起」标注、TTL 不豁免沿用既有兜底（R-9 skipped 幂等覆盖）；③ **R-15 双通道优先级 = input_truncated 优先**（§10.5）——同 run 双条件走截断单条、批聚合排除截断 cluster；④ **R-16 缺行诊断自动核对**（§10.5）——缺行自动拉 runs 比对 excluded_case_ids 判「窗口外欠测」；⑤ **R-17 K 序列版本锚 = versions 读面**（§10.5）——候选版本序以 agent 已见版本全集为准，中间有发版无 error run → 缺行中断（杜绝 v1+v3 假连续 false-fixed）。承接 solution_detail v1.8（§7.6 v1.8 blockquote / §5.1④⑥ / §6.2 窗口刷新 / §8.7 runs+versions / §14 E-23~E-27） |
| v3.5.9 | **Task #4-③ H7 批 C/低危 5 条 R-20~R-24 落字（权威同步 solution_detail v1.9 / offline error-backflow-phase2.md v0.7.2）**：① **R-20 空答 fail 语义定性 + pre-scan 门禁立项**（§10.5 注）——R-12 core 空答 fail 不动；复现 run 空答 fail = leakage 空话术复发证据（与词表命中 fail 同属未修复、证据形态不同）、空答不落 na 落 verifier fail 保持；pre-scan 显式立项 Phase B code_detail 实施门禁（owner = offline 实施）；② **R-21 root-late 补判通道**（§7.2/§10.2）——残 trace 判定（judged=1）后迟到的 root（root_ok 0→1、root_status∈{error,timeout}）不重跑整 trace，消费 step4 仅补一次 root 级候选（root_error_type 走 L1/L2 值域筛）→ 复用聚类（同键 count+1/补快照、异键开新簇、已 closed 跳过）；幂等 = trace_judge_state.root_late_complement CAS 标记；③ **R-22 收尾回填 na 统一 scheduler_unexecuted**（§10.5 注）——scanner 回收/orchestrator cancel/run 级超时收尾三路径回填 na 同源 error_type、「na case 必带 error_type」不变量全覆盖收尾路径、timeout 不作收尾回填原因（offline phase2 §6.3/§6.4）；④ **R-23 timeout 三层语义分界**（§10.5）——case 级 na 源 / run 终态 timeout 不直接触发 needs_review（B-3 兜底 = claim TTL 回退 open）/ run 级超时收尾回填 na（环境级）→ 关联 pass case 入 unclean_run 批——「run 超时非源」排除直接驱动、不排除环境级回填污染间接作用；⑤ **R-24 requeue guard 状态域精确化**（§10.4/§7.2）——requeue 复位条件 = cluster.status ∈ {open, claim, needs_review}（fixed/inactive 禁、走 superseded+reopen 重建）+ 锚点保护（不动 fix_version/claim_k/TTL）+ verify 兜底 = 仅 pending invalidated link。承接 solution_detail v1.9（§4.3④/§4.4/§5.1⑩ root_late_complement / §6.1-6.2 root-late / §7.4/§9.4 R-24 / §7.6 v1.9 blockquote / §14 E-28~E-29） |
| v3.5.10 | **平台间契约反转语义同步（对齐 solution_detail v1.23；第 1~3 刀，判定链方向反转 = offline 主动推、online 零出站）**：① **回查判定不再是拉取型 job**——原 `worker/recheck_job.py`（60s 轮询 offline `GET /runs` / `/runs/{id}/results` / `/agents/{a}/versions` 三读面）与 `core/offline_client.py` **已整删**，`RECHECK_INTERVAL_S` 删除；改 offline 在回归 run 产出后 `POST /backflow/regression-results`（fire-and-forget、5s 超时、offline 侧 3 次重试 1s/2s/4s、**online 不重试**），online 在**接收端点内同事务**跑 `verify.judge_link`；`judge_link` 去 `client` 参数，判定数据源 = **本 link 已落 `verify_run_record` 行集**（载荷 `raw_json` 原样留档）**+ cluster 字段**（§10.5 判定数据源改写）。② **载荷契约变更**——**删** `bound_version_first_seen`（强制但零读点）、**增必填** `prev_terminal_version`（值可空 = 无前次终态 run；堵「单值水位无法枚举中间版本 → 跨缺版丢推送 → 假累积 K → 静默误判为已修」）+ `agent_latest_version`（offline 侧该 agent 已有终态 run 的最大版本，恢复发版水位：`V > agent_latest_version` = offline 还没跑到该版本 → 保持 `no_progress`；**它判不了「水位之内是否漏收了一笔」，后者由 `prev_terminal_version` 承载**）；`case_id` 重复 → 整单拒。③ **缺行中断判据改 agent 级**（`verify._agent_versions`，link→cluster 两跳）承载——原 **R-17「K 序列版本锚 = offline『agent 已见版本』versions 只读面」改锚**（版本全集来源改本 link 已落行集 + `prev_terminal_version`，**防假连续目标不变**）；原 **R-4/R-16 依赖的 `excluded_case_ids` 只读面不存在** → 缺行诊断不再沿用「自动拉 run 比对溢出列表」，**相应作废**（§10.5 缺行 bullet 改写）。④ **判定安全网新增** `worker/rejudge_job.py`（60s、纯本地、**零 outbound**）——扫 `status=='claim'` ∧ 有现行 pending link 的簇、重放本 link 已落行补判一次，覆盖「① claim 晚于推送 ② 判定抛异常被降级 ③ 状态过渡窗口」三类现场（原 recheck 的安全来自「循环」而非守卫本身，删轮询即删安全网，故必须补回），收口判据 = 「本批零终态迁移」非「空批」；**job 数 = 6（judge_scan / cluster / assemble / claim_ttl / rejudge / rollup，6→5→6）**。⑤ **「结果未达」claim 判据收敛**（§10.4/§10.5）——claim 侧不再靠轮询超时判定：由 `claim_ttl_expire` conv 记录在 ∧ `fix_version` 非空 ∧ 现行 pending link 零 `verify_run_record` 判「结果未达」，含抑制集（未超窗 / 过期记录须晚于锚 `assembled_ts` / 空 `assembled_ts`）；派生读字段 `result_gap_suspected`（**零 DDL**）+ 前端警示行。⑥ **保留的 pull 方向不变**——online 组装 payload 供 offline `pull-API` 拉取（D19/D20）不受本反转影响，反转仅针对**结果回传方向**。承接 solution_detail v1.23（§1.3 周期表 / §7.3 / §7.5 / §7.6 判定语义块 / §8.7~§8.9 / §12.2 O-2 / §14.2 E-7·E-26 作废 / §14.4） |
| v3.5.11 | **R-7 改判权威同步（对齐 register R-7 改判记录 + solution_detail v1.23 就地订正注）**：原判据（按 `reject_detail` 缺项分「可愈性」——字段缺/词表空=可愈、「版本不识别」=不愈默认禁勾）经查**不成立**——① **「不愈类」在跨端链路上不可达（2026-09-11 重估终版，前两版均误）**：`version_drift` 是**死分支**——触发条件被 **online pull 层上游**挡死（版本不匹配 → 400 `ERR_PULL_0002` 拒单，`pull.py:88`，实测 `test_backflow.py:237`；`case_type` 非白名单 → 返空集，`pull.py:94`），offline 声明与校验均为常量 `"1.0"`（offline `solution_detail.md:292`/`:309`）⇒ **收不到会触发它的信封** ⇒ 「版本不识别」类不落行、不进 `requeue` 集合；**改判理由 = 对象不可达，非「成本/收益不划算」**；② **「跨端扩词表」腿作废**：ack 线路**有 `reason` 槽**（`api/pull.py:63`）但**无对象可区分**，扩词表亦无意义；③ 实害路径**「无调用方」，非「关闭」（⚠️ 措辞订正）**（前端零批量入口已逐面取证 / offline 无 backflow 模块 / 脚本零命中；生产代码唯一调用点 = 端点自身）——⚠️ 端点此刻**是活的**：`require_admin` **只挡非 admin**，**admin 持 token 可 curl 直接触发**；**UI 关便利性、不关可达性**。**（⚠️ 2026-09-11 作废：本句系本行早前版本残留，与本行前半「改判理由 = 对象不可达，非「成本/收益不划算」」自相矛盾，已作废，勿引用）**。改用**行为数据判据**（该 link 历史重推次数 `requeue_count`，口径**不含本次**），范围 = **单 link 路径**（详情页 ≥2 转**双次强确认**）；**门控强度实为降档**（原 default-deny、覆盖批量勾选 → 现 default-allow + warn、仅单 link），**批量侧标注已裁定不做、本条结清（2026-09-11）**。**已知局限**：① `requeue_count` 只在重推**失败**时累积，区分不了「不可自愈」与「admin 手快」，现有数据无法证伪；② 口径不含本次 ⇒ **第 3 次**才触发。承接 §10.4 / UI 表两处订正注 |
| v3.5.12 | **§17 #14 拆半落定（#14a SSO 已定 / #14b 网关拓扑待 infra 回执）**：① **#14a SSO = 确认默认值「平台内部自持 JWT、登录取代网关侧 SSO」**，依据 = 实测实现已完全符合（`core/security.py:18/38/49-56` HS256 access 15min + 不透明 refresh 7d / sha256 存 `user_session`；自建登录页 `LoginView.vue`；`deps.py:26-35` 只读 `Authorization: Bearer`，全仓零网关注入头依赖）⇒ **零实现成本**；**备选方案（infra 前置 SSO + 签名身份透传头 + backend 验签）当前不可实现**——infra 全仓 `oidc|oauth|sso|keycloak|casdoor|auth_request|auth_jwt` 零命中、compose 无任何认证服务，无对接对象。⚠️ **本定案带条件、非永久**：若 infra 后续强制前置 SSO，按备选列重新定案，本行「已定」不构成对备选的否决。② **#14b 网关拓扑 = 待 infra 回执（未定）**，四点提问单落 `docs/infra-access-matrix.md`「网关/SSO 定案提问单」（TLS 终止在哪一层 / online 是否需网关 server 块 / 「backend 仅接受网关来源」落地形态 / `{env}.` 与 infra 现有 `{name}.local` 命名规则的关系）。③ **事实面订正**：`task.md:167` 既往句「网关运行时已存在、online 侧加一个 server 块即可验（`resolver` + `set $upstream` 运行时解析，加完即生效）」**只对转发面成立**——实测 `infra/api-gateway` = `nginx:1.27-alpine`、仅 `listen 8099` 明文、**无 443/无证书/无 `auth_request`/无 JWT 模块**、6 个 server 块（`gq/cs/cc/sp/eval.local` + 兜底 403）且 **online 零 server 块、零 upstream**（`eval.local` 指的是 offline）⇒ **网关就位 ≠ §17 #14 已落**。④ **新发现**：`solution.md:682`「浏览器 → 公共网关（终止 TLS）→ backend」与实际链路形态不符——实测 **offline 的 `frontend/nginx.conf:9/33` 才是公网边缘**（`browser → 前端 nginx → api-gateway:8099（明文）→ ai-eval-backend`），**「终止 TLS」实际不在网关层**，须与 infra 对齐。⑤ **「backend 仅接受网关来源」= 零实现**（无 `TrustedHostMiddleware`/`ProxyHeadersMiddleware`/来源校验；仅有「backend 不映射宿主端口」部署缓解），即 T-5.3 自 T-4.9 接过的「绕过网关直连 backend 被拒」断言当前无实现对象 |
