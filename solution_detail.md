...
> 修订记录：2026-09-10 **v1.21（T-3.9 验收阻塞项修正：兜底吸收误回流判定语义 + E-17 补探针，= T-3.10）**——① **缺陷**：§6.1 明写「兜底吸收现场（request ok + llm_call error）不产 L1/L2 候选（L3 二期）」，但 `analyzer/classify._collect_candidates` 对 `err_entries` 的迭代**无条件**、`decide()` 无前置守卫 → 请求成功的 trace 只要子节点有 llm 错误即产 L1 候选，回流建簇并触发 offline 回归，**虚报「agent 有 bug」白烧 offline 资源**。**长期未发现的根因 = 检测盲区是结构性的**：全部既有用例的 `root_status` 只取 `"error"`/`"timeout"`，**从未取过 `"ok"`**。② **为何真实可达而非理论构造**：`root_ok` 语义 = 「root 已到达」非「root 成功」（`consumer/state.py` root 到达即置 1、status 另存一列；R-21 迟到 root 同构造）；「request ok + llm_call error」是 SDK 既定埋点形态（`record_llm` docstring：裸调用「先记 error 再抛」由 agent 包装点保证，§2.4 前提）；`judge_scan_job._facts_from_row` 零过滤透传 → 缺陷完整穿透到回流。实测：`root_ok=1 / root_status='ok' / root_error_type=None / err_entries=[llm_timeout]` 过完整 `decide()` → `layer="L1"`（期望 `none`）。③ **修正**：门控落 `_collect_candidates` 局部而非 `decide()` 前置——当前唯一生产调用方即 `decide()`，门控贴着迭代放，未来新增调用方自动继承该不变量；判据 = **`root_status == "ok"` 单条件**（**刻意不叠加 `root_ok`**：`root_status` 全仓唯一写点 = `consumer/state.py` 的 `state.update(root_ok=1, …, root_status=event.status, …)`——两者**同一条语句**置位，故 `root_status=="ok"` ⟹ `root_ok=1`，合取项恒真、可证冗余；留单条件 = 只押「root 终态」一个语义，不与派生布尔标志的将来定义漂移耦合。此收敛系用户 2026-09-10 裁定）。**判据为何不用「`root_error_type` 为空」**：timeout root 按 step5 同样不带 `error_type`，用后者会连带切掉 timeout 分支，那是本批未讨论的第二处行为变更；残 trace `root_status` 恒 NULL（§5.1⑩），故本门控与残 trace 分支天然不相交。④ **已知未决歧义（显式标注、本批不触碰）**：step5 只说 timeout 事件自身不产候选、对「timeout root 的子节点 error 是否参与」沉默，而 step4 又写 L2 判定用「request **或**子节点」——两条互斥原文未消解，timeout root 的子节点候选**维持改动前既有行为**（仍参与），待真实流量再定，勿凭原文硬猜。⑤ **E-17 补探针 C-15**（原零覆盖：`claim_probe` 全文件 grep `payload_id` = 0 命中，C-4 只覆盖前半段）——reopen 后改 fix_version 再 claim → 组装出**新 link + 新 payload_id**（非复用旧 link）→ 新版本回归 passed → fixed，且**旧 failed 时间线保留不覆写**；配套新助手 `_assemble_new_link`（**不复用 `_activate`**：其 `.one()` 无 pending 筛选，旧终态 link 在场会 `MultipleResultsFound`）。⑥ 验证：backend 单测 **388 passed**（基线 385 + 3 新增，**既有 `decide()` 用例零调整**——已逐条核实无一构造 `root_status="ok"`，改动在参数空间上够不着）；原缺陷复现点由 `L1` 归零为 `none`；**claim_probe 容器内真库 15/15 全绿**，cluster 10/10 / assemble 8/8 / pull 13/13 零回归；本窗口新增后端 diff **ruff 零新增**（`classify.py` F401·E501 与 `test_analyzer_classify.py` I001 三条经 HEAD 基线比对确认为存量）。**本批只消解兜底吸收这一处可判定冲突，不改 §6.1 step4「request 或子节点」与 §2.5 L329「子节点不单独定层」的原文歧义本身**；`root_late_decision` / `root_late_complement` 是另一条不调 `_collect_candidates` 的路径，零交集。solution v3.5.9 / offline phase1 v0.2.2 / phase2 v0.7.2 语义基线不动。
> 修订记录：2026-09-10 **v1.22（T-3.6 收口：§1.3 时间驱动 job 周期表与实现对齐 + 三项剩余核查结清，纯文档口径修正）**——① **漂移（本版修正对象）**：§1.3 原文写「judge_scan/cluster/assemble 1min；claim_ttl 10min；recheck 1~5min；reentry 1~5min」，而实现在 P2-1~P2-5 落地过程中已改为 `backend/app/worker/main.py:35-42` 常量——judge_scan 60s、assemble 60s（与原文 1min 等价，仅换算表示不同）；**cluster 15s**（P2-1 有意提速：判定到期即应尽快归并进 error_cluster）；**claim_ttl 60s、recheck 60s**（P2-4 有意选择：TTL 与回查取与 assemble 同级粒度——§7.2「无在线拉取时钟」下全周期 job 驱动，60s 比 10min 更早发现超窗与离线回写）；**reentry 无独立周期**（2026-09-09 用户拍板内联 cluster_job、不落 reentry_job.py，故原「1~5min」一栏撤销）。② **为何改文档而非改代码**：上述常量均为已收口批次（P2-1/P2-4）的**实现选择**，改代码即行为变更并牵连 job 触发节奏；且该行原文自身标注「均【实现约定】」——约定随实现走。故本版以 `main.py` 常量为**唯一事实源**并显式声明，避免后续再各说各话。③ **T-3.6 三项剩余核查结清（均已由既有批次落地，非本版新实现）**：**分布式单飞 CAS** —— `judge_scan_job.py:103-112`（`WHERE judged=0` + rowcount 判命中，他方已判则跳过不覆写）/ `cluster_job.py:79-89`（`processed` CAS，rowcount≠1 抛 `_RowRaced` 回退 + `uk_cluster_dedup` 冲突吸收）/ `assemble_job.py:31-34,63-72`（候选排除 pending link + `uk_link_current`/`uk_link_payload` savepoint 吸收）/ `claim_ttl_job.py:63-74`（`status='claim' AND claim_due_ts<now` CAS，raced 跳过）；**失败退避** —— `worker/main.py:111-188` 六个 loop 统一 `except → logger.exception + 定周期 sleep` 自愈（**形态 = 定间隔重试，非指数退避**；指数退避全仓仅存于 consumer `_backoff` 与 `offline_client`，勿混谈）；**requeue 防抖** —— `requeue.py:29` `REQUEUE_DEBOUNCE_MINUTES = 5`，纯守卫 `requeue_guard_errors` + CAS 落库 `WHERE assembled_ts <= cutoff` **双校验**、锚 `assembled_ts`（P2-3 R-24）。④ **本版明确不计入 T-3.6 的部分**：**双实例单飞并发验证**归 T-4.5（task.md 原文已列「E-16 双实例单飞」）；**rollup 真实 ES 集成探针**留集成阶段（`test_rollup.py` docstring 自陈；rollup 本身是确定性 `_id` 覆写幂等设计，不适用锁）；**「job 失败自监控计数可见」不在 §14.4 写侧判据内**（该判据只要求「各时间驱动 job 单飞无重复执行」+「requeue 防抖 ≥5min 生效」两件事），故**不因 task.md 的措辞在此新编实现**——worker 侧现无失败计数器 / heartbeat doc / metrics 端点属实，若后续要补须另行立 T 项并先定可见面挂靠；consumer 侧 `DroppedCounter` 与 heartbeat `dropped` 字段属**事件丢弃**计数、与 job 失败无关，不可挪用。⑤ 本版**零代码改动**，仅对齐 §1.3 一行 + 登记本行。
> 修订记录：2026-09-10 **v1.23（平台间契约反转：回归结果由 offline 主动推送，online 出站依赖归零）**——① **动机**：online 原持三个 offline 只读面（`GET /runs`、`GET /runs/{id}/results`、`GET /agents/{a}/versions`），由 `worker/recheck_job.py`（60s）轮询驱动 claim 回查判定。「online 主动查 offline」与本平台实际方向相反——回归 run 是 **offline 跑的**，online 只是"等结果的人"；且该设计使 online 单仓无法独立验收。② **新契约**：新增 `POST /backflow/regression-results`（§8.7 载荷表）——offline 在 `error_regression` run 终态 commit 后 fire-and-forget 推送「run 终态 + 逐 case 原始行」，online 收到即落 `verify_run_record` 并重算该 link 判定。**判定内核（§7.6 单错级判定 / no_progress / fix_version 守卫）原样保留，仅数据源由「轮询拉取」改「被动接收」**——offline 本就只产原始行（`pass_fail + error_type`），合成结论（K 序列 / clean-unclean / reentry / needs_review）全在 online 侧，故**零判定逻辑搬迁**。③ **幂等与落点零新表**：幂等键 = 现成 `uk_verify_run(link_id, run_id)`（`models/error_flow.py:138`）；载荷字段与 `verify_run_record` 现成列一一对应（`run_id`/`bound_version`/`case_pass`/`run_status`/`raw_json`）——**无 DDL、无新表**。④ **作废清单**：三端点 + `BACKFLOW_INBOUND_SECRET`（`scope=platform:readonly`）凭证链；online 侧 `core/offline_client.py` 整删、`worker/recheck_job.py` 整删、`RECHECK_INTERVAL_S` 常量删（worker job **6 → 5**）。⑤ **R 条去向**：R-4（`excluded_case_ids`）与 R-16（字段位）**取消**（推送为全量对账，不存在"缺行→轮询"场景）；R-3 的 online 侧缺行语义同步取消（**offline 侧版本差集补建保留**）；R-5 降为载荷单字段 `bound_version_first_seen`（只保留守卫所需最小信息，不再暴露版本列表）。⑥ **两处真实能力损失（显式记录，非等价替换）**：`api/backflow.py` claim 表单「fix_version ∉ agent 已见版本」软校验**失去数据源** → 降级为「online 已收结果的版本集」软提示；`verify.py`「fix_version 未发版 → `no_progress`」守卫改由 `bound_version_first_seen` 承载——**offline 不带该字段则守卫失效**（契约必填项，offline 侧不得省略）。**（⑭(b) 更正：该守卫实际由 `agent_latest_version` 单值水位承载，`bound_version_first_seen` 在本批已从载荷删除；本句的「由 XX 承载」为第 2 刀期的判断，据实回填。）**⑦ **新增必要配套（非可选优化）**：#7「回查失败待人工」与 #9「assembled 待 N 天」**合并为同一套阈值标记**——claim 态超 `claim_ttl` 仍无任何 `verify_run_record` / assembled 态超阈值 → 统一文案「**回查结果未达（疑似 offline 停摆），人工核查**」，读数 MySQL 现算。理由：fire-and-forget 下 online 无法感知 offline 存活，不做此标记 = 把原来可控的"停轮"换成**不可观测的死锁**。⑧ **验收边界**：环 2（E-23~E-29）在 online 侧以「自打 POST」验**接收侧**全链；真 offline 汇合标为遗留项，**上线门前必须清**。⑨ **offline 侧同步**：`error-backflow-phase1.md` §2.4/§8/§9 契约锚点与凭证链同步改写（该系列在 offline **代码零落地**，故为纯文档变更、**零代码返工**）。⑩ **交付边界**：本版只定契约；代码改造、`#7+#9` 标记、测试重写见后续批次。solution v3.5.9 语义基线不动；**offline phase1 因本次契约同步升 v0.2.3**，phase2 v0.7.2 待后续批次随 R 条改写一并升版。⑪ **第 2 刀开工时的两处实证修正（据实，非设计变更）**：**(a) 凭证形态**——online 实现为静态预共享 secret（`api/deps.py:50-58`），**非** offline §9 设计的 JWT；本版 ②/⑥ 处原写的「`scope=backflow:report` / 按端点校验 scope」为**未落实的机制描述**，已按 §8.8 改写为「三端点共用一 secret、不做分置」，offline §9 同步标注「online 未采用 JWT 体系」。**(b) 载荷新增 `agent_latest_version`（必填）**——`verify.py:247-255` 的**防假连续**原依赖 `list_runs` 返回「该版本有无终态 run」（`cur is None` 且 `V not in existing` → `gap_version` 中断）；推送模式下 online 不再查询，**「offline 还没跑到该版本」与「offline 跑过但无 run」不可区分**——按「无记录即中断」会让 claim 永远判不出 fixed（假阴性），按「无记录即跳过」会丢失防假连续（假阳性）。故补该字段作「offline 已覆盖版本水位」：`V ≤ 水位` 缺记录 = 真缺行→中断；`V > 水位` = 还没跑到→保持 pending。**offline 漏带则防假连续守卫失效**。两处均已落实到 §8.7 载荷表与 §8.8。⑫ **第 2 刀收口（本版内，2026-09-10）**：**(a) 「结果未达」claim 侧判据由「在线时窗」改「事件派生」**——原 ⑦ 写的「claim 态超 `claim_ttl` 仍无 `verify_run_record`」**在实现上恒为假**：`worker/claim_ttl_job.py:31-84` 每 60s 把所有超窗 claim 回退 `open` 并清 `claimed_at`/`claim_due_ts`（`fix_version` 保留），该现场**最长只存在 60s** → 被动探测落空。新判据 = 「存在 `claim_ttl_expire` 转换记录（回退时必写，持久证据）∧ `fix_version` 非空 ∧ 现行 pending link 零 `verify_run_record`」，并加**抑制条件**「处于未到期 claim 期内不标记」（被回退的 cluster 可再次 claim 而旧记录仍在，不抑制会在人工正常处置期间误报）。**(b) 阈值 N 定死** = `dict_config.claim_ttl_days`（默认 14），同源不新编魔法数，代价 = assembled 分支需等满 N 天。**(c) `finished_ts` 补 ISO8601 校验**（非法 → `ERR_PULL_0002`）。(d) **`cases_dropped` 口径定死**：幂等重放**恒返与首次相同**的值、**不归零**（理由见 §8.7「响应字段口径」）。**(e) `links_advanced` 口径定死** = 本次真正新落 run 行的 link（重复推送 / orphan 为空）。**(f) v1 硬约束**：一次回归 run 只对应一个 cluster。**(g) 新增真库探针** `backend/tests/integration/push_probe.py`（数据位落库 / 幂等重放 / orphan / 两类拒单 / 白名单分流 / `result_overdue` 两分支**含反例对照**——无对照组的绿是假绿）。**(h) claim 分支锚点补 `pending` 约束（修误报）**：原 ⑦/⑫(a) 未限锚点形态，实现沿用了「取不到 pending 就退最新一条 link」的兜底，导致 **已 `fixed` / 已 `inactive`** 的簇上四条件全成立而误报——`fixed_review(approve=True)` 把现行 link 打 `passed`（`claim.py:246`）、`ignore` 打 `superseded`（`api/backflow.py:602`），两者都使 link 离开 pending。改判据为「锚点必须是现行 pending link」；**为何不用 `cluster.status` 白名单**：白名单在将来新增非终态时静默漏报（正是本标记要修的失效模式），link 谓词被漏动只会误报——响亮地错优于安静地错。**但 `pending` 必要不充分**——`reopen` 后 `assemble_job` 重建全新 pending link，陈旧回退记录使前四条全成立 → 对刚组装 0 天的 link 误报。**故再加「回退记录须晚于 `anchor.assembled_ts`」**（本轮等待起点，`assembled_ts` 为空亦抑制），把 claim 分支的证据锁死在**当前这一轮等待**内。**(i) 载荷内 `case_id` 重复 → `ERR_PULL_0002` 拒单**（原实现 `next()` 静默取首条、无留痕，会掩盖 offline 侧拼装 bug）。⑬ **第 3 刀（判定内核切推送源 + 轮询链整删 + 缺行中断换锚，2026-09-10）**：**(a) 判定内核数据源切换**——`verify.judge_link` 去掉 `client` 参数，per-case 判定行、run 内环境级 na 明细、发版水位、缺行中断全部从 `verify_run_record.raw_json`（载荷**原样留档**）重派生（`cases[]` / `agent_latest_version` / `prev_terminal_version`），**判定规则本体（R-2 ①②③④⑤ / R-19 归类 / R-10 / R-15 / R-17 K 折叠）零改动**——offline 本就不产结论，无逻辑搬迁。**(b) 两处死代码清理**：原 `_pick_run`「completed 优先」在推送源下恒等（只有终态 run 会推）→ **删**；原 `_find_case_row` 的 `rows[0] if rows else None` 兜底 **不能沿用**——旧读面 `run_results(run_id, case_id=)` 已按 case 过滤故兜底安全，推送载荷 `cases[]` 是**全量**行，兜底会把**别的 case 的 pass/fail** 当成本 case 判定（**静默错判**，比 `missing` 危险得多）→ 改**严格命中**、未命中即 `missing`。**(c) 缺行中断换锚**：原「versions 读面有该版无 error run → 中断」改为 §8.7 载荷新增的 `prev_terminal_version` 单值指针对照**本地行集**（含「只看 `≥ fix_version` 前序」边界，理由见 §8.7 载荷表）。**(d) 轮询链整删**：`core/offline_client.py` + `worker/recheck_job.py` 整删、`RECHECK_INTERVAL_S` 删（worker job **6 → 5**）、`worker/__init__.py` job 清单同步；**`config.offline_base_url` 字段保留**（不在删除清单内，仅作部署 `.env` 兼容，注释标注「已无消费方」）。**(e) `links_advanced` 语义二次升级** = **真发生终态迁移的 link**（见 §8.7 响应字段口径）。**(f) 载荷新增必填 `prev_terminal_version`**（值可 `null`）——**offline 不带则防假连续第二道闸失效**。**(g) R-5 软提示降级实证**：claim 表单软校验数据源改为本地「已收结果的版本集」，**语义确已弱化**——合法的新 `fix_version` 在收到该版结果前会命中「未观测到该版本」软提示（**只提示不拦**，§8.4/§9.3）。**(h) 探针改造**：`claim_probe` C-3~C-7/C-12/C-14 由「假 offline 读面」改**直造 `verify_run_record`（含 `raw_json`）+ 推送端点/`judge_link` 驱动**、C-13 改推送编排；`push_probe` S-1 补**判定写回断言**（link/cluster 真变化）+ 新增 S-14/S-15 **缺行中断含反假绿对照**——**实测**停用该判据后 S-14 转红（cluster 变 `fixed`、`links_advanced` 非空）而 S-15 与其余场景仍绿，改回即全绿；容器真库 **push 17/17 · claim 15/15 · cluster 13/13 · assemble 10/10 · pull 13/13**。**(i) 残留风险（据实登记）**：① 第 2 刀期（002 号 writer）落库的历史行 `raw_json` **无 `cases` 键**（老 writer 只写 `case_pass`），第 3 刀判定读 `cases[]` 会把这些行判 `missing`——**不做数据迁移**（v1 上线前库内无真实流量，迁移无收益）；② `prev_terminal_version` 是 **agent 级**单值而判定行集是 **link 级**，同一 agent 多 cluster 并行 claim 时可能产生「水位内但不在本 link」的**假 `gap`**（**安全向**：只延后不误判，登记待观）。⑭ **第 4 批（判定安全网：轮询重试的替代 + ⑬(i) 两项残留闭合，2026-09-10）**：**(a) 缺行中断判据扫描范围 link 级 → agent 级**（闭合 ⑬(i)②）：`prev_terminal_version` 本身是 **agent 级**事实，原实现拿**本 link 的行集**去查，同 agent 多 cluster 并行 claim 时会产生**假 `gap`**（安全向但**静默拖住推进**、且无 UI 可见面）——现查**该 agent 已收版本全集**（`verify._agent_versions`，link→cluster 两跳）。**防假连续未削弱**：v2 推送真丢时全库也无 v2 行 → 仍判 `gap`（push_probe S-19 两向都断言）。**(b) 老格式行（`raw_json` 无 `cases` 键）→ 不可判**（闭合 ⑬(i)①）：保持 `pending` + `logger.warning`，**不判 `missing`**——按 `missing` 跨过会让 v1/v3 变成相邻纯净 pass（k=1 直接**假 fixed**）；**不做数据迁移**（v1 前无真实流量；若线上已有真实 claim 现场须先清或补迁移，见代码注释）。**(c) `bound_version_first_seen` 从 §8.7 载荷表删除**（契约面收缩，offline 侧**不再带**）：该字段必填但判定链**零读取点**（原以为由它承载 `no_progress` 守卫，实为 `agent_latest_version`），保留只会让 offline 白填——**⑪ 的相应表述已据实更正**；`agent_latest_version` **仍必填**。**(d) 判定触发补安全网**：判定触发已从「无限重试轮询」退化为「**推送到达一次性事件**」，补 `worker/rejudge_job.py`（**纯本地重放已落库结果行，零 outbound**，与已删的 recheck（拉 offline）**不是一回事**）：扫 `status=='claim'` ∧ 有 `pending` link 的簇逐条 `judge_link`，60s 一轮，worker job 数 **5 → 6**（`worker/__init__.py` docstring 已把「6」的口径写死，防后人按「recheck 已删」改回 5）。覆盖三类「事件到达时未就绪」：**claim 晚于推送**（建簇 open → 自动组装 → offline 拉走 → 推回时仍 open → 判定被守卫跳过 → link 永挂 pending）、**判定抛异常**（下述 (e)）、**其它状态过渡窗口**（needs_review resolve/reopen、TTL 回退、invalidate/requeue）。**(e) 判定异常降级 + savepoint**（`api/backflow.record_regression_result`）：判定块 `try/except` + `begin_nested()`，异常 → `logger.exception`（link/cluster/run_id）+ **跳过判定但结果行照常落库、响应仍 200**，交 rejudge 补判。**为什么降级而非回滚**：回滚把「数据先保住」与「状态立刻迁移」捆绑，牺牲的是**数据**——状态可补判，**丢掉的推送不可补**（online 无法向 offline 索要）；旧架构下 recheck 判失败同样不影响已落库的行，本刀不得低于它。savepoint 阻挡的是**半态落库**（真判定链边判边写，可能在链后半段才炸；裸调用时半笔 `link` 迁移会随 commit 落库且再也补判不到）——**注意别把理由写成「异常污染事务致 commit 失败」，MySQL 语句级报错不作废整个事务，实测不成立**（S-18 第一版即因此假绿）。**(f) rejudge 收敛判据 = 「本批零终态迁移」而非「空批」**（实测踩到）：本 job 候选谓词**不自清**（gap / 未达 K / 不可判的簇判定完仍是候选），照抄 assemble/claim_ttl 的「批循环直到空批」会让 `run_rejudge` **永不返回**、worker 该 loop 从此不 sleep 地空转打库——真库探针直接跑一轮即挂死，已由单测钉住。**(g) 读面派生标记 `result_gap_suspected`**（§9.2 详情响应，**零 DDL**，与 `result_overdue` 同源的现算字段）：语义 = 本簇现行 `pending` link 命中缺行中断现场（判据与判定内核**同一实现** `verify.link_gap_version`，不另抄一份）；前端 `BackflowClusterDetailView` 以警示行展示（**不限状态**——判成 `fixed` 后仍需可见，缺口可能是假修复的成因），**只标示「疑似少一笔」、不做对账面板**。**(h) `_claim_warning` except 分支补 `logger.warning`**（软提示查询失败原为静默吞，返回形状不变、仍 best-effort）。**(i) 探针与实测**：`push_probe` 新增 S-16~S-20（顺序一/顺序二/降级+补判/agent 级 gap 两向+读面标记/老格式行），容器真库 **push 22/22 · claim 15/15 · cluster 13/13 · assemble 10/10 · pull 13/13**、backend **438 passed**、ruff 干净；**反假绿逐条实测**（临时停用判据 → 仅对应场景转红、其余全绿 → 改回）：S-16 不跑 rejudge → S-16 红；去掉 savepoint → S-18 红；判据改回 link 级 → S-19 红；关掉 (d) 老格式判据 → S-20 红（单测同步红）。**⑬(i) 两项残留风险至此全部闭合**。

> 修订记录：2026-09-14 **v1.24（阶段 4 集成验收 · §14.5 用例表判据口径订正）**——① **依据**：阶段 4 独立复核（登记见 `docs/integration-report.md` §6 **F-11**）揭出本表 6 条表述与产品代码不符，逐条复核后定音 = **4 条真错（X-2/X-7/X-13 尾锚、X-4 旧口径、X-10 措辞、X-8 路由）+ 1 条口径模糊（X-12）**；② **三条尾锚订正**：X-2 `§4.5`→`§2.6 脱敏`、X-7 `§2.1`→`§5.2 周滚动`、X-13 `§7.1`→`§5.2 ILM/保留期`——**判据 = 本表 `§x.y` 一律为本 detail 内部锚**（X-5 `§7.3/§7.4`、X-4 `§8.2`、X-11 `§1.3/§7.4` 均指本文件自身），跨文件引用时文中显式写「solution §16」；③ **X-4 检索口径去旧**：`search_after` 属 v1.23 前旧实现，现为 `collapse(trace_key)` + `from/size` 偏移翻页，改为「末页后再翻**稳定返回空 items**」；④ **X-10 措辞订正**：`page_size` 恰 200 是**入参上界**（`le=MAX_LIST_RESULTS`），`offset ≥ 200` 返 **400 `ERR_TRACE_0002` 而非截断**（带 `truncated` 的是详情 ≤500，列表响应无此字段）；⑤ **X-8 路由口径订正**：`window` 为离散枚举 `{1h,24h,7d}`，**不存在 `24h+1s`**（越界值直接 400 `ERR_METRICS_0001`），真实路由是 **1h/24h 实时 vs 7d rollup**；⑥ **X-12 观测面口径明确化**（**不改判据**）：online 侧可验的是**词表固化保真与信封透传**，判分/抽查归 offline；⑦ **本次零代码改动、零判据实质变更**——X-8 前半「第 7 vs 8 天」一句**刻意未动**（其所描述对象在实现层不存在，已随 F-12 并入待定项「缺陷处置口径」）。

> 修订记录：2026-09-14 **v1.25（§5.1 补全局约定「不设物理外键」，阶段 4 待决项结清）**——① **问题的来路**：阶段 4 集成实测 F-1 发现「簇行已删而子行仍在」，一度被读作「两子表缺 `ON DELETE CASCADE`」；实查后升级为全库现象（`dev.obs` 13 表 0 外键），遂立待决项「全库无外键是**刻意为之**还是**历史遗留**」（登记见 `docs/integration-report.md` §6 F-1c）。② **定性 = 刻意为之**，三条证据 = (a) 本节 DDL 是**手写**的完整建表语句，逐个具名 `UNIQUE KEY`/`KEY`、逐列带语义注释，而**引用列一律只建索引不建约束**——「想过引用关系却主动不设」；(b) `app/models/user.py:4` 逐字声明「**全仓不建物理外键（与 DDL 一致），agent 归属等逻辑引用由应用层保证**」，`app/models/agent.py:4` 同旨；(c) 初始迁移头 `821efe0a89c0` 标注 `auto generated by Alembic`，即迁移层是模型层的忠实产物、**非独立决策**——决策点唯一，在本节。③ **本版改动（纯文档）**：§5.1 末尾补一条全局约定，把「引用完整性由应用层保证」写成显式契约，并将本节钉为**唯一事实源**（`app/models/*.py` 中同旨 docstring 降为对它的引述）。④ **一处订正**：此前 `integration-report.md` F-1c 记「ORM/迁移/DB **三层一致**」易被读成三方独立互证，据 (c) 应记为「**一个决定 + 两次无损耗复制**」，证据强度低于前记。⑤ **动机未留痕，本版不推断**：为何选应用层保证而非物理外键（性能 / 按父行批量清理 / 跨进程写入顺序等均为**候选、无证据**）在本仓任何文档与注释中均无留痕。⑥ **已知缺口（据实登记）**：该约定在**删除路径**上曾有实现缺口（2026-09-13 实测 dev 库 2 条悬空 link/conv）——F-1b 已向 `tests/integration/push_probe.py` 补入按 `push-trace-` 前缀的悬空行兜底扫描（**不静默删、命中即打印**）。⑦ 本版**零代码改动、零判据变更**。
> 修订记录：2026-09-14 **v1.26（`search_after` 陈旧口径全站订正，纯文档）**——① **对象**：「关键字检索深翻页」在 v1.23 契约期已由 `search_after` 改为 **`collapse(trace_key)` + `from/size` 偏移翻页**（深度上限定在 `offset ≥ 200` → 400 `ERR_TRACE_0002`），§8.2 已同步；但**同一口径散落于本文件 §14.1 S-5 行与另两份文档**，v1.24 只改了 §14.5 用例表的 X-4 一处 ⇒ **漏改 3 处**。② **本版改动（3 处，零代码）**：本文件 **§14.1 表 S-5 期望列**；`solution.md` **关键字查询段**（原文「search_after 深翻页，避免大 from 偏移拖垮租户档节点」**理由与实现相反**——实现恰是 `from/size` 偏移，上限定在 200，已改为「`collapse(trace_key)` + `from/size` 偏移翻页 + `offset ≥ 200` → 400，**深度设上限以避免无界 from 偏移**」）；`task.md` **T-1.4 验证目标 S-5**。③ **根因** = 改动范围被默认等同「§14.5 用例表」，而该口径是**跨文档散落拷贝**——**「先改记得的几处」必然漏，且漏的恰是未建模的那份**（同 F-11 族失效模式）。④ **终检** = 全仓 `*.md` 内 `search_after` 仅余**引述/订正**上下文，零未订正站点。⑤ 本版**零代码改动、零判据变更**。

> 修订记录：2026-09-14 **v1.27（T-3.11 ③「型 D」取证与订正：`watch` 值域漏两值，纯文档）**——① **来路**：`task.md` T-3.11 ③ 登记「型 D 4 条（代码有、文档无）」，其中一条原写「`status`/`watch` 实有 4 值而文档写 2」。② **取证结论（逐条核，不看登记摘要）**：(a) `status` **双侧一致**（`backflow.py:490` = `open/claim/fixed/inactive/needs_review`，§8.4 本表逐字相同）⇒ **该半边登记不实**，无需改；(b) **`watch` 确有漏**——代码 `backflow.py:491` = `Literal["assembled","draft","active","invalidated"]` **四值**，本表原写「`watch=assembled/draft`」**漏后两值** ⇒ **已补为四值全枚举**（语义 = 现行 pending link 的 `offline_status`）；(c) 「`result_overdue`/`result_gap_suspected` 未进文档」两字段：`result_gap_suspected` **两侧齐全**（§9.2/§8.4 有语义、前端 `types.ts:308` + `DetailView.vue:328` + 单测）；`result_overdue` **语义已文档化**（本文件 §8.4 第 1046 行 / §9.3 第 1255 行「回查结果未达（疑似 offline 停摆），人工核查」）——**但暴露另一层：前端零消费**（`frontend/src/` 内 `overdue` **零命中**：无类型声明、无渲染、无单测），即 §9.3 要求的呈现面**在线侧不存在** ⇒ 已另行登记为 `docs/integration-report.md` §6 **F-18**；(d) logout body 契约 / `/traces` total cardinality 两条**已于 2026-09-11（`105cd3e`）补毕**。③ 本版**零代码改动、零判据变更**。
> 修订记录：2026-09-14 **v1.28（T-3.12 批 2a-2：§8.5 health 端点落地 + 一处判据外推的订正）**——① **落地**：`GET /admin/agents/{id}/health` 按实机实现（`app/api/admin.py` + `app/store/es.py`），**两处按实机订正** = (a) **不返回 `spool_pending`**（§3.6 心跳 body 定义含该字段，但**双端都无写入方**：平台侧 `consumer/main.py:76-83` 构造的 doc 只有 `{node, agent, ts, dropped, source}`、SDK 侧 grep 零命中）；(b) **`dropped` 取最新一条心跳的快照、不做窗内求和**（它是 consumer **进程内累计计数**，`consumer/main.py:87` 逐字「按 (agent, reason) 累加，心跳任务定期快照」——窗内每条写的是**同一个累计值**，求和等于重复相加；实测二者差三个数量级）。
> 修订记录：2026-09-14 **v1.29（T-3.12 批 2b：§8.5 凭证读面落地 + rotate 整条下移 T-5.3）**——① **范围收窄（开工前取证推翻原计划）**：原计划的「凭证**两端点**」在取证后**只剩一端点**。两条硬事实：**(a) rotate 的三个动作全在 infra**——§13.2（`:1430`）逐字要求「向 infra 申请新 SASL 账号 → 落新密 → 旧账号 ACL 撤销 → SDK 断线重连」，online 侧**无任何执行面**（T-4.10 已于 `task.md:141` 把「执行动作」下移 T-5.3）；**(b) 新口令无来源**——`agent_credential` 表 dev 库 **0 行**、全仓除 models/alembic 外**零读写方**（2026-09-14 实测），若让 online 自造一个口令写进 `secret_cipher`，只会产出一条 **infra 侧永远对不上**的记录——**比空表更坏**（运维会以为已轮换，而 broker 侧毫无变化）。⇒ **端点与执行面一并归上线门**，入参/出参随 infra 账号发放通道定案（**用户 2026-09-14 拍板**）。② **「脱敏」的实现口径 = 不回传、且不读取**：文档 `:1137` 只写「secret 脱敏」未定义形式，而 `secret_cipher` 存的是**密文**（本来即随机串）⇒ 对密文做掩码零信息价值，设计本意是「不回明文」。v1 实现 = 该字段**不出现在响应体、且端点连 `secret_cipher` 这一列都不读**（不取即不可能误传）。**故本批不依赖 `Fernet`**——`fernet_keys` 配了、启动强校验非空，但全仓 `Fernet`/`MultiFernet` **零使用点**（2026-09-14 取证）；这不是「用它之前先补它」，而是这条路径根本用不上它。③ **另两条裁定**：**无凭证行 ⇒ 200 + `credential=null`**（「未发凭证」是合法态，非错误；404 会让前端分不清「agent 不存在」〔本面为 400〕与「还没发凭证」——两者处置动作完全不同）；**200 响应体不含 secret 字段**，故**不设 `has_secret` 之类恒真布尔位**（`secret_cipher` 是 `nullable=False`，「对象存在」已等价于「有 secret」）。④ **验证**：单测 **8 例**（含**响应体原始文本无 secret 字样**与**「一读 `secret_cipher` 就炸」的替身**这两条护栏）；真库探针 `admin_credential_probe.py` **10/10 PASS**（**有/无凭证双侧对照**——只测「没有」那一侧的话，「端点恒返 null」也会全绿；**用真库密文值反查响应体**而非查关键词）；**反假绿实测**：临时让端点多回 `secret_cipher` ⇒ C-4a/C-4b 立即转红、其余 8 项仍绿，回滚即复绿；探针前后 `agent_credential` 全表 **0 行 ⇒ 0 行**、无残留。⑤ **未验边界（如实标注）**：`rotate` 无用例可写（端点不存在）；`Fernet` 加解密**不经过**该路径、也无从验（全仓无实现）——探针里 `secret_cipher` 只是一个**普通字符串**，**不代表真加密语义已成立**；浏览器 e2e 只能验 `credential=null` 空态（dev 库 0 行），**不当全量验收**。⑥ 本版**零判据变更**——收窄的是**端点范围**（两端点→一端点），health/字典面判据未动。
> ② **判据外推的订正（本版最该记的一条）**：§8.5（`:1134`）/ §9.1（`:1238`）/ §3.6（`:434`）/ §9.2 页面表（`:1253`）四处原把「**无 `last_seen`**」写成「**未接入 SDK**」——但该判据承载的只是「**查询窗内无心跳**」（窗 = `keyword_search_days`），**区分不了「从未接入」与「曾接入但断联超窗」**，把后者说成前者**方向相反**（前者是待办、后者是故障）。已四处同步收窄，前端渲染文案由「未接入 SDK（查询窗内无心跳上报）」改为「**查询窗内无心跳上报**」。⚠️ **教训**：该外推**不是前端文案随手写的，而在 §9.1 的判据定义里**——前端只是**照文档实现** ⇒ 订正必须**文档与实现同步**；只改那个字符串会立刻留下新的「文档↔实现不符」。站点全集 = 16 处 / 8 文件（渲染面仅 1 处），残留的「未接入」字样**均为订正上下文里的引述**。
...
| offline 依赖边界 | D19/D20 信封 + pull 传输 + 回写/**结果接收**的 **online 侧**实现 | §7 + §8.7 |
...
> **本树是设计视图**（首 commit 前的模块划分约定），**未穷尽全部文件**；模块的新增/合并以 `backend/app/` 实态为准，**两者不一致时以代码为准**。本节不承担「随实现同步维护」的职责。
>
> **v1.23 订正（按实现实态修错位）**：`core/auth.py`→`core/security.py`、`core/deps.py`→`api/deps.py`、`consumer/mask.py`→`consumer/mask_recheck.py`、`consumer/es_dispatch.py`→`consumer/es.py`、`analyzer/trace_state.py`→`consumer/state.py`；`core/http.py` 删除（平台间 httpx 客户端从未落地，且 v1.23 后 online 零出站更无必要）、`analyzer/metrics.py` 与 `schemas/` 目录删除（从未落地；实时 agg 实际在 `store/` + `api/metrics.py`）。
...
│   │   │   ├── security.py       # JWT 签发/校验/refresh/吊销（admin 与 viewer 角色，§13.1）
│   │   │   ├── errors.py         # 统一错误码 + 异常→HTTP 映射（§8.9）
│   │   │   ├── log.py            # structlog/logger 统一（入参出参 debug，CLAUDE 规范）
│   │   │   └── audit.py          # config 变更 / 人工操作审计写盘（**未落地**，规划项，§13.5）
│   │   ├── models/               # SQLAlchemy ORM（§5.1，与 DDL 一一对应）
│   │   ├── consumer/
│   │   │   ├── main.py           # 每 agent 一消费协程组；批量拉取→处理→提交
│   │   │   ├── validate.py       # 事件契约强校验（§4.1）
│   │   │   ├── mask_recheck.py   # 脱敏复核（SDK 掩码核对，仅校验不还原，§13.4）
│   │   │   ├── es.py             # 事件→ES ingest（事件/日志分 index、_id 幂等）
│   │   │   └── state.py          # 判定态持久化（§4.3 累积态表）
│   │   ├── analyzer/
│   │   │   ├── classify.py       # L1/L2 分层判定 + L2 OR 门控（§6.1）
│   │   │   ├── cluster.py        # 归一化、去重、聚类窗口（§6.2）
│   │   │   └── context.py        # 字典事实读取薄层（agent/dict_config/interface → 判定原料，§5.1）
│   │   ├── converter/
│   │   │   ├── envelope.py       # D19 信封构造（schema_version / evidence / assert 区，§6.3）
│   │   │   ├── no_fallback_cfg.py# 组装时固化 fallback_utterance 词表快照（§6.3 step3/§7.1）
│   │   ├── backflow/             # 回流域包（§7/§8.4）：claim.py / verify.py（判定内核）/ batches.py / ack.py / requeue.py / recurrence.py
│   │   │   （**v1.23**：原 `converter/backflow.py` 从未落地；平台间接收面在 `api/backflow.py` + `api/pull.py`，**判定链零出站**）
│   │   ├── worker/
│   │   │   ├── judge_scan_job.py # 判定执行方：扫 judged=0 ∧ ttl_until≤now → classify → judged=1（§4.3/§6.1）；顺带清理过期判定行（§4.3/§10.1 purge）
│   │   │   ├── cluster_job.py    # 周期聚类扫描：judged=1 ∧ processed=0 进聚类（§6.2）；**reentry 版本门控内联本 job**（无独立 reentry_job，§7.5）
│   │   │   ├── assemble_job.py   # cluster→payload 组装（§6.3）
│   │   │   ├── claim_ttl_job.py  # claim 复核窗超窗回退 open（§7.6）
│   │   │   ├── rejudge_job.py    # 本地补判安全网：扫 claim 簇重放已落库结果行（**零 outbound**，§7.6/§8.7）
│   │   │   └── rollup_job.py     # 7d 小时级指标 rollup（§5.3）
│   │   │   （无 requeue_job：invalidated→assembled 复位由 admin/offline **请求驱动**，落 `app/backflow/requeue.py`，§7.4）
│   │   └── api/                  # 路由（§8 全集）；无 HTTP ingest——上报只走 Kafka（§3.3）
│   │       ├── auth.py  ├── trace.py  ├── metrics.py  └── backflow.py  # 回流 API 全集（§8.4）+ 平台间读面
│   │       ├── pull.py  ├── deps.py   ├── schemas.py  └── router.py    # pull-API 服务端 / 依赖注入 / pydantic 请求响应模型 / 路由装配（§8.7）
│   ├── tests/                    # 单测 + 冒烟（§14）
│   └── docker/                   # Dockerfile
├── sdk/
│   ├── pyproject.toml
...
> 时间驱动 job 周期（均【实现约定】，单飞见 §5.3/§7.6；**周期以 `backend/app/worker/main.py` 常量为唯一事实源**，本行为 v1.22 对齐后的实现实态）：judge_scan 60s；cluster 15s；assemble 60s；claim_ttl 60s；rejudge 60s（**本地补判，零 outbound**——判定已由结果推送端点同步驱动，见 §7.6/§8.7）；rollup 每小时对齐整点（+5s 抖动）；**无独立 reentry job**（同键再现归并由 cluster_job 内联，见 §7.5）；requeue 仅 admin/offline 触发，不设周期。多实例部署时 job 需分布式锁/CAS 抢单飞，防 rollup/聚类双写。
```
agent 业务 → obs_sdk 打点(内存有界队列) → Kafka obs.agent.<name>
   ...
   → offline 回归 run → POST /backflow/regression-results（落 verify_run_record + 同事务判定）→ verify_status passed/failed 单错级
...
P2(回流): analyzer 聚类/去重 → converter 信封+落库 → offline pull-API/回写/结果推送 → 前端回流页
...
| `agent_version` | str? | 否 | agent 部署版本，SDK init 从环境注入（R5 版本锚定源，§7.3）；可空但 gq/cs/sp 建议必填 |
...
| `agent_version` 空值 | 允许，但回流链路建议 trigger_version 非空（仅溯源；判定锚定 fix_version 见 §7.6） |
...
CREATE TABLE `error_cluster` (
  ...
  fix_version   VARCHAR(64) NULL,                    -- claim 时填写（R5 判定锚定版本；error 回流下与 offline run.version 共享版本字面量域、不强行 semver，Task #4-② B-5）
...
CREATE TABLE `error_case_link` (
  ...
  fix_version    VARCHAR(64) NULL,                   -- 关联 claim 填的修复版本（判定锚定）
...
-- ⑥ verify_run_record 复验判定历史（单错级 × 版本时间线）
...
>
> **全局约定（不设物理外键，v1.25 补记）**：本节全部表**不建 `FOREIGN KEY`、亦无 `ON DELETE CASCADE`**——凡引用父表的列一律**只建具名 `KEY` 索引**（如 `KEY idx_session_user (user_id)`、`KEY idx_conv_cluster (cluster_id)`、`KEY idx_link_verify (verify_status)`）。**引用完整性由应用层保证**：写入与删除路径须自行维持行序与孤儿防护，**清理类代码不得假定删除父行会连带删除子行**。
>
> **本节为唯一事实源**：`app/models/*.py` 模块 docstring 中「不建物理外键 / 与 DDL 一致」的表述均为对本节的指向与引述，不得作为独立口径引用。
>
> **该约定的实践薄弱点（据实登记）**：删除路径是它最易失守处——2026-09-13 集成实测曾在 dev 库发现「簇行已删而 link/conv 子行仍在」的悬空样本（详见 `docs/integration-report.md` §6 F-1，兜底扫描已补入 `tests/integration/push_probe.py`）。**新增按父行批量清理的代码时，须同时覆盖「父行已不存在」这一分支**。
...
- **重试自愈已天然过滤**（SDK 逻辑调用最终成功 `status=ok`，§2.4）；兜底吸收现场（request ok + llm_call error）不产 L1/L2 候选（L3 二期）——但其 trace 本身仍可被维度 1 查询、其 llm_call error 仍进指标（§2.4 前提）。**【实现约定·门控已落地（T-3.10）】**判据取 `root_status == "ok"`（`analyzer/classify._collect_candidates` 据此跳过子节点候选遍历），**刻意不用「`root_error_type` 为空」**——后者会把 timeout root（step5：timeout 事件不带 error_type）一并切掉，属未讨论的第二处行为变更；残 trace（`root_status` 恒 NULL）/ error / timeout 三类行为不变。
...
> 依据：solution.md §10.3/§10.5/§11/§12。**本平台间的三组端点 online 侧全部实现**；offline 侧只列「期望行为」（Task #4 落）。方向澄清（**v1.23 反转，契约注见 §8.7**）：pull 为 **offline 主动来 online 拉**；~~回查为 online 主动查 offline~~ → **回归结果由 offline 主动推给 online**（`POST /regression-results`）。**铁律：online 不持 offline 地址、不发起任何出站调用**——本平台与 offline 之间只有"被拉"与"被推"两种关系。
...
| | `versions` | `{trigger_version, fix_version}`（fix_version 组装时刻取值，**claim 前组装恒空、事后不重写**——判定锚定 `cluster.fix_version`，§6.3/§7.6，O-2 已裁定；可空） |
...
| 激活 | offline→online | rejected 后 offline 重扫自检通过 / 收单即激活 | 回写 `offline_status=active` + `case_id`（复用 payload_id 幂等；**收单即激活也须带 case_id**，否则单错级判定断链，§7.6） |
...
| **回归结果推送** | offline→online | offline error_regression run 终态 commit 后主动推（`POST /regression-results`，§8.7） | 落 `verify_run_record`（`raw_json` 原样留档）+ 逐条判单错 → 更新 `verify_status`（§7.6 单错级**判定内核原样保留**，仅数据源由「轮询拉取」改「被动接收」） |
| ~~回查 run_results~~ | ~~online→offline~~ | — | **v1.23 作废**：online 不再主动回查 offline（§8.7 契约注） |
...
2. ~~`list_runs` 需支持按 `agent + version + status` 过滤回归 run 且**按含 case_id 过滤**；`run_results` 返回 `case_id + case_type + pass_fail`。~~ **（v1.23 作废：online 不再调这两个读面——offline 改为「run 终态后主动推 `POST /backflow/regression-results`」，载荷自带 `cases[]`（`case_id + case_type + case_pass`），§8.7 载荷表。）**
3. 回归 run 不占互斥槽、不参与 max_active_runs；对回归 run 设独立保留档（pinned/豁免 cleanup），清理先归档 pass_fail 终态（归档前保留终态字面量 + na 计数，Task #4-② B-6）。
4. error-only run 的 `pass_fail` = executor 技术判定 ∧ verifier no_fallback 合成，由 verifier 阶段落终值（不经 SCORING、不产 agent_score）——online 判定对 error 生效依赖此语义。
...
**requeue 守卫（v1.9 R-24 状态域精确化）**：仅 `offline_status=invalidated` 且 **`cluster.status ∈ {open, claim, needs_review}`** 可复位——**fixed/inactive（已 closed）禁 requeue**（需重测走 superseded+reopen 重建新 link/新 payload_id，防对已收敛证据翻案）；claim / needs_review 态**允许**（供 claim 回归 run / needs-review-resolve 补充证据的拉取复位，§7.6）。**锚点保护**：requeue 仅复位 link（invalidated→assembled，复用 payload_id + 重填 payload_json + 刷新 `assembled_ts`），**不动 cluster 锚点**——fix_version / claim_k / TTL 保留，run 判定仍以现 claim 锚定（§7.6）；复位不改 `verify_status`（保持 pending → 仍占现行位，§5.1 注）。**verify 兜底**：仅 `verify_status=pending` 的 invalidated link 走此路（已推进 passed/failed/superseded 的不在此路）。防抖限流：同一 link 人工重推间隔 ≥【实现约定】5 分钟。**reason 门（2026-09-16 补，T-3.3 余项）**：`invalidate_reason=offline_cap_gap` 的 link **一律禁 requeue**——该 reason 的恢复面在**离线侧**（补登记 agent/interface），online 补不了登记，故人工重推有**三个真实损失**：①**假动作**（离线对 `offline_cap_gap` 只放 `ack_status ∈ {none,pending}` 重处理，而这类行已 acked ⇒ 重推后必被跳过，admin 看到 200 而实际零处理）；②**载荷分叉**（requeue 会用现 cluster+现词表重渲染 `payload_json` 并刷新 `assembled_ts`，而离线 inbox 里仍是旧 `envelope_json` ⇒ 日后离线探测态自愈建出的 case 来自旧载荷）；③**污染 R-7 可愈性计数**（`requeue_count` +1，前端 ≥`SUSPECT_REQUEUE_THRESHOLD` 转强确认，把「从未被受理」记成「重推多次仍不行」）。正确路径 = 离线「等待接入」探测态每小时自愈（offline §5.8）。**批量入口 `requeue-batch` 的候选本就限 `online_content_gap`（`requeue.py` where），单点入口此前漏此门，本次补齐**；前端 `linkCanRequeue` 同步不渲染按钮（后端守卫为准，前端隐藏只是第一道）。

**批量 requeue（v1.6 R-7）**：`POST /backflow/links/requeue-batch`（§8.4）——admin 筛选 `offline_status=invalidated + invalidate_reason='online_content_gap'`（可加 agent/缺项过滤）批量复位，逐行复用本守卫 + 防抖 ≤N/min【实现约定 20】。**可愈性标注**：按 invalidate detail 缺项区分「补齐+重推可愈」（字段缺/词表空，补齐 `no_fallback_config` 即愈）vs「requeue 不愈」（版本不识别：schema_version 非 1.0 / case_type 新白名单值，需 offline 升级/联调同步才愈，phase1 §6.3 同源语义）——批量 UI 对不愈行**默认禁勾（强确认才放行）**，防误导 admin 反复重推。**（v1.23 反查订正：本段所述可愈性标注 + 不愈行禁勾/强确认门控未交付** —— 实现仅落批量端点，`requeue.py` docstring 把可愈性改写为「可愈性 = 补齐现场即愈」的机制描述；**register R-7 原拍板含标注并否决了备选 B「仅批量端点不加标注」**）；**（2026-09-11 改判：换判据、单 link 路径交付）** 原判据经查不成立（**改判理由是「原判据要区分的对象在跨端链路上不可达」，非「不可实现」**；⚠️ 2026-09-11 重估终版，此前「成本/收益不划算」的写法已作废：① 初版写「「版本不识别」**不落 invalidated 行**」方向对但因由错（引了推送接收面的码）、中版写「**确实落 invalidated 行**」只读设计未问可达性、亦误——**终版**：`version_drift` 是**死分支**，其触发条件被 **online pull 层上游**挡死（版本不匹配 → 400 `ERR_PULL_0002` 拒单，`pull.py:88`，实测 `test_backflow.py:237`；`case_type` 非白名单 → 返空集，`pull.py:94`），offline 声明与校验均为常量 `"1.0"`（offline `solution_detail.md:292`/`:309`）⇒ 收不到会触发它的信封、**不落行**；② 扩词表腿**作废**——无对象可区分），改以**行为数据**为判据——该 link 历史重推次数（`requeue_count`，口径**不含本次**）≥2 时前端详情页转**双次强确认**（`BackflowClusterDetailView.vue:SUSPECT_REQUEUE_THRESHOLD`）；`requeue_link` 返回值与该判据同源（`requeue.py:163`，查在 `session.add` **之前**）。**注意：以上交付落在单 link 路径；本段所述的批量路径标注已裁定不做、本条结清（2026-09-11）**（**两条实质理由 = 实害不可达 / 残留后端无消费者**；「门控无 UI 可落」系**当期事实、非不可实现性论证**，完整取证见 register R-7 改判记录末段；日后真接批量入口时重开）返回逐行结果（requeued / skipped+原因）并记 conversion_record（actor、筛选条件、逐行结果）。
...
### 7.5 reentry：线上观察哨位（**内联于 cluster_job，无独立 job**——原 `worker/reentry_job.py` 从未存在）

- 观察键 = error 去重键（agent+interface+error_type+input_hash，与 §6.2 同源）。
- 触发：cluster 已 claim/fixed 后，同键错误**线上再现**（judged 为 L1/L2 的新事件）。**版本门控（v1.1，序比较语义 Task #4-② B-5 细化见下）**：仅当新现事件版本与 `cluster.fix_version` 满足**等值+日期前缀门控**（修复确已上线、回归有据）才开 reentry；修复上线中/未上线再现 → 只按 §6.2 窗口 `count+1`，不开新 cluster（防修复落地前反复抖 cluster、空转回归）。**reentry 产物 cluster（generation>1）后续 claim 受 R7 约束（§7.6 v0.6.1 判定）**：claim 版本若 = 该 (agent,version) 已有 completed 回归 run 的版本 → 旧 run 已 pass 不 auto-fixed、转 needs_review（reason=`reentry_same_version`）。
...
| `inactive` | ignore | → open（反悔/复发重新开） | link 已 superseded 才停判定（§7.5） |
...
- `verify_status`：pending（含"待 &lt;version&gt; 回归 run"）/ passed / failed / invalidated / superseded。**终态只读**：已定 passed/failed/superseded 的 link 不被迟到 run 改写（只追加 `verify_run_record` 时间线；§7.6 单错级判定/终态只读）。
- **单错级判定规则（solution §10.5；auto-fixed 判定语义 v1.5 = claim 固化 `claim_k` + claimed-case 级纯净判据，见下 v1.5 blockquote）**：**判定数据源（v1.23 第 3 刀）= 本 link 已落的 `verify_run_record` 行集**（载荷 `raw_json` **原样留档**，逐 case 行/发版水位/前序指针全从这里重派生）+ cluster 字段，**不再是 offline 只读面**；判定锚定 = **claim 填的 fix_version** 起该 (agent,case) 的**已收**回归 run 序列，逐版取行内 `case_pass` 判定；**不取乱序 run**（防多版本先后触发拿错 run）；该 case 纯净可判版 pass → 计入 K（计满 `claim_k` → verify passed → cluster fixed，闭环）；run 内他错仍红不阻塞本错 closed（na 影响域归类见下 v1.5）。**触发点 = 结果推送端点内**（`api/backflow.py:record_regression_result`，`flush` 后 `commit` 前**同事务**跑 `verify.judge_link`）+ **`rejudge_job` 本地补判兜底**（60s，**零 outbound**）——**判定链不含任何 offline 出站调用**（§8.7）。

> **回归 run 判定语义（Task #4-② 契约修订 B-1(a)/B-1(b)，2026-09-03）**：
> - **判定源限定**：recheck 只消费 offline error_regression run 终态 `status ∈ {completed, partial_failed, timeout, cancelled}`，**绝不含 running/pending**（半态先落库的部分 pass 行不可作终值，防 premature fixed）。**【v1.23 取代】拉取型 recheck 已整删；「结果源」= offline 推送的 `POST /backflow/regression-results` 载荷（**仅终态 run 才推**，故「绝不含 running/pending」自动成立），判定在推送端点内同事务执行（§8.7）。**
> - **同 (agent,version) 多 run 选取序**：锚定 claim fix_version 后，该 (agent,version) 若存在多条 run（崩溃重跑/坏终态重建产物），优先最近 `completed`；无 completed 取最近可判终态（`status ∉ {pending, running}`）。claim 未 closed 期间每次判定取「当前最近可判 run」判定、有推进即更新（未定前用新 run；不覆写已 closed）。**【v1.23 取代】推送源下同一 (agent,version) 的多个 run 由 offline **各推一笔**、逐笔落库（`uk_verify_run(link_id, run_id)` 幂等）；「多 run 选取序」不再需要——`_pick_run` 已删（v1.23 修订记录 ⑬(b)）。**
> - **per-case 值域映射**（`run_results.pass_fail`；auto-fixed 触发在**纯净 run 前提 + K 稳定序列**下执行——v0.6.1 精化见下，本条保留 pass/fail/na/缺行四值语义）：`pass` → 该 case 该版判定**通过**（纯净可判版内计入 K；置 verify passed 需 K 满，非纯净 run 内 pass 不触发 verify passed、转 needs_review(`unclean_run`)）；`fail` → error 仍在（verify failed → 即时 reopen，cluster 回退 open、K 清零）；`na` 行（携带 `error_type`）→ 该 case infra 无法判定 → claimed cluster 置 needs_review（reason=`na`，同 run 同 error_type 聚合为批，见下）；case 在该 run **无结果行（缺行）≠ na** → 无终值，继续轮询至 claim TTL 超窗回退 open（K 不计数不终止），不触发 needs_review。**【v1.23 取代】`run_results.pass_fail` → 载荷 `cases[].case_pass`；「轮询」→ 「保持 `pending`，等后续推送或 `rejudge` 补判」；并新增 `gap`（`prev_terminal_version` 缺行 → **中断不累计 K**，§8.7）。**
...
> - **K 稳定序列（B-11/R6）**：该 case 自 claim fix_version 起、**连续 K 个「纯净可判」版本 run 均无 fail** 才置 verify passed（K=2 默认、可配【实现约定】dict_config；fix_version 自身为第 1 版确认；K=1 兼容 v1.3 单版行为）。fail 任一版本 → verify failed、即时 reopen、K 清零；逐版判定追加 `verify_run_record` 时间线（载体为既有表，**不加纯净性列、不加 offline 字段**——纯净性由 recheck 从关联 run 的 `na_case==0` 派生，R6 载体承诺保持）。**【v1.23 取代】纯净性由判定内核从本 link **已落结果行**（载荷 `cases[]`）+ run 内环境级 na 现场重派生——**不读任何 offline 读面**；判定触发改推送端点（§8.7）。**
...
> - **K-TTL 收敛终点（v0.6.1 定义③）**：verify passed 仅 K 满时置；收敛 = **两时钟先到者**——K 满 → cluster fixed（TTL 停钟）/ claim TTL(14d) 超窗 → 回退 open（K 观察中止、下轮 claim 重计）；TTL 自认领起算，K 观察/判定推进不重置、不延长 TTL；缺行/不纯净中断观察继续轮询至 K 满或 TTL 到期，无无限期挂起。**【v1.23 取代】不再有「轮询轮次」——「继续观察」= 等后续结果推送到达即补判（`rejudge_job` 兜底，§8.7）；**K-TTL 两时钟收敛终点不变**。**
>
...
> - **① 缺行成因诊断（R-4）**：claimed case 持续缺行（自 fix_version 起无该版本 run 终值、按 ⑤ 轮询）时，claim 详情给出成因诊断入口——可能成因 = offline cap 截断挤出（newest-active-first 挡在 run 外）或未达终态。**v1.8 R-16（自动核对升级）**：recheck 判缺行时自动 GET runs?agent=&version= 拉该版 run、判 `case_id ∈ excluded_case_ids`（R-4 溢出 case 列表字段，落 list/detail 位置留 Phase B 核对）——∈ → 自动判**「窗口外欠测」**（非修复失败）→ 走错误量治理 / 人工 fixed；∉ → 维持「未达终态/其他」并引导人工去 offline 平台 `excluded-cases` 只读面兜底核对（§9.3）。判定维持：不自动 reopen、不进 needs_review、不计 K 不清零（维持 ⑤ 无终值轮询至 TTL）。不跨端保窗口（phase2 v0.6 L111 拍板维持）。**【v1.23 作废】本条 R-4 整条取消——`excluded_case_ids` 读面、`recheck` 自动 GET runs 均不存在（推送为**全量对账**，run 跑过的每个 case 都在 `cases[]` 里，无「缺行 → 轮询」场景，§8.7 作废清单）。缺行语义改由载荷 `prev_terminal_version` 单值指针承载（缺行 → `gap` 中断、保持 `pending`）。**【v1.23 反查订正】上述作废理由不完整**——「全量对账」解决的是「不知道 run 跑了哪些 case」，不解决 R-4 要的「知道缺行、但不知成因」：cap 挤出与「该版本本就不含此 case」在 `cases[]` 下**依然不可区分**（两者都表现为 case 不在 `cases[]` 里）。**净损失 = 归因能力永久丧失**（登记见 §12.2 O-8）。另：**末句「缺行 → `gap` 中断」把两个粒度混为一谈**——**case 级缺行 → `missing`**（`verify.py:85`「run 在但 case 缺行」→ 不自动 reopen/needs_review/计 K，**cap 截断走此支**）；**版本级指针缺环 → `gap`**（`prev_terminal_version` 本地无记录，`verify.py:299`）。内核两 outcome 本就分开，此处仅文档措辞含混。**
...
> - **④ R-16 缺行成因诊断自动核对（接 v1.6 ① R-4）**：`excluded_case_ids`（offline run 只读面溢出列表）核对从「UI 引导人工」升级为**自动**——recheck 判 claimed case 在 bound_version run 缺行时自动 GET runs?agent=&version=、判 case_id ∈ excluded_case_ids：∈ 自动标「窗口外欠测」（cap 挤出、非修复失败）；∉ 维持未达终态并人工兜底。字段落 runs list/detail 位置 = Phase B 实现核对项。**【v1.23 作废】R-16 整条取消：`excluded_case_ids` 只读面 + 自动 GET runs 均不存在（§8.7 作废清单）。****【v1.23 反查订正】连带结果 = 缺行成因诊断（v1.6 ① / 本条）整体退化为静默**——cap 挤出的 claimed case 走 `missing` 支（无终值 → 保持 `pending` → claim TTL 到期回退 `open`，可再 claim、可反复挤出），**online 侧无任何可见面**；**不会**误触「疑似 offline 停摆」（该探测要求零 `verify_run_record`，而每个推送 run 都落一行，`backflow.py:940`）、**不会**误触 `result_gap_suspected`（版本级判据，§8.4）。归因归属改判 = **offline 侧本地**（`case_truncated` 诚实标记，成因 = offline error case 容量治理），online **不恢复**该能力（2026-09-11 用户拍板）。登记见 §12.2 O-8。
> - **⑤ R-17 K 序列版本锚 = versions 读面（接 v1.4/v1.5 K 判据）**：候选版本序列从「出现过 error run 的版本序」改 **R-5 versions 读面（agent 已见 manual/held_out 版本全集 = 发版拓扑权威）取 ≥ fix_version 的全版本序**（window_days 默认覆盖 claim TTL 14d）逐版判：versions 有该版且 error run 有终值 → 判 K；versions 有该版但无 error run → **缺行中断**（下一可见 error run 不续缺环——中间版丢 run 时不得 v1(pass)+v3(pass) 假连续 → K 满 false-fixed）；该版 error run 迟到补建（R-3 差集）终态到达 → 补判推进。error run 由 manual/held_out 终态触发 → error run version ⊆ versions 全集，versions 恰补「有发版无 error run」维度。**v1.23 改（第 3 刀）：versions 读面整组作废**（§8.7 作废清单）——「有发版无 error run」这一维度**推送源无法枚举**（offline 只带单值水位 `agent_latest_version` + 单值前序指针 `prev_terminal_version`），故中断判据改由后者对照**该 agent 的已收版本全集**承载，**语义目标（杜绝中间版丢 run 的假连续 false-fixed）不变**，只是证据来源从「版本全集差集」降为「单值指针缺行」；「迟到补建 → 补判推进」在推送源下 = 迟到的结果推送到达即补判（判定在推送端点内**逐笔**执行，不再有轮询轮次）。
>
...
>
- claim TTL 到期 / 回归 failed / **结果未达超阈** → 分别回退 open / 保持 open / 聚类详情提示「**回查结果未达（疑似 offline 停摆），人工核查**」（§16 容错）。**v1.23**：online 不再主动回查，「连续失败退避重试超上限」这一触发源已消失，改为**被动探测**——claim 态超 `claim_ttl` 仍无任何 `verify_run_record`、或 assembled 态超「已待 N 天」阈值，两者共用同一阈值与文案（§7.2/§8.7 契约注）。
...
| `GET /backflow/clusters` | viewer | 列表/筛选：`agent interface layer status`（status 含 open/claim/fixed/inactive/needs_review + `watch=assembled/draft/active/invalidated` 待拉取/确认筛选，**四值全枚举**、语义 = 现行 pending link 的 `offline_status`；**⚠️ 2026-09-14 订正**：原写「`watch=assembled/draft`」**漏后两值**，与 `backflow.py:491` 的 `Literal` 不符）；返回代表 trace（**`first_trace_id`**，ErrorCluster 既有列，前端跳 trace 详情）、input_hash、去重/生成计数、关联 link 摘要 |
...
| `POST /backflow/links/requeue-batch` | admin | **批量 requeue（v1.6 R-7）**：body `{filter:{agent?, invalidate_reason:'online_content_gap'}}` → 逐行 §7.4 守卫 + 防抖 → 返回逐行结果 + skipped 原因（~~含可愈性标注，不愈行需 force 确认~~ **v1.23 反查订正：未交付** —— 无 `force` 参、无逐行标注；且「`force`」本身是文档笔误：设计定位为**前端 UI 强确认门控**（register R-7），非后端 body 参数；前端批量重推 UI 整块未做）**；**（**2026-09-11 改判**）可愈性标注已以行为数据判据在**单 link 端点**交付（`POST /backflow/links/{id}/requeue` 返回 `requeue_count`）；本批量端点**已裁定不加标注、本条结清（2026-09-11）**（理由见 §7.4 同段；**取证订正**：成功行本就有逐行 `link_id` 的 conv，`link_id=None` 聚合行只汇总 skipped） |
...
| `PUT /interfaces/{id}` | admin 补标：`{llm?, llm_source?, body_search?}`（归一化核对 = 修改 interface 串需记 conversion 审计【实现约定】；⚠️ **2026-09-14 订正：本行入参里并没有 `interface` 字段，而它是唯一键列 `uk_interface(agent_id, interface)` ⇒ v1 不支持改串**（改它等于换实体）。批 2a-1 实测：多余的 `interface` 字段被入参模型忽略、串不变）；~~疑似漏标告警处理（§8.5）~~ **（2026-09-14：§8.5.1 自动补标 v1 不做，人工补标本端点即为全部；见 §8.5.1 裁定块）** |
| `GET /agents/{id}/credential` | Kafka 凭证查看（admin）。**2026-09-14 落地口径**：「secret 脱敏」实现为**不回传该字段、且连 `secret_cipher` 列都不读**（密文做掩码零信息价值，且 `Fernet` 全仓零实现）；**无凭证行 ⇒ 200 + `credential=null`**（合法态，非 404）、agent 不存在 ⇒ 400。⚠️ **「轮换入口」不在本端点/本页**——见下行。 |
| `POST /agents/{id}/credential/rotate` | 凭证轮换（版本化、吊销即时生效=撤 ACL+断连接，§13.2）。⚠️ **2026-09-14 下移 T-5.3**：三个动作（新 SASL 账号 / 撤旧 ACL / 断连接）**全在 infra**，online 侧无执行面；且新口令**无来源**——online 自造一个只会写出 infra 侧永远对不上的记录。**端点与执行面一并归上线门**，届时入参/出参随 infra 账号发放通道定案。 |
| `GET /agents/{id}/health` | agent 上报健康小卡：最近 1min/5min 上报事件量、dropped、spool_pending、last_seen_ts（读自监控心跳 `obs.selfmonitor`，§3.6/§13.2）；agent 未接入=无 last_seen，前端据此出「查询窗内无心跳上报」文案（§9.1）。⚠️ **2026-09-14 措辞收窄**：原写「未接入 SDK」——该判据承载的是「窗内无心跳」，**区分不了**「从未接入」与「曾接入但断联超窗」（窗 = `keyword_search_days`），把后者说成前者方向相反（前者是待办、后者是故障）。⚠️ **2026-09-14 批 2a-2 落地，两处按实机订正**：① **不返回 `spool_pending`**——平台侧心跳 doc 实际只有 `{node, agent, ts, dropped, source}`，SDK 侧 grep 零命中（双端无写入方，同 §3.6 订正）；② **`dropped` 取最新一条心跳的快照、不做窗内求和**——它是 consumer **进程内累计计数**（`consumer/main.py:87`），窗内每条写的是同一个累计值，求和等于重复相加（实测差三个数量级）。实现 = `app/api/admin.py`、`app/store/es.py`；真库真 ES 探针 = `tests/integration/admin_health_probe.py`（12-12）。 |
...
> **（2026-09-14 裁定：v1 不做，改由人工补标）** 本节整节**不实现**，上面原文划线保留仅为留档。三条依据：
> 1. **它替代的是一次点击**——人工补标端点 `PUT /interfaces/{id}`（§8.5 `:1137`）**已存在且已验收**（T-3.12 批 2a-1，真库探针 G-5 实测 `llm 0→1` + `llm_source=manual` + `llm_suspect 1→0` 落库）。本节要做的是「让 job 从 ES 流量**猜**哪些接口漏标」，相对人工入口无新增能力。
> 2. **代价不对称**——需**新增第 7 个周期 job**（`worker/__init__.py` 的 job 集 docstring 明立「job 数 = 6」护栏），外加两处契约歧义裁定（`llm_suspect` 何时清 0、llm_call 是否按 `status` 过滤，原文均未定义）与审计量治理（逐轮逐接口置位 ⇒ 单接口每晚约 288 条审计，须按 agent 归并）。
> 3. **它建在空载面上**——`interface` 表在 dev 库 **0 行**且**全仓无 INSERT**（2026-09-14 实测；`app/` 内 `Interface` 仅被 SELECT），而该列 `llm_suspect` **全仓唯一写点是把 1 置回 0**（`api/admin.py:573-576`），**没有任何置 1 的写点**。即：本节连同其读面建在一张从无写入方的表上。
>
> **连带**：`llm_call_observe_window_min` / `llm_call_observe_threshold` 两键**v1 无消费方**（见 §10.1 配置表注）；前端 `/admin/agents` 的「疑似漏标」列与「确认后自动解除」提示**已移除**（该提示在承诺一个永不发生的行为）；`llm_suspect` **列保留不动**（不写迁移），将来若复活本节，加回写点即可。
...
**offline → online（结果推送侧，v1.23 新增；online 实现，offline 出站契约）**：

| Method & Path | 说明 |
|---|---|
| `POST /backflow/regression-results` | offline 在 `error_regression` run 终态 commit 后**主动推送**结果。鉴权 ~~`scope=backflow:report`（§8.8）~~ **（v1.23 作废：据实收敛为 evaluator 静态预共享 secret、无 scope 分置、非 JWT —— 原 JWT/scope 机制 online 从未实现，§8.8）**；幂等键 = `uk_verify_run(link_id, run_id)`，重复推送 → 200 + `duplicated:true`；响应 `{accepted, duplicated, run_record_id, links_advanced[], cases_dropped}`。online **收到即同步**落 `verify_run_record` 并重算该 link 判定（§7.3/§7.6，判定内核原样） |

载荷字段规则（online 校验面）：

| 字段 | 必填 | 规则 |
|---|---|---|
| `schema_version` | 是 | `"1.0"`；非 `1.0` → `ERR_PULL_0004` 拒单并计数 |
| `agent` / `agent_version` | 是 | `agent` 同 payload 信封 `source.agent`；`agent_version` = 本 run 绑定版本字面量（原 `verify_run_record.bound_version`） |
| `run_id` | 是 | offline run id，**幂等键的一半** |
...
- **R-5「agent 已见版本」→ 只读面整体作废**（**⑭(b) 更正**：原写「降为载荷单字段 `bound_version_first_seen`」，该字段在第 4 批已删——它**必填却零读取点**，保留只会让 offline 白填；守卫实由 `agent_latest_version` 承载）：online 只需守卫所需的最小信息，不再拿版本列表；**连带损失**——claim 表单「fix_version ∉ 已见版本」软校验的数据源消失，降级为「online 已收结果的版本集」软提示（§8.4/§9.3 文案同步，见 v1.23 修订记录）。
- **R-17 的「K 判据序列源」→ 改从已收 `verify_run_record` 自建 + 缺行中断改由 `prev_terminal_version` 承载**（v1.23 第 3 刀）：不再依赖 offline 版本列表（原 versions 读面「**有发版无 error run**」这一维度的中断判据随之消失——推送源**无法枚举中间版本**，只能给出单值水位），online 侧按 `bound_version` 升序自建序列；「中间版 v2 结果推送丢失」这一**最危险的假连续**（online 只见 v1(pass)+v3(pass) → K 跨缺版累计满 → 误判 `fixed` 假修复）改由载荷 `prev_terminal_version` 单值指针对照**本地行集**判定（非空且本地无该行 → `gap` 中断不累计 K；`null` 不中断），详见 §8.7 载荷表该行。

> **失败与保活语义（fire-and-forget 的必要配套，非可选）**：offline 终态 commit 后**异步**推送，超时 5s、重试 3 次（1s/2s/4s），全败记 error 后放弃、**不阻塞 run 收尾**；online 为接收方**不重试**。**online 无法感知 offline 存活** → 两类现场统一标记：① **claim 侧改用「事件派生」，不用在线时窗**（v1.23 第 2 刀收口）——**存在 `claim_ttl_expire` 转换记录**（`claim_ttl_job` 超窗回退时必写，且回退**保留 `fix_version`**）**∧ `fix_version` 非空 ∧ 现行 pending link 零 `verify_run_record`**。**为什么不用 `claimed_at + claim_ttl` 现算**：`claim_ttl_job` 每 60s 把所有超窗 claim 回退 `open` 并清 `claimed_at`/`claim_due_ts`（`worker/claim_ttl_job.py`），该现场**最长只存在 60s**，在线现算判据**恒为假 = 被动探测落空**；转换记录 + 残留 `fix_version` 才是持久证据。**抑制条件**：cluster 处于**未到期** claim 期内（`status='claim' ∧ now ≤ claim_due_ts`）**不标记**——被回退的 cluster 可再次 claim 而旧记录仍在，不抑制会在人工正常处置期间误报。② assembled 态 link 超「已待 N 天」阈值（§7.2 已有展示口径）。两者共用同一阈值 **N = `dict_config.claim_ttl_days`（默认 14；同源不新编魔法数，代价 = assembled 分支需等满 N 天）**与文案。**claim 分支的锚点必须是仍停在 `pending` 的现行 link**——**不能**用「取最新一条 link」兜底：`fixed_review(approve=True)` 会把现行 link 打 `passed`、`ignore` 打 `superseded`（`claim.py:246` / `api/backflow.py:602`），此后四个条件全成立，会在**已 fixed / 已 inactive** 的簇上误报；不用 `cluster.status` 白名单是因为白名单在将来新增非终态时**静默漏报**（正是本标记要修的失效模式），link 谓词若被新状态漏动只会**误报**——响亮地错优于安静地错。**且该回退记录必须晚于锚点的组装时刻**（`latest_expire.ts > anchor.assembled_ts`；`assembled_ts` 为空亦抑制）——claim 分支只认「**本轮**等待」的证据，而 `assembled_ts` 正是本轮等待的起点（requeue 刷新即归零，§7.2「已待」本义）。**为什么必须加这条**：`reopen`（`claim.py:168-191` 只改 status，**不清 `fix_version`、不动 conv**）后 `assemble_job` 会**重建一条全新 pending link**——此时陈旧回退记录 + 新 link 使前四条全成立 → 对一条**刚组装 0 天**的 link 误报「offline 停摆」，`since_ts` 却指向多日前的旧回退；`invalidate → requeue`（刷新 `assembled_ts`、verify 仍 pending）属同一类。**为什么不用「扫 `ts > latest_expire.ts` 的 assemble/claim/reopen/requeue 记录」**：一次字段比较即可达成，且 `assembled_ts` 本就是本轮等待的**权威起点**，扫 conv 是绕过权威源另起判据。**`assembled_ts` 为空亦抑制**：link 未组装 = offline 尚无 payload 可跑，此时报「offline 停摆」是误诊（该现场归 assembled 分支）。**前提约束**：① 依赖 `claim_ttl_job` 成功跑过并写下 `claim_ttl_expire` 记录，故本标记**以 online worker 在跑为前提**（worker 停轮时 online 侧全部时间驱动 job 同步停摆，该标记失效不构成新增风险）「**回查结果未达（疑似 offline 停摆），人工核查**」，读数 = MySQL 现算（与 §7.5 blocked 复发读数同源策略）。**不做此标记 = 把原来可控的"停轮"换成不可观测的死锁**（§16 风险行）。
>
> **v1.5 判定依赖（R-2/B1 修订，语义不变仅换数据源）**：`na_case` 计数与 per-case `error_type` 均从推送载荷的 `cases[]` 现算（claimed case 行纯净判据 + na error_type 影响域 环境/case 归类，`na_case==0` 不作纯净判据）。
>
> **判定并入推送端点 + 缺行中断（v1.23 第 3 刀）**：判定**在推送端点内执行**——`record_regression_result` 的 `flush` 之后、`commit` 之前**同事务**跑 `judge_link`（落库即判、判定与结果原子；轮询链已整删，这是**唯一**触发点）。闸门四条：① 只对 **`cluster.status == 'claim'`** 的 link 判定（cluster 非 claim 一律跳过；`claim` 是唯一可判定态）；② **link 自身 `verify_status != 'pending'` 亦跳过**（终态只读，迟到 run 不覆写——`claim.py` 的 CAS 与 `judge_link` 局部守卫双保险）；③ orphan run 行（`trigger_signal_id` 关联不到）与幂等重放（`duplicated=true`）**不触发判定**；④ `ensure_unclean_batch` 随判定链带走，且**只对本次触发记录**入批——判定每次都是对全链**重放**，历史行再入批会重开人工已 resolve 的同 key 批（清 resolve 审计），那是重放副作用不是新污染。**`gap` 是「我已漏收一笔」的显式结论态**（非失败态）：不迁移、不落批、保持 `claim` 等 offline 补推；fire-and-forget 无补推机制时该现场由上方停摆标记「回查结果未达（疑似 offline 停摆），人工核查」兜底收口。
...
| 回流-聚类详情 `/backflow/clusters/:id`（v1.20 已实现，独立路由） | links 表（case_type/offline_status/verify_status/source）、版本×pass/fail 时间线、conversion_record 中文时间线；⚠️ **批 50（#33）订正**：本行原写「＋**admin** invalidate/requeue」「人工操作区（ignore/claim/needs_review-resolve/fixed-review/admin 动作，按 §9.4 门控置灰）」「claim 复核窗倒计时（1s tick + 45s 可见轮询，超时回退 open 提示）」——**这三项对应的人工作业面已随批 35-A/B 整体撤除**（前端组件与后端写端点均已删净，见 `docs/ux-review-newbie.md` §四十四~§四十七）。**人工处置面现不存在，本行不再作为待建清单使用**；**input_truncated 当前判定态警示 + 被未决 unclean_run 批挂起徽标 + open_batches「处置整批」入口（v1.8 R-13/R-14、v1.20 R-14 批读面：读 `link_refs` 关联、不新增存储列）**；**reentry_observe 复发 caption（v1.20，§9.3）** | §8.4 cluster/{id} + link 操作 |
...
| 结果未达超阈（**v1.23：原「回查连续失败」**，#7/#9 合并） | 提示「**回查结果未达（疑似 offline 停摆），人工核查**」 | §7.6/§8.7 |
...
| invalidated 列表批量重推 | 「批量重推」动作 + ~~可愈性标注（空词表/字段缺=可愈可勾选；版本不识别=禁勾、强确认才放行）~~ **（v1.23 反查订正：未交付）**（**2026-09-11 改判**：已改以行为数据判据交付于**详情页单 link 重推**——`requeue_count≥2` 转双次强确认；**本行所述「invalidated 列表批量重推」整块 UI 仍未做，且已裁定不做（2026-09-11）**——「门控无 UI 可落」正源于此（系当期事实），本条结清）+ 结果逐行回显 | §7.4/§8.4 v1.6（R-7） |
...
- link requeue(admin)：仅 `offline_status=invalidated` 且 `verify_status=pending` 且 `cluster.status ∈ {open, claim, needs_review}`（§7.4 守卫 v1.9 R-24 + 防抖；fixed/inactive 禁 → superseded+reopen）；**且 `invalidate_reason ≠ offline_cap_gap`**（2026-09-16 补，§7.4 reason 门——该 reason 恢复面在离线侧）。
...
| `claim_ttl_days` | 全局 | 14 | claim 复核窗（§7.6；**收敛 = 两时钟先到者**：K 满 → fixed 停钟 / 本 TTL 先到 → 回退 open，K 观察/判定推进不延长本 TTL，v1.4） |
...
| `llm_call_observe_window_min` / `llm_call_observe_threshold` | 全局 | 24h / 10 次 | ~~疑似漏标自动补标窗口（§8.5）~~ ⚠️ **2026-09-14：v1 无消费方**——§8.5.1 不做；两键**仍留在 `core/seed.py` 与 dict_config**（删库内已落行不划算），但**全仓除 seed 外零命中**，改它们**不产生任何效果** |
...
| O-2 | `fix_version` 组装时序（组装先于 claim 时 link 不含 fix_version） | **已裁定（v1.1）**：组装（§6.3）不写 fix_version；claim 后判定锚定 `cluster.fix_version`（§7.6 判定内核 `judge_link`——推送端点内 + `rejudge_job` 补判两处调用；+ §5.1 verify_run_record.bound_version）；requeue 重推复用 payload_id | 与 Task #4 对账仅剩 offline 侧 run 绑定版本解析 |
...
| O-8 | v1.23 反转的**诊断能力净损失**：cap 截断归因（区分「该版本全判」vs「窗口截断欠测」） | **已裁定（2026-09-11 用户拍板：不恢复）**：R-4 人工核对 + R-16 自动核对随 offline 只读面整组取消（§7.6 v1.6① / v1.8④ 作废注 + 各自 v1.23 反查订正）。后果 = cap 挤出的 claimed case 走 `missing` 支、**静默**保持 `pending` 至 claim TTL 回退 `open`，online 侧零可见面（不误触「疑似 offline 停摆」/`result_gap_suspected`，见 §7.6 v1.8④ 订正） | 归因归属改判 **offline 侧本地**（`case_truncated` 诚实标记，成因 = offline error case 容量治理）；**offline 侧「本地诚实诊断 + 告警」为实施要求，不得随之静默丢弃**（offline `error-backflow-phase2.md` §2.3 已登记）。**offline 侧同名待议项 = `error-backflow-solution_detail.md` §10.3 末「⚠️ 待议项（本批如实登记，未决）」**（机制侧台账另见 `error-backflow-phase2.md` §2.3/§6.1 注 1「降级为 offline 本地诚实诊断 + 告警」）——本裁定后该处应同步结清（offline 仓冻结中，**本轮不动、登记为待办**） |
...
- 平台间：offline 专用服务账号（evaluator）+ 独立凭证 + 端点白名单 + ~~双向认证~~ **（v1.23 作废：online 已零出站，实质为单向凭证 offline→online 持 secret，§8.8）**；pull-API `case_type` 白名单 + `schema_version`；响应 evidence 二期字段恒 null 契约（§8.7）。~~online→offline 回查凭证轮换同步~~ **（v1.23 作废：online 已零出站，无 online→offline 调用即无该凭证链；`BACKFLOW_INBOUND_SECRET`/`scope=platform:readonly` 一并作废，§8.8。）**
...
> 单测覆盖核心逻辑（Service/analyzer/converter 分支）；Controller/Repository/工具不强制（项目质量规范）。单测聚焦：L1/L2 判定、去重归一、聚类窗口、信封构造、词表快照、claim CAS、判定守卫、幂等。
...
| S-5 | 关键字检索命中 error_msg | 命中 trace 列表 + 深翻页（**`collapse(trace_key)` + `from/size` 偏移**，`offset ≥ 200` → 400 `ERR_TRACE_0002`；**非 `search_after`**，v1.26 订正）；限 7d/超时/上限生效 |
...
| E-7 | claim → 单错级判定 | claim 填 fix_version → offline 回归 run（含该 case）终态 → **offline 推 `POST /backflow/regression-results`** → 载荷该 case `case_pass=true` 落 `verify_run_record` + 同事务判定 → verify=passed → cluster fixed（closed_by=auto_regression）；同 run 他错仍红不阻塞（**v1.23：触发由「online 轮询拉」改「offline 推」，判定内核与结论不变**）|
...
| E-26 | ~~R-16 缺行自动核对 excluded~~ **（v1.23 取消，本用例作废）** | **整条作废**：`excluded_case_ids` 只读面与 `recheck` 自动拉 run 均不存在（§8.7 作废清单 / §7.6 v1.8 ④ v1.23 作废注）。**等价语义由 E-27 的 `gap` 中断承担**（缺行 → `gap`、保持 `pending`，不自动 reopen）；**验收时不得再测「excluded_case_ids / 窗口外欠测」**——该字段与读面在实现内已归零 |
| E-27 | R-17 K 序列锚（**v1.23 第 3 刀改判据**） | 中间版 v2 丢 error run（洪峰拒建 / 结果推送三次全败）→ online 以载荷 `prev_terminal_version` 对照**该 agent 已收版本全集**（本批由 link 级改全局）**见缺行 → 中断**，v1+v3 不假连续 K 满；v2 结果迟到到达即**补判**触发 reopen，不出现 false-fixed。**原判据「recheck 以 versions 读面见 v2 有发版无 run」已随读面作废**（§8.7 作废清单 / §7.6 v1.8 ⑤ v1.23 改注）；online 侧已由 `push_probe` S-14/S-15 实测（含反假绿对照），真 offline 汇合留阶段 4 |
...
| E-29 | R-24 requeue guard 状态域 | fixed/inactive cluster 的 pending invalidated link → requeue 拒（409，提示走 superseded+reopen 重建）；open/claim/needs_review cluster 的 pending invalidated link → requeue 复位 assembled、**fix_version/claim_k/TTL 锚点不变**、判定仍以现 claim 锚定（§7.4/§9.4） |
...
| X-2 | 脱敏死角 | 脱敏键不存在/值已是掩码形态/超长值/嵌套异常层级 → 不炸、不二次脱敏、可追踪（§2.6 脱敏；安全实现见 §13.4） |
...
| X-4 | 检索注入与转义 | 关键字含引号/通配符/保留字符/中文分词边界词 → 查询不报错、不误命、不返回错误 scope；末页后再翻**稳定返回空 items**（v1.23 口径：`collapse(trace_key)` + `from/size` 偏移翻页，非 search_after）（§8.2 检索） |
| X-5 | 平台间契约异常 | pull-API 收到 case_type 非白名单 → 返回空集；schema_version 不匹配 → 拒单并计数；回写字段非法/状态越界 → 幂等拒绝不污染状态机（§7.3/§7.4）。**v1.23 结果推送面**：`schema_version` 非 `1.0` → `ERR_PULL_0004` 拒单并计数；`case_type` 非白名单 → **丢该行 + `cases_dropped` 计数、不整体拒单**；`run_id` 重复 → 200 幂等不重复落库不重复判定；`trigger_signal_id` 关联不到的孤儿 run → 留档但不推进任何 link（§8.7） |
| X-6 | 大对象边界 | payload 超 Kafka 上限、evidence 实文恰 8K/超 8K 截断、缺 input 残现场只计数（E-13 集成复验）（§3/§6.2） |
| X-7 | 时间边界 | 事件 ts 未来/1970/UTC 与本地时区交界 → 周 index 归属与 date_histogram 分桶正确；跨周切换落在前后两周的检索去重不重不漏（§5.2 周滚动） |
| X-8 | 窗口边界 | error 同键恰 7d 聚类窗边缘（第 7 vs 8 天）→ count+1 vs generation 新开（E-4 补强）；指标 **1h/24h 实时 vs 7d rollup** 路由切换（`window` 为离散枚举 `{1h,24h,7d}`，不存在 `24h+1s`——越界值直接 400 `ERR_METRICS_0001`）；rollup 迟到恰 6h 幂等重算/超 6h 不再重算/缺桶回退实时 + 页面标注/尾小时实时补齐（§5.3） |
| X-9 | 判定时序边界 | judge_scan 到期瞬间补判、重复到期不重判（E-16 补强）（§4.3/§6.1） |
| X-10 | 数量级边界 | trace 检索：`page_size` 恰 200 为**入参上界**（`le=MAX_LIST_RESULTS`），`offset ≥ 200` **返 400 `ERR_TRACE_0002` 而非截断**（列表响应无 `truncated` 字段，带 `truncated` 的是详情 ≤500）；末页后翻；单 trace 日志上万行首屏懒加载不拉爆（S-5 补强）（§8.2） |
...
| X-12 | 词表边界死角 | 空表/极短表 fail-closed（E-5 复验）；动态前缀/拼接/大小写/换行/空白差异/超长词/重复词在词表残余承认范围内行为可观测（solution §16 登记口径，不要求全拦、要求假绿时可抽查发现）。**观测面口径（2026-09-14 明确化）**：online 侧可验的是**词表固化保真与信封透传**（`parse_words` 原样进 `no_fallback_config.words`、`resolve_fallback_wordlist` 取 per-agent 版本），**判分/抽查归 offline**——本阶段验前者，后者由 offline 侧承担 |
| X-13 | 保留期边界 | ILM 30 天删除后检索与看板缺口标注行为（衔接 T-5.2）（§5.2 ILM/保留期） |
