...
| `bound_version_first_seen` | 是 | **替代原 R-5 只读面的最小信息（其一）**：该 (agent, version) 在 **offline 侧**是否首次出现终态 run。online 以此重建「fix_version 未发版 → `no_progress`」守卫（§7.6）——**offline 不带此字段则该守卫失效** |
| `agent_latest_version` | 是 | **替代原 R-5 只读面的最小信息（其二，v1.23 第 2 刀补）**：offline 侧该 `agent` **已有终态 run 的最大版本**（与上一字段同一次查询产出，按 §7.5 日期前缀门控序求最大）。online 据此恢复**防假连续**——`V ≤ agent_latest_version` 的版本**应有** `verify_run_record`，缺 = **真缺行**（中断判定，防 v1(pass)+v3(pass) 而 v2 缺失被误判 fixed）；`V > agent_latest_version` = offline **还没跑到** → 停在此、保持 pending。**offline 不带此字段则防假连续守卫失效** |
...
**响应字段口径（v1.23 第 2 刀收口）**：`links_advanced` = 本次**真正新落 run 行**的 link（重复推送为空、orphan 为空；本刀不产生 `passed`/`failed`/`superseded` 终态迁移，那属判定内核）。`cases_dropped` = **载荷校验产物，与是否落库正交**——幂等重放仍返回**与首次相同**的值，**不归零**：首次推送若响应丢失而实际已成功，重试拿到 `duplicated:true` 时 offline 若见到 `cases_dropped:0`，会误判「没丢数据」而实际丢了行；恒返原值让重试拿到**一致答案**。**v1 硬约束：一次回归 run 只对应一个 cluster**（`trigger_signal_id` 单值；跨 cluster 批量回归不在 v1 范围）。
...
- **R-5「agent 已见版本」→ 降为载荷单字段 `bound_version_first_seen`**：online 只需守卫所需的最小信息，不再拿版本列表；**连带损失**——claim 表单「fix_version ∉ 已见版本」软校验的数据源消失，降级为「online 已收结果的版本集」软提示（§8.4/§9.3 文案同步，见 v1.23 修订记录）。
- **R-17 的「K 判据序列源」→ 改从已收 `verify_run_record` 自建**：不再依赖 offline 版本列表，online 侧按 `bound_version` 序自建序列。
