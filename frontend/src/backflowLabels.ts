...
// ─── K 进度短句（2026-09-20 批 47 / 任务 #45）─────────────────────────────
// 病根（用户原话）：「详情页已经显示回归验证 run pass 了，为什么在详情页和列表页都还是
// 未处置的状态？！不止这一条数据是这样」。
//
// 事实核查：**判定规则就是 K=2**（`verify.py:101-114` `decide_k`：相邻版本连续纯净 pass，
// `seq >= claim_k` 才判 passed，`claim_k` 默认 2）⇒ 只通过 1 次时状态**本来就该是 `open`
// （未处置）**，这不是「状态没更新」。实测 3 条 open 簇**全部**卡在 seq=1（run 都是
// 2026-09-16 14:25:46 同一批推来的，此后无第二次）。
//
// ⚠️ 真正的缺口在**展示面**：详情页早就写了「回归已连续通过 1/2 次」，而**列表页只写「未处置」**
// —— 用户在列表上看不出这簇「在等第二次回归、且已经等到了 1/2」，只能读成「状态没变/系统没反应」。
// 所以本函数的职责只有一个：把**已经取到的 `seq`**（任务 #36 起列表接口就返回了）露出来。
//
// ⚠️ K 序列的推进不归本函数、也不归 online：`backflowLabels.ts` 上方已记明 online **从不 push**，
// 是 offline 自己来拉（`pull.py:81`）⇒ 「第二次 run」只在 offline 下一轮拉到该用例时才出现。
// 本函数**不承诺**它一定会来（4 天没来就是没来）—— 故文案只陈述「已通过 x/y 次」，不写「即将收口」。
//
// 返回 '' = 不显示（没有进度可报）。**`seq === 0` 与 `seq === null` 一律不显示**：
// 前者是「一次没过」，后者是「不适用」（无现行 link / link 非 pending / 无 case_id）——
// 两者渲染成「回归 0/2」都会凭空造出一个不存在的进度（[[field-exists-is-not-field-informs]]）。
export function kProgress(c: { seq: number | null; claim_k?: number | null }): string {
  const seq = c.seq ?? 0
  // 后端同款兜底（`verify.py:253` `int(cluster.claim_k or 2)`）：字段缺省时按默认 K 算，
  // 否则会渲染出「回归 1/undefined 次」。
  const k = c.claim_k ?? 2
  if (seq < 1 || seq >= k) return ''
  return `回归 ${seq}/${k} 次`
}
