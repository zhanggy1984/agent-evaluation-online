...
- **T-4.11 前端端到端联调（§9.3/§9.4 全条）**：登录 → 菜单按角色渲染；看板/链路/回流三类页与 API 数据联动；状态文案逐条（assembled/draft/invalidated/pending-fix 标注"近似 offline 权威集"；⚠️ **批 50（#33）订正**：原列「**claim 倒计时**」——该控件及其背后的人工作业面已随批 35-A/B 整体撤除，条目保留仅为记录原验收范围）；空态（全站无数据不误读、二期入口隐藏不灰置）；**needs_review v1 边缘出现**（claim 回归 error run 结果行的判定产物才出现：na 结果行 → reason `na`；reentry 同版本 claim 旧 run pass 行 → reason `reentry_same_version`；复现输入截断 → reason `input_truncated`——**三者皆 claim 级单条走 cluster 级 `needs-review-resolve`**（⚠️ **批 50（#33）订正**：该端点已随批 35-A 删除，**此路当前无实现**）；**`unclean_run` 不入 `needs_review` 态**（只经 `needs_review_batch` 批载体、批引 cluster 保持 claim，v1.8 R-14），reason 值域 = **{na, reentry_same_version, input_truncated}**（⚠️ 2026-09-14 订正：原写 `{na, unclean_run, reentry_same_version}` —— **`unclean_run` 不入 needs_review 态**（只经 batch 载体，v1.8 R-14 / `claim.py:9`）、且漏了 `input_truncated`；权威 = `claim.py:32 REVIEW_REASONS`，见报告 §6 **F-20**），§10.5）。**验证目标**：detail §9.3 关键交互逐条通过；浏览器端无死功能入口（F1/F3/F4 收敛验证）；admin 操作留审计。
...
### ⑥ 顺带发现（**批 25 已查清并订正 —— 本条原登记有错**）

**`contract-check.backflow_allow`：库内 = `1`，`seed.py:36` 注释 = `0`（引 D18）。**

⚠️ **订正**：本节原写「**未查明**：库里这行何时、被谁改成 1」—— **答案是有的，就在本仓
`task.md:445-452`（T-5.8 阶段 B/C，2026-09-16）**。我**没读就先登记了「未查明」**，
还据此排了「另起一批」，等于把已经查清的事重新排期。

**查清后的事实链**（`task.md:449-451`）：

1. D18（`solution.md:65`）明写 cc「**维度 3 不纳入**」⇒ seed 的 `0` 是 D18 原设计值，注释本身**准确**；
2. 2026-09-16 用户一度拍板把 cc 纳入维度 3（临时例外）⇒ 阶段 B 执行 `0→1`；
   **同日内即撤销该例外** —— C1 七环实测证明 cc 两层**结构不可达**（根侧 `error_type` 全在
   `classify._layer_for` 的 11 词之外 ＋ `run_task_async` 是后台异步、与 LLM 故障不在同一 HTTP
   生命周期 ⇒ **即便开门也不成簇**）；
3. 「撤销例外」≠「回滚改库」：两个取值**行为完全等价**（都产不出候选）⇒ 处置 = **只登记、不动库**；
4. **最关键的一条**：`seed.py:122` 是幂等空更新 ⇒ D18 的 `0` **从未落库过**，
   **改建前实测即 = `1`** ⇒ 「cc 的 allow 取值」**不取决于**那次改库 —— 它本来就不是 0。

**⇒ 结论：seed 保持 `0`、库保持 `1`、代码与库一个字都不用改。原「另起一批」作废。**

**唯一遗留**（`task.md:452` 已登记备查，本轮不改）：`analyzer/classify.py:8` 注释称
「cc 双保险 = `agent.backflow_allow=0`（seed D18）＋ `backflow_enabled`」，**实测只剩一重**
（`backflow_allow` 实为 1），方向 = **fail-open**；不影响 cc 现状（两层不可达）。
