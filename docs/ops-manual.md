...
| **告警发出**（webhook / 邮件 / 外部通知） | — | — | ⚠️ **无通道**（`core/config.py` 无 webhook/smtp 字段，仅进程日志）——**⚠️ 2026-09-16 订正：本行原为整行否定，只对「TTL 类告警」成立**。**分两半读**：**(a) `waiting_days` TTL 类告警 = 确实不要**（裁定正确，见下方口径 ①）；**(b) 系统级告警**（漏采/断档 · offline 停摆 · inbox ack 积压 · ES 容量）**此前从未被裁定过，是被本行连带走掉的** ⇒ **已立 `task.md` T-5.5 补实现**。**⚠️ 2026-09-17 订正：原写「当前状态 = 仍未实现」已失效** —— T-5.5 **批 1（③ ack 积压告警 + 通道基建）已落码并真机验收通过**（实施仓 = **offline**；权威记录 = offline `error-backflow-status.md` O-D.4，本节 §5 第 5 行已同步订正）。**但只完成 ③ 一项**：①④② 仍未做，**不得读作「系统级告警已齐」** |
...
| 回查结果未达 | `result_overdue` | ✅ **已呈现**（**⚠️ 2026-09-17 订正**：原记「❌ 前端整条不消费」，前端已实现） | 后端已算（`api/backflow.py:399-407` / `:334-397`），**登记 = `integration-report.md` §6 F-18**（本节已加失效标注） |

**⚠️ 运营动作的现实**（**2026-09-17 重写**，原句「F-18 未闭合前，**「回查结果未达」这条在界面上看不到**——…只能查库或等 F-18 处置」**已不成立**）：F-18 **前端已实现** ⇒ **本条在详情页可见**（`BackflowClusterDetailView.vue`，命中即渲染后端原串文案）。**仍有边界**：**未做浏览器实测**，可见性由单测 + `vue-tsc` 支持，**不是页面实测结论**；且后端判据自带 pending/claim 期抑制/本轮性三道闸，**不命中不代表 offline 正常**。
...
| # | 边界 | 性质 | 登记 |
|---|---|---|---|
| 1 | ~~「回查结果未达」前端不呈现~~ **已闭合（2026-09-17）** | ~~实现缺口~~ 已实现 | F-18（**已加失效标注**；**残留 = 未做浏览器实测**） |
...
| 5 | ~~无任何外部告警通道~~ → **分两半**（2026-09-16 订正） | **TTL 类** = 设计如此（不设时钟、非告警）✅ 裁定正确；**系统级**（漏采/断档 · offline 停摆 · ack 积压 · ES 容量）= **此前被本行连带走掉、实为未裁定**，已立 `T-5.5`。**⚠️ 2026-09-17 订正：原写「当前仍未实现」，该句已失效** —— T-5.5 **批 1（③ inbox ack 积压告警 + 通道基建）已于 offline 仓落码并**真机验收通过**（单测 7 passed；真库探针连跑两遍各 15 passed / 0 failed；撤销回读零残留），实施仓 = **offline**（`app/core/alert.py` + `app/runner/ack_stale_probe.py`），权威记录 = offline `error-backflow-status.md` O-D.4。**但四项里只完成 ③ 一项**：① 漏采/断档（`_selfmonitor_loop` 体**未取证**）、④ ES 容量**未动**、② offline 停摆**只做得到「卡死」、进程死无观测点** —— 均为批次切分内的未做项（批 1b / 批 2），**不得读作「系统级告警已齐」**。另：告警要有人**收到**需配 `ALERT_WEBHOOK_URL`（属环境输入，未配则降级为进程日志），**该 URL 当前未配**。 | `api/backflow.py:320/421`（仅 TTL 半）；`task.md` T-5.5（系统级半） |
...
*⚠️ **该新归因的证据等级**（分两层）：**① trace 面不可验** —— trace 文档**不采集调用方**（字段全集无来源标识，`extra` 白名单 `{request_id, task_id, job_id, conv_id, sub_agent, prompt_kind}` 亦无来源键）⇒ 靠 trace **永远分不清**调用方是真实用户 / offline 评测 / 探针。**② 旁路 access log 可判，且两源旁证一致**（2026-09-16 实测）：`contract-check-frontend` 唯一访问者是 healthcheck（`Wget`）；`sp-web` **277 条请求行来源 IP 全为 `172.23.0.1`（宿主网关）**、UA 全为 `python-httpx` ×270 / 我方 Chrome ×6 / `curl` ×1、时间限 **08-31~09-01**。⇒ 表述为「**缺该输入（trace 面不可验、旁路两源旁证一致）**」，仍**非「实测为 0」**（日志有留存窗口），也不代表**生产环境**上线后无流量。*（**本条初版曾误写「cs / sp 的日志未出 stdout」—— 实测两者均为 `access.log -> /dev/stdout` 符号链接、**可查**；错因 = 把空输出当负面证据，同笔订正。）*
