...
| **告警发出**（webhook / 邮件 / 外部通知） | — | — | ⚠️ **无通道**（`core/config.py` 无 webhook/smtp 字段，仅进程日志）——**⚠️ 2026-09-16 订正：本行原为整行否定，只对「TTL 类告警」成立**。**分两半读**：**(a) `waiting_days` TTL 类告警 = 确实不要**（裁定正确，见下方口径 ①）；**(b) 系统级告警**（漏采/断档 · offline 停摆 · inbox ack 积压 · ES 容量）**此前从未被裁定过，是被本行连带走掉的** ⇒ **已立 `task.md` T-5.5 补实现，当前状态 = 仍未实现** |
...
| 5 | ~~无任何外部告警通道~~ → **分两半**（2026-09-16 订正） | **TTL 类** = 设计如此（不设时钟、非告警）✅ 裁定正确；**系统级**（漏采/断档 · offline 停摆 · ack 积压 · ES 容量）= **此前被本行连带走掉、实为未裁定**，已立 `T-5.5`，**当前仍未实现** | `api/backflow.py:320/421`（仅 TTL 半）；`task.md` T-5.5（系统级半） |
