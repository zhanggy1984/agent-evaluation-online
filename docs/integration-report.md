...
### F-15（2026-09-14）：T-4.1 最后一条残留「大 trace 懒加载」**定性为结构型**（我先前猜的容量型是错的）**并实测 5/5 PASS**
...
**② 实测 5/5 PASS**（探针 = `d6_probe#大 trace 懒加载`，1 event + **200 条日志**，走真机 HTTP）。
⚠️ 记账口径：5 条断言里第 1 条是**前置检查**（200 条须全部落地，不足即判 FAIL 而非静默通过）——
它**只在数据没落地时才出声**，故全绿时终端只打 4 行 PASS。**断言数是 5，不是 4**（别按 PASS 行数记账）。
...
探针自清已核（ES `delete_by_query` + MySQL `DELETE`）。**探针总断言数 25 → 30**（S-1 7 + S-2 4 + S-3 5 + S-4 3 + S-5 6 + 大 trace 懒加载 5）。
