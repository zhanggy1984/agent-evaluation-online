...
| 8 | **T-4.11** 残留 | `needs_review` v1 三 reason（na / input_truncated / reentry_same_version；**2026-09-15 订正**：原写 `unclean_run` 系**沿用了 task.md 两处错值**，代码权威值为 `input_truncated`，见 §6 **F-20**）前端呈现 / 菜单按角色渲染 / 看板类页 / **`result_overdue`「回查结果未达」提示前端整条缺失（2026-09-14 新挂，见 §6 F-18）** | 其余已覆；**F-18 只到静态 grep，未做浏览器实测**（`obs-frontend` 当前 Exited） |
