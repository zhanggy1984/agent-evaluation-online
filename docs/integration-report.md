...
5. 阶段 4 已开工（2026-09-13 起），**执行期发现登记于 §6**。§6 的 **F-x 是「待处置发现」而非「已定性缺陷」**——定性需先拍板缺陷处置口径（待定项「缺陷处置口径」）。
...
| 优先 | 任务 | 剩余内容 | 备注 |
|---|---|---|---|
| 1 | **T-4.7**（**E-18~E-20 三条**） | ~~Kafka broker 停 / ES 不可用 / MySQL 不可用~~ **三条已于 2026-09-13 真机注入全部通过**（见 §1 与 §6 F-4）；残留 = E-19 的「该小时 rollup 缺口标注」**已于 2026-09-14 定性 = 无实现对象**（设计要求见 `consumer/es.py:18-19`、实现缺失；详见 **§6 F-13**），非「未验」 | **E-21 的「网关不可达」注入半边不可做**——网关不在本阶段（见 §4.1），其**判定逻辑**已覆（§1） |
...
| 9 | **T-4.1** | ~~detail §14.1 冒烟 S-1~S-5 逐条复验~~ **五条已于 2026-09-14 全部复验完毕**：`d6_probe.py`（原 S-2/S-3 扩为 **S-1~S-5**）**25 断言全绿**，走真机 HTTP + 真 Kafka + 真 ES/MySQL（见 §1 与 §6 **F-14**）。**S-5 的三条原文口径已核**：限 7d（正反对照实证）/ 上限（400 实测）/ 深翻页（口径已 v1.26 订正） | 残留 = **「检索超时」未故障注入**（无「慢 ES」注入手段，只登记接线事实，**不记作已验**）/ **「在网关侧生效」不可验**——网关不在本阶段（见 §4.1）。**「大 trace 懒加载」已于同日定性 = 结构型并实测 4/4 PASS**（判据 = 首屏上界与总行数无关，N=200 即证）；残留容量尾巴（500 个 event 行的首屏量级）下移 T-5.3（见 §6 **F-15**） |
...
>
> **处置结果（甲类四条，2026-09-14 全部落地）**：**F-1** 三拆（1a 已删 2 行 / 1b 已修兜底扫描 + 负对照取证 / 1c 实查后**订正定性**：全库零外键属既定设计、非缺陷，另立「无 FK 是刻意还是遗留」待决）；**F-2** 降格声明（§14.4 五 job = **3 强 / 2 未验**，非 5/5）；**F-5** 已改 `_backoff` docstring（措辞订正，**行为未变**）；**F-12** 已于同日拍板 A（登记 + 并入本项，不补实现）。
...
**来路**：T-4.7 E-19 判据原文（detail §14.2）= 「事件落**待补写 spool** 或**显式丢弃** + **该小时 rollup 缺口标注**；判定/聚类/回流不受影响」。2026-09-13 三条故障注入实测只验了「显式丢弃 + `es_fail` 计数」与「判定不受影响」两条半边，**标注半边未验**（报告 §2 序号 1 与 §6 F-4 均标残留）。
...
### F-14（2026-09-14）：T-4.1 §14.1 冒烟 **S-1~S-5 逐条复验 = 25 断言全绿**——含**两条出在我这边的判据失误**
...
**① 定性 = 结构型**（不是容量型）：被验命题不是「1000 行时跑得够快」，而是「**首屏上界与 trace 总行数无关**」。该命题由两个实现层硬上界承载：
...
探针自清已核（ES `delete_by_query` + MySQL `DELETE`）。**探针总断言数 25 → 31**（S-1 7 + S-2 4 + S-3 5 + S-4 3 + S-5 6 + 大 trace 懒加载 6）。
