...
| # | 缺口 | 性质 | 归口 |
|---|---|---|---|
| ① | **词表（`fallback_utterance`）变更无执行载体**：无写入端点（`api/router.py:13-20` 只挂 5 个 router）、`require_admin` 无路由使用、**无审计落点**、**无 version 自增实现**（`models/config.py:23-24` 仅注释；`no_fallback_cfg.py:4-5`「变更 +1，admin-only」同样是注释） | 实现缺口 | **`T-3.12` 范围内**（其范围**明写含 `dict_config` 配置`**）——**已在册、不新立**；本条把「写入 + version 自增 + 审计」**显式列为 T-3.12 验收内容**，防实现时漏掉 |
