...
| **F-1c** | `ON DELETE CASCADE` 缺失 | **订正定性（2026-09-14 实查后）**：成立，但**不是这两张表的例外，而是全库设计**——`information_schema` 显示 `dev.obs` **13 张表 / 0 个外键**，且 `app/models/` 与 `alembic/` **双双零 `ForeignKey` 声明**，三层一致。⇒ **无 FK 是既定设计、非缺陷**；孤儿防护的责任按设计本就落在应用层与清理脚本上，而 **F-1b 补的正是这一层**。故**不「升格为缺陷」，也不提「补一条级联」**——那会与全库约定冲突。**真正的待决问题另记**：无 FK 是刻意为之还是历史遗留？（若刻意，须在文档里写明「子表完整性靠应用层」；若是遗留，属整库级动作，须单独立项。） |
