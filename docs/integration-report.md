...
| 9 | **T-4.1** | ~~detail §14.1 冒烟 S-1~S-5 逐条复验~~ **五条已于 2026-09-14 全部复验完毕**：`d6_probe.py`（原 S-2/S-3 扩为 **S-1~S-5**）**25 断言全绿**，走真机 HTTP + 真 Kafka + 真 ES/MySQL（见 §1 与 §6 **F-14**）。**S-5 的三条原文口径已核**：限 7d（正反对照实证）/ 上限（400 实测）/ 深翻页（口径已 v1.26 订正） | 残留 = **「检索超时」未故障注入**（无「慢 ES」注入手段，只登记接线事实，**不记作已验**）/ **「在网关侧生效」不可验**——网关不在本阶段（见 §4.1）。**「大 trace 懒加载」已于同日定性 = 结构型并实测 6/6 PASS**（判据 = **后端 API 首屏面**返回量上界与总行数无关，N=200 即证）；残留容量尾巴（500 个 event 行的首屏量级）下移 T-5.3（见 §6 **F-15**） |
