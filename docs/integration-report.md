...
| **T-4.13** ②④⑥ + **T-4.14** ①②⑥ | 脱敏死角（`find_sensitive_leak`）/ 检索注入与转义（真机 `/traces?keyword=`）/ 大对象边界（`snapshot_input` 8K）/ 时间边界（`iso_week` ISO 跨年归属）/ 窗口边界（`late_k` 扫描集与窗口外不回填）/ 词表边界死角（`parse_words` 保真 + 缺 agent 行降级） | **探针 33/33 PASS**，2026-09-13（真实现函数直驱 + 真机 HTTP + 真 ES + 真 rollup 轮）；**X-13 保留期下移 T-5.3、X-10 数量级半边下移**。详见 §6 F-10 |
...
| 6 | **T-4.13** ②④⑥ | ~~脱敏死角 / 检索注入与转义 / 大对象边界~~ **三条已于 2026-09-13 实测通过**（探针 33/33，见 §1 与 §6 F-10） | ①③⑤ 早已覆 |
| 7 | **T-4.14** ①②⑥ | ~~时间边界 / 窗口边界 / 词表边界死角~~ **三条已于 2026-09-13 实测通过**（同上）；**⑦ 保留期下移 T-5.3**（detail §14.5 X-13 原文自写「衔接 T-5.2 保留期」，本阶段无保留期策略可验） | ③⑤ 已覆；**④ 数量级边界**折半——`page_size≤200` 深翻页守卫已由 T-4.8 J3 覆盖，容量半边下移 T-5.3 |
