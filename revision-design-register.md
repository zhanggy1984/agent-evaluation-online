...
> **口径分两件，勿混**：① **推送「状态」**（「未 push / 未 commit」这类**断言**）记为落地时点的快照，
> **一律不回填**——逐行回填证不出各批真实推送时点，反而将其伪造成回填当刻（本区旧行的推送字面即按此保留）；
> ② **提交「引用」**（「hash 见 git log」这类**无提交号的指代**）**能定位就必须补**——它不承担时间语义、
> 只承担可复核性，留着就等于一句不可复核的引用，且**不该躲在①后面**。
> ②已于 2026-09-15 补两处（P2-6 行 = `004dccf`、T-3.10 行 = `c7169ea`，两者均已实测在 `origin/main`）。
