...
**设计原则**：把"复现环境可靠吗"与"claimed case 自己有可判证据吗"分开。recheck 判据锚点从 run 改为 **claimed cluster 对应 case 在该 run 的行**；na 对 pass 的"污染力"按其 error_type 归为**环境级 vs case 级**两类（error_type 值域权威源 = phase2 §6.4 na 五源；na 行必带 error_type，只读面已够，offline 零机制）。**（语境注，2026-09-10）本句「recheck 判据锚点」的**机制语境**已被 v1.23 取代**——拉取型 `recheck` 已整删，判定改在结果推送端点内同事务执行；**判据本体（case 级锚点 + error_type 影响域分流）不变**，故本节其余论证仍成立。
...
**问题回顾（核 detail §6.2 L801-805 + phase2 §6.1 注1 + v0.6 注 L111）**：窗口（7d）内同键复发只 count+1、**不重生成 link** → 一个 cluster 的 case 在窗口期内保持同一 case；claim 后 user 等回归观察。高错误率 agent 持续激活**其它新错误**的 case（newest-active-first）→ 该 claimed 老 case 被挤出 cap 窗口 → 之后**每个 fix_version 版本 run 都不含该 case** → recheck 缺行 ≠ na → 轮询至 claim TTL → 回退 open → 用户重 claim → 又缺 → **每 14d 原地转圈**。此场景与 R-1（有 run 但 K 不够）正交：**case 根本不在 run 里，claim_k=1 也救不了**。v0.6 注 L111「R6 边界声明」已对"单次截断中断 K"拍板接受退化；R-4 是"高错误率下**永久**挤出"，超出该边界声明覆盖的退化成度。**（语境注，2026-09-10）本段描述的「缺行 → 轮询至 claim TTL → 回退 open」机制语境已被 v1.23 取代**——推送为全量对账、无「缺行 → 轮询」场景，缺行改由 `prev_terminal_version` 单值指针承载（`gap`、保持 `pending`）；R-4 本体已按 §8.7 作废清单取消。历史记录原文保留不改。
...
5. claim 后 recheck 无 run 期前端持续显示「待 <version> 回归 run」+ 版本活跃度（避免静默）。 **（语境注，2026-09-10）「recheck 无 run 期」已被 v1.23 取代**——现形态为「等结果推送未达 / `gap` 保持 pending」，前端「不静默」的展示意图不变。历史记录原文保留不改。
...
**评审记录（2026-09-07 用户拍板）**：
- ① **采纳「软校验+新读面」**：offline 新增只读读面 = agent 已见版本列表（manual/held_out runs distinct version + 最近终态时间，只读不改 run 语义）；claim 提交前 trim + 大小写/v 前缀归一提示；fix_version ∉ 已见版本 → **告警确认**（展示已见版本，确认后才提交，非硬拦——豁免首版修复尚未终态/对账周期滞后合法场景）；claim 后 recheck 空窗期前端持续显示「待 <version> 回归 run」+ 版本活跃度。 **（语境注，2026-09-10）「offline 新增只读读面 = agent 已见版本列表」已被 v1.23 取代**——该读面整体作废，claim 软校验数据源降级为「online 已收结果的版本集」；「只提示不硬拦」的交互意图不变（见登记表 R-5 行）。历史记录原文保留不改。
...
（`conversion_record` 审计链，`action='requeue'`）为信号：已重推 ≥ 阈值仍被打回 ⇒ 疑似不可自愈
⇒ 前端**双次强确认**。

**⚠️ 门控强度实为降档，勿写「形态保留」**：原设计 = 不愈行**默认禁勾**（default-deny，且覆盖**批量勾选**）；本实现 = 所有行默认可点、≥阈值才弹两次确认（**default-allow + warn**），**且仅覆盖单 link 路径、不禁止任何勾选**。触发条件由「按缺项预判」换成「按行为事实」，但**默认态相反**。

- **范围 = 单 link 路径**（本轮只做这条）：`requeue_link` 返回 `requeue_count`；读面
  `_cluster_links` / `get_cluster_detail` **一次 `GROUP BY link_id` 成批取**（防 N+1）。
  **批量路径已裁定不做、本条结清（2026-09-11）**——**两条实质理由**：㈠ 要防的**实害已证不可达**
  （见本块第 2 项 ⇒ 门控**没有要防的对象**）；㈡ 残留的**后端半边无消费者**
  （端点唯一调用方 = curl 的 admin）。
  **⚠️ 当期事实、非理由**：前端批量重推入口整块不存在——「不愈行禁勾」是勾选框交互，**无框可禁**；
  **此系现状描述，非不可实现性论证**（UI 不存在是因为没人做，不是因为做不了）。
  **⚠️ 取证订正**：原文写「行为不可按 link 归因」**不准确**——`requeue_batch` 成功行经
...
- **批量侧标注（评审记录 ① ②）已裁定不做、本条结清（2026-09-11）**，detail §7.4/§8.4/§9.3
  ...
  （「不愈类」不可达 ⇒ 无对象可区分；门控又无 UI 可落；残留后端无消费者）⇒ **前提变更
  即否决依据变更**，本裁定 = **实质采纳备选 B（带理由，非默认滑落）**。原「判据待定」
  一并作废：判据**已定**（行为数据 `requeue_count`），只是**不落在批量路径**。
- **已知局限 ①（固有噪声，未解决）**：`requeue_count` 数的是**失败次数**——重推**成功**即自愈、
  不进计数，只有反复失败才累积。故它**区分不了**「确实不可自愈」与「admin 手快、现场尚未补完就推」。
  **现有数据无法证伪这一点**，属该判据的固有噪声，本轮未解决、亦无法以测试发现。
- **已知局限 ②（阈值语义的派生后果）**：口径 = 历史次数（**不含本次**），故历史 0 / 1 / 2 对应
  **第 1 / 2 / 3 次**重推 ⇒ **第 3 次才触发**强确认。若期望「第 2 次即拦」，须把常量改为 1
  或把比较改为 `> threshold`。**当前实现 = 第 3 次**，勿按「第 2 次」理解。
- **已撤回的推断（2026-09-11 记后即撤回，勿复用）**：本处曾登记「`requeue_batch` 筛选只吃 content_gap 行 ⇒「版本不识别」类进不了候选集 ⇒ 实害不成立」——**该推断已被证伪，撤回**。证伪证据 = offline `error-backflow-phase1.md:319-322` 与 `error-backflow-solution_detail.md:311/332` 明写：`version_drift`（版本不识别）**与 content_gap 共用 reject_code `online_content_gap`** → ack invalidated ⇒ 它**正好落在** `requeue.py:209` 的筛选内、**伪装成可愈那一类**。真正的缺口是「**区分信息不跨端**」（`PullAckRequest` 无 detail 槽，offline 本地 `reject_detail` 从不传给 online），**非「对象不存在」**。撤回理由 = 我据 online **单侧**代码外推到**双端契约**；**本文第 2 项已同批订正（2026-09-11 重核完毕）**：`ERR_PULL_0004` 确在推送接收面、`pull.py:88` 为拉取请求级，均非 offline 信封链；且原写「不落 invalidated 行」**亦为误判**——offline 侧证据见该项内。
...
- **R-16 → 缺行诊断自动消费 excluded 读面**：detail §7.6 v1.6①「UI 引导人工离线核对」升级自动——recheck 判缺行 → 自动 GET runs?agent=&version= → case_id ∈ excluded_case_ids → 自动标「窗口外欠测（cap 挤出、非修复失败）」沿用现语义（不自动 reopen/不进 needs_review）；∉ 维持人工兜底引导；excluded_case_ids 落 runs list 每行 / detail 位置 = **Phase B 实现核对项**。 **（语境注，2026-09-10）本条 R-16 语境已被 v1.23 整体作废**（`excluded_case_ids` 只读面 + 自动 GET runs 均不存在），见登记表 R-16 行「v1.23 已作废」后注。历史记录原文保留不改。
- **R-17 → K 序列版本锚 = R-5 versions 读面（agent 发版拓扑权威）**：候选版本序列定义——recheck 以 GET versions（window_days 覆盖 claim TTL 14d）取 ≥ fix_version 全版本序逐版：versions 有该版且 error run 有终值 → 判 K；versions 有该版但无 error run → **缺行中断**（下一可见 error run 不续缺环，杜绝 v1+v3 丢中间版假连续 false-fixed）；迟到补建终态到达后补判推进。根因 = 版本连续性缺权威锚（非迟到补建计入）；versions 端点用途从「claim 表单提示」升级「**K 判据序列源**」（recheck 每轮多一次 RPC）；offline 零新机制（versions 读面 R-5 已在）。 **（语境注，2026-09-10）本条 R-17 语境已被 v1.23 改锚**——K 序列源改为「本 link 已收 `verify_run_record` 行集自建」，缺行中断改由 `prev_terminal_version`（agent 级全集对照）承载；**防假连续目标不变**，见登记表 R-17 行「v1.23 已改锚」后注。历史记录原文保留不改。
...
>
> **状态列时效口径（2026-09-15 立）**：**未按惯例回填的行**，其「未 commit / 未 push（等授权）」记于
> **该批落地时点**，是当时的真实状态、**不随后续推送回溯改写**——故旧行字面停留在「未 push」并不代表
> 现在还欠一次推送。**此半句只管未回填的行**（本区多数行属此类）；某行是否已推送，一律以 `git` 实测为准：
> `git rev-list --left-right --count origin/main...main` 输出 `0 0` 即本地无未推送提交，
> 本区一切「未 push / 未 commit」断言**在当时之后已全部为假**。
> **回填按 register 既有惯例办，勿另立规则**：本区多行自陈「**行尾 commit 注记按 register 惯例于后续批次回填**」
> （且已有行执行过），故「该不该回填」是**既定工序**、不是可拍板的选项。2026-09-15 已按此惯例回填
> P2-4 / v1.23 / P2-6 / T-3.10 四行，**回填内容 = 回填当刻的实测事实**（`main == origin/main`、工作区干净），
> **原文能保留则括注保留**（v1.23 行「原记『local ahead 2…』系落地时点快照」／P2-6·T-3.10 行原记本身
> 已含提交号、无冲突故径改；P2-4 行原记「未 push」**本身即假断言**，故径改不保留）。
> ⚠️ **两半的优先级写死**：**已回填的行以本半（回填内容）为准，不再适用上一条「不回溯改写」**——
> 上一条只管未回填的行。⚠️ 惯例的反面才是伪造 —— 写成「**落地当时就已推**」才算篡改时点，而惯例从不要求那样写。
> （立此条前的 2026-09-15 一版曾写「推送状态一律不回填」，**已废**：它用自己的新规则覆盖了在场惯例，
> 且把「本轮没做的回填」重新定性成「有意不做」——该形态 =「用豁免规则掩盖没做」，勿再犯。）
> **另一件不同的事 = 提交「引用」**：「hash 见 git log」这类**无提交号的指代**，**能定位就必须补**——
> 它不承担时间语义、只承担可复核性。已于 2026-09-15 补两处（P2-6 行 = `004dccf`、T-3.10 行 = `c7169ea`）。
...
| P2-2 | **阶段 3 P2 回流第二批 D19 信封组装**（= T-3.2，detail v1.16）：① **触发形态（用户拍板）** = 纯 `assemble_job` 周期补偿扫描（`ASSEMBLE_INTERVAL_S=60`）——不做 cluster 首现即时内联（cluster_job 与 consumer root-late 两处产 open 由同一扫描幂等覆盖、单一写路径；新开簇至出 link ≤60s，offline 分钟级轮询可接受）；② 新 `converter/no_fallback_cfg.py`——`resolve_fallback_wordlist` 读 **per-agent** `dict_config(agent_id←Agent.id by name, key='fallback_utterance')` → `(words, version)`（`fallback_utterance` 为 per-agent 键、GLOBAL_DEFAULTS 无全局回退概念），命中返 config_value list + config.version、配置行缺/agent 缺 → `([],0)` fail-closed；③ 新 `converter/envelope.py`——`build_envelope` 按 §7.1 sample 逐键产 D19 信封（schema_version=1.0 / case_type=regression_error / payload_id=uuid4 / source{agent,interface,trace_id←first_trace_id,cluster_id,generation} / versions{trigger_version, fix_version=cluster 即时值（claim 前 null、永不回写）} / evidence{input=快照 json.loads 成功嵌还原值失败嵌原文, output·session_snapshot·retrieve_hit=null} / assert{no_fallback:{rule:wordlist,config_ref:{wordlist_version}}} 与 no_fallback_config{words,wordlist_version} 同刻同源）；`assemble_cluster` = 事务内 resolve→build→写 `error_case_link`(assembled+pending、source_trace_id/trigger_version/fix_version/input_truncated 自簇复制、payload_json=ensure_ascii=False) + `conversion_record(action=assemble)`（**input_truncated 不进 JSON、随 link 列透传**）；**空词表照建信封**（words==[] fail-closed 载体 → offline 结构自检 content_gap）；④ 新 `worker/assemble_job.py`——扫批判据 open ∧ input_snapshot 非空（E-13 快照缺跳过、R-18 回填后下轮扫到）∧ **无 `verify_status='pending'` link**（§6.3 窗口抑制，invalidated 仍 pending 占位不重组装），order by id limit 200 批循环收敛、每候选 `begin_nested` savepoint + IntegrityError（uk_link_current/uk_link_payload 并发双 worker）吸收跳过（E-12 先例）、每批一次 commit；⑤ worker/main 新增 `_assemble_loop`（60s run→sleep 串行不重叠、单轮异常 logger.exception 下轮自愈同 judge/cluster）、start 健康日志补 assemble_interval_s | detail **v1.16**（修订记录 + §6.3 触发句追加「纯周期扫描实现形态」注记）；task.md T-3.2 推进注；git 历史 | **landed（2026-09-09）**：backend 单测 **287 passed**（基线 269 + 新增 18：parse_snapshot_input 4 / build_envelope 4 / resolve_fallback_wordlist+parse_words 4 / assemble_cluster 4 / assemble_loop 2）；本窗口新增后端 diff ruff 零新增（E501 两处已修）；**assemble_probe 集成探针容器内真库 8/8 全绿**（A-1 首现组装 link+conv+envelope 逐字段含中文 ensure_ascii 可回读 / A-2 窗口抑制 / A-3 快照 NULL 只计数不组装 / A-4 空词表 words==[] / A-5 uk_link_current 二次组装 IntegrityError / A-6 fix_version 组装即时值带出 / A-7 fixed 排除 / W-1 resolve 真库读三态）；**已 commit 已 push origin**（代码 + detail v1.16 + register 本行 + task T-3.2 = main `306e444`，`main==origin/main` 工作区干净；经 ssh.github.com:443 一次性通道，origin 22 端口不通）；**obs-worker 已重启拾取 `_assemble_loop`**（库内 0 open 候选空扫静默，无真实组装发生——重启后行为归位属预期） |
| P2-3 | **阶段 3 P2 回流第三批平台间契约 pull-API + ack 回写 + admin invalidate/requeue**（= T-3.3，detail v1.17）：① 鉴权 = `/pull/*` 预共享静态 secret（env `EVALUATOR_SERVICE_SECRET` Bearer `require_evaluator`、未配置 fail-closed 全 401 `ERR_PULL_0001`、不接平台 JWT）；backflow 人工面 = 平台 **admin** JWT（deps.py 新增 `require_admin`，viewer → ERR_AUTH_0002 403）；② 新 `app/backflow/` 域包——`ack.py`：纯 `ack_decide`（§7.3 矩阵 + R2 例外 gate link 现行 invalidate_reason=offline_cap_gap→active + 幂等 noop）+ `pull_payloads` keyset（join cluster 按 agent、(assembled_ts,id) 升序、since_ts≥、limit+1 截断）+ `apply_ack` **CAS WHERE offline_status=前置**（rowcount=0 重读：==目标幂等 200 / R3 带当前状态，E-15 并发交错吸收）；`requeue.py`：纯 `requeue_guard_errors`（R-24 = invalidated∧verify=pending∧cluster∈{open,claim,needs_review}∧**防抖 ≥5min 锚 = assembled_ts**）+ `requeue_link`/`requeue_batch`（复用 payload_id + 现词表 `build_envelope(payload_id=…)` 重填 payload_json + 刷新 assembled_ts + 清标注 + 不动 cluster 锚点 + conv(action=requeue)；batch 逐行 savepoint + 汇总 conv）+ `invalidate_link`（仅 assembled/draft→invalidated(manual)+conv）；③ API：`app/api/pull.py`（/pull/payloads——schema_version≠1.0→ERR_PULL_0002、case_type∉白名单→200 空集、坏游标 400；/pull/ack——未知 payload ERR_PULL_0003(404)）+ `app/api/backflow.py`（links/{id}/invalidate、links/requeue-batch 静态段先声明、links/{id}/requeue）；`AppError` 加 `extra`（R3 响应体带 offline_status+invalidate_reason）；`build_envelope` 加可选 `payload_id` 参数；**零 DDL 零新 worker job**（error_case_link 既有列 + conversion_record 自由串；纯请求驱动） | detail **v1.17**（修订记录，§8.4 backflow/§8.8 evaluator 凭证/错误码历版已就位）；task.md T-3.3 推进注；git 历史 | **landed（2026-09-09）**：backend 单测 **307 passed**（基线 287 + 新增 20 test_backflow）；本窗口新增后端 diff ruff 零新增；**pull_probe 集成探针容器内真库 HTTP 级 13/13 全绿**（分页升序/agent 过滤 / since 边界 / ack 全矩阵 + R2 例外 + R3 带态 / E-15 幂等 + 双 ack 交错 / E-22 requeue 旧水位重拉 / admin invalidate·requeue·批量 + viewer 403 / 未知 payload 404）；E-5/E-6/E-15/E-22 环 1 验（集成复验阶段 4 T-4.3）；**已 commit 已 push origin**（代码 + detail v1.17 + register 本行 + task T-3.3 = main `de81255`，`main==origin/main` 工作区干净；行尾 commit 注记按 register 惯例于 P2-4 批回填）；obs-worker 无新 job 无需重启 |
| P2-4 | **阶段 3 P2 回流第四批 人工处置状态机 + 回查 verify 收口**（= T-3.4，detail v1.18）：① 新 `app/backflow/claim.py` **五端点全 CAS `WHERE status=期望旧值` + conv 审计**——claim（open→claim，fix_version 必填 + trim 归一、**claim_k 固化** {1,2} 缺省 dict_config `auto_fixed_k_default`=2、TTL = `claim_ttl_days` 默认 14d）/ ignore（open→inactive + 现行 pending link 先 superseded）/ reopen（fixed/inactive/needs_review→open）/ needs-review-resolve（单条：reopen_cluster→open / escalated §16 只记录不迁移）/ fixed-review（**admin**：approve:true→claim→fixed(closed_by=admin_review)+link passed、false→claim→open+link failed；viewer 调 → ERR_AUTH_0002 403 不单方 closed）；**ERR_CLUSTER_0002(409) 本批首用激活**（CAS 竞态/重复处置；非法迁移/值域 → ERR_CLUSTER_0003(400)、不存在 → ERR_CLUSTER_0001(404)）；② 新 `app/backflow/verify.py` **回查判定内核**——error_type 影响域常量表（**环境级 5** + **case 级 10** + 未知默认从严 env，R-19 字面量护栏）+ 纯函数 `route_verdict`（fail→reopen / na→needs_review 单点 / pass∧截断→needs_review 单条 **R-15 双通道优先** / pass∧环境级 na→unclean_run / pass∧纯净→count_k）+ `decide_k`（**相邻纯净 +1** R-17 防跨缺版假连续、seq≥claim_k→passed）+ 集成 `judge_link`（versions 读面 ≥ fix_version 全序逐版、fv 未发版 no_progress、缺版 gap 中断、uk_verify_run 幂等留档终态只读、R-16 excluded 欠测标注；K 满 → link passed + fixed(closed_by=auto_regression) **E-7** / fail → 即时 reopen）；③ 新 `app/backflow/batches.py`（R-14/R-9）——`ensure_unclean_batch`（uk_batch_agg **全状态唯一幂等单条**：resolve 后同 key 复发 → 复用同批重开、open 批 cluster 按 id 去重、**批引 cluster 保持 claim 不入 needs_review**）+ `resolve_batch`（CAS 整批单事务抢占，**动作集 {reopen_cluster, escalated}**：reopen_cluster 逐引用 cluster claim→open + 现行 pending link superseded 释放 cur_key + **每结果 conv 审计（resolved_ts 落审计）**、R-9 语义 reopened/skipped_already_open/skipped_fixed/manual_review；escalated §16 批升级不迁移 cluster、交人工后续处置）；④ **读面 3 GET 响应字段级钉死**（overview `{clusters,links,to_fix,by_agent[]}`——to_fix = active∧verify∈{pending,failed} 本地镜像近似标注 / clusters `{items,total,page,page_size}` 筛选 agent/interface/layer/status/watch + link 摘要 / detail 含 verify_runs 版本时间线 + conversions 审计 + waiting_days）；⑤ ~~新 `app/core/offline_client.py`（list_runs / run_results / agent_versions 三读法 + 3x 指数退避 + MockTransport 测试注入）+ config 新键 `offline_base_url` 空 = recheck_job 停轮（复用 evaluator_service_secret 双向 Bearer）~~ **（v1.23 第 3 刀已整删：判定数据源改 offline 推送载荷、online 零出站；`offline_base_url` 字段仅留 .env 兼容）**；⑥ 新 worker job ×2（`claim_ttl_job.py`：claim_due_ts 超窗 CAS claim→open + fix_version 保留溯源 + conv claim_ttl_expire **E-8** / ~~`recheck_job.py`：judge_link 逐行事务~~ **（v1.23：整删，补判由 `rejudge_job.py` 承接——同调 `judge_link`，机制由「拉 offline」改「重放本地已落行、零 outbound」）**）各 60s 挂 worker/main；⑦ **batches.py 真 bug 修复**（`ensure_unclean_batch` create 分支误取先前 select 的 None 批对象 `.id` → AttributeError，探针 C-5 捕获、单测 create 路径未覆盖由 DB 语义探针补位） | detail **v1.18**（修订记录）；task.md T-3.4 推进注；.env.example 补 OFFLINE_BASE_URL（空默认 + offline 配套轨落地后填注记）；git 历史 | **landed（2026-09-09）**：backend 单测 **342 passed**（基线 307 + 新增 35 test_backflow_claim）+ 本窗口新增后端 diff ruff 零新增；**claim_probe 集成探针容器内真库 HTTP 级 13/13 全绿**（C-1~C-13：claim 固化+due≈now+14d / ignore·reopen·fixed-review + viewer 403 / **C-3 E-7** 纯净 K 满 → verify_run_record×2 + fixed(auto_regression) / **C-13 ~~recheck 编排~~ → 推送编排**（v1.23：假 offline 读面改「直造 `verify_run_record` + 推送端点/`judge_link` 驱动」→ fixed_auto）/ C-4 fail reopen / C-5 unclean 批 + cluster 保持 claim / C-6 同批双 ref resolve（含 escalate 子校验：批 resolved 不迁移 cluster）+ resolved_ts / C-7 na 单条 resolve / **C-8 E-8** TTL 超窗回退 / C-9 gen>1 claim 软提示 / C-10 CAS 双发竞态 ERR_CLUSTER_0002 / C-11 读面 shape+分页 / C-12 excluded 欠测标注）；batch resolve/ensure 5 项加固随 code 落（escalated 动作 + resolved_ts + 每结果 conv 审计 + note 截断 ≤1024 + ensure_unclean_batch 同 key 复发复用重开防 uk IntegrityError）；E-7/E-8 环 1 验（真机阶段 4 T-4.4 环 2）；**~~obs-worker 仍跑 P2-3 代码，claim_ttl/recheck 两新 loop 待重启拾取（未授权未重启）~~ ⟵ 2026-09-10 实测回填（原表述已过时）：obs-worker 已于 2026-09-09T16:08:40（本地）重启拾取——**证据链 = 容器 `/app` 为 bind mount 至宿主机 `backend/`（非镜像烘焙），且进程启动时刻晚于两目标文件各自最后一次改动**（`worker/main.py` ← `7c17929` 15:30、`analyzer/cluster.py` ← `0543698` 16:06）→ **该进程加载的必为 P2-5 树**：`main.py` 6 loop 全武装（judge/cluster/assemble/claim_ttl/recheck/rollup，L63-68；**v1.23 后该清单已改为 judge/cluster/assemble/claim_ttl/rejudge/rollup——本句为 P2-5 时点事实、保留不改**）、`cluster.py` 含 P2-5 `reentry_gate_allows`（**原写「docker cp 实测」措辞不准**：bind mount 下 `docker cp` 只是读宿主机文件，单独看不构成「进程加载了什么」的证据）；**停→起：2026-09-09T08:54:51Z Exited(137)（停因未取证，「随宿主机停机」系推断）、2026-09-10T03:22:17Z 已再次重启**（cluster_merge 15s / judge_scan 60s 稳定 tick、零异常；offline_base_url 空 → recheck 停轮——**该机制已随 v1.23 第 3 刀整删，本句为 2026-09-09 时点事实**）；**已 commit = main `7c17929`**（ahead 1 已随 P2-5 批推送；行尾 commit 注记按 register 惯例于 2026-09-15 回填（回填时实测 `main == origin/main`、工作区干净）） |
| P2-5 | **阶段 3 P2 回流第五批 reentry 版本门控 + 终态只读显式守卫**（= T-3.5，detail v1.19）：① §7.5 版本门控纯函数 `reentry_gate_allows(agent_version, fix_version)`（analyzer/cluster.py，B-5 契约）——两侧各截前 10 位日期前缀（YYYY.MM.DD），**前缀相等 → 仅完全等值放行**（同日不等值按"修复上线中/未上线"处理，防 r100<r47 字典序把已上线误判未上线）；前缀不等 → 按前缀**字符串序**（新现日期 > fix 日期放行、< 不过）；任一侧非日期形态/空 → 整体不透明**字面量等值**；candidate None → False（无版本证据不开，人工 reopen 兜底）；② `_merge_once` 接线（**内联 cluster_job，用户拍板不落独立 reentry_job**）：pick_merge_target 返 "open" 后、建簇前，取**最高代终态簇**，仅当 `status=="fixed" ∧ fix_version 非空` 才咨询门控（inactive 终态 / UPDATE fixed 无 fix_version → 无门控基础维持 P2-1 无条件开，S-3 + test_worker_cluster 零破坏）；**门控不过 → merge 返新动作 "blocked"（DB 零落**：不建簇不 count 不 conv，judged 行仍被 processed CAS=1 消费——阻断可见性按拍板归 P2-6，**v1.20 定音 = 后端读 MySQL `trace_judge_state` 保留窗现算（推翻本行原「前端 ES 派生」；前端无 ES 通道、ES 无 judged/root_input_hash/candidate 语义不可还原）**）；门控过 → 建簇 gen+1 + **同 savepoint 事务落 `ConversionRecord(action="reentry", actor_user_id=None, detail=§7.5 原文提示)`**（IntegrityError E-12 twin 吸收回滚一并回滚 conv 不产生孤儿；重查转 count 分支不重放门控，conv 只由 twin 那份带出不双写）；merge 返回契约更新 `created/counted/skipped/blocked`（cluster_job·state.py 两调用方均忽略返回值 → 零破坏）；③ **E-10 终态只读显式守卫** = `verify.judge_link` 入口若 `link.verify_status != "pending"` → `no_progress`（"link 非 pending（终态只读，迟到 run 不覆写）"）——把仅靠 recheck 扫描谓词保证的终态只读固化为 judge_link 局部不变量（P2-4 已固：recheck 谓词只扫 pending + `_decision_from_row` 只读 + `uk_verify_run` 幂等） | detail **v1.19**（修订记录）；task.md T-3.5 推进注 + T-3.6 reentry_job 条目标注「已并入 cluster_job（P2-5），不落独立文件」；git 历史 | **landed（2026-09-09）**：backend 单测 **352 passed**（基线 342 + 新增 10 TestReentryGate 全分支矩阵：同日同值放行 / 同日不等值不过 / 跨日后于 fix 放行 / 跨日早于 fix 不过 / 纯日期等值 / 非日期字面量 / 单侧非日期字面量等值 / None·空串·纯空白 False / 空白 trim / agent 非日期 vs fix 日期 False）；本窗口新增后端 diff ruff 零新增；**cluster_probe 集成探针容器内真库 10/10 全绿**（S-7 **E-9** reentry 复发 → gen+1 + conv(action="reentry") + g1 不动 / S-8 同日不等值 blocked 零落 / S-9 早于 fix blocked 零落 / S-10 无门控基础回归护栏——fixed 无 fix_version 照开 gen2 + inactive 终态照开 gen2）；**claim_probe 14/14 全绿**（含 **C-14 E-10**：link passed + cluster fixed 后直接 judge_link 注入迟到 fail run → outcome no_progress 不覆写、verify_run_record 不追加、cluster 仍 fixed）；E-9/E-10 环 1 验（真机阶段 4 环 2）；obs-worker P2-5 无新 loop（门控内联既有 cluster_job），**P2-4 的 claim_ttl/recheck 两 loop 已于 2026-09-09T16:08 重启时一并拾取（**v1.23 后为 claim_ttl/rejudge——recheck loop 已随第 3 刀删除，本句为 P2-5 时点事实**）**（bind-mount + 启动时刻证据链详见 P2-4 行；**2026-09-10T03:22Z 已重启、运行中**）；**已 commit 已 push origin = main `0543698`**（`main==origin/main` 工作区干净；行尾 commit 注记按 register 惯例于 P2-6 批回填——本条即该回填） |
| P2-6 | **阶段 3 P2 回流第六批 前端回流页 + blocked 复发读面实现形态修正**（= T-3.7，detail v1.20）：① **前端回流页落地**（先例 = TraceDetail 独立壳，两页**独立路由**非抽屉）——「回流看板」= **第六个一级菜单** `/backflow`（viewer/admin 均可见、**无菜单级 gating**；admin-only 动作前端按 `role==='admin'` 隐藏 + 后端 require_admin 二次鉴权）；列表页 = overview 卡（cluster 状态分布 / link verify 分布 / 待修复集本地近似 + 标注 / by_agent 分布）+ 筛选 agent·interface（近 7d 实测并集）·layer·status·watch + **cluster 表（状态 pill / error_type / input_hash / 代表 trace `first_trace_id` 跳 trace 详情 / count / generation / first·latest ts / fix_version）** + 分页（静态 + 手动刷新，不轮询）；详情页 = 元数据 + links 表 + verify_runs 版本×pass/fail 时间线 + conversions 中文审计时间线 + 人工操作区（按 §9.4 门控置灰）+ claim 复核窗倒计时（1s tick + 45s 可见轮询，超时回退提示）+ input_truncated 警示 + **被未决 unclean_run 批挂起徽标 + 「处置整批」** + **reentry_observe 复发 caption**；**二期入口（弃留墙/quality）整条隐藏不渲染**；文案集中 `src/backflowLabels.ts`（offline_status / verify_status / reason / cluster 状态 / conversion action 中文 + reentryCaption 逐字钉定）；② **blocked 复发读面实现形态修正（用户拍板，推翻 v1.3/v1.19「前端 ES 派生」）**——前端只能经 backend API 取数（无 ES 通道），ES 仅存原始 trace 事件（无 judged/root_input_hash/candidate 语义，判定侧归一哈希不可还原）→ 改**后端读 MySQL `trace_judge_state`（近 `trace_judge_purge_days` 保留窗）现算**（命中键 = agent + root_input_hash（与 cluster.input_hash 同源）+ interface + error_type∈candidate_error_sets；`mode=fixed` 只计 ts≥fixed 锚 ∧ 门控不过、`mode=claim` 计 ts≥claimed_at 全计；fixed 锚 = conversion_record action∈{auto_fixed, fixed_review} max ts）；**仍不新增存储列**（零 DDL/新列/job）；③ **后端最小增量（全响应加法）**——新 `app/backflow/recurrence.py`（纯函数 `recurrence_rows_py` + `cluster_reentry_observe`，复用 `analyzer/cluster.reentry_gate_allows`）；`GET /backflow/clusters/{id}` 响应追加 `reentry_observe`（claim/fixed 态否则 null）+ `open_batches`（`NeedsReviewBatch.status=open` ∧ `JSON_CONTAINS(link_refs, {cluster_id})`，items `{batch_id,run_id,agent,bound_version,error_type,ref_count}`）；`_cluster_item` 补 `first_trace_id`（ErrorCluster 既有列）；**claim 端点 R-5 版本预检做全**（offline 配置时 best-effort 查 `agent_versions` 读面，fix_version 未见 → 行内告警列已见版本；offline 未配 → None 沿用 generation>1 软提示；**非硬拦**） | detail **v1.20**（修订记录 + §7.5 门控不过复发可见性派生口径改写（删「查 ES」）+ §8.4 两读面行补 `first_trace_id`/`reentry_observe`/`open_batches` + §9.1 第六菜单登记 + §9.2 回流两行占位→已实现 + §9.3 新增 blocked caption 行·批挂起行·claim R-5 做全措辞）；register P2-5 行尾 commit 注记回填 + 行内「前端 ES 派生」措辞修正（本批一并）；task.md T-3.7 推进注 + L47/L116 ES 措辞修正；git 历史 | **landed（2026-09-10）**：backend 单测 **385 passed**（基线 352 + 新增 33 `test_backflow_recurrence`：`recurrence_rows_py` fixed/claim 全分支矩阵 + 锚边界 + latest_version 取序 + fix_version 空 → None + error_type 命中集合 / `_cluster_item`·detail 字段级 / claim 版本预检 fake transport 三分支）；本窗口新增后端 diff ruff 零新增；前端 `vue-tsc --noEmit` + `vite build` 绿；**环 1** 验（假 offline/online 数据；真机浏览器 e2e 留阶段 4）；**已 commit = `004dccf`（随本批同笔，本 register 此行的登记亦在该笔内）；commit 注记按 register 惯例于 2026-09-15 回填（回填时实测 `main == origin/main`、工作区干净）** |
| **T-3.10**（**修正批，不新编 P2-N**——P2 编号止于 P2-6，本批非实现批而是**验收阻塞项修正**) | **T-3.9 验收阻塞项修正：兜底吸收误回流判定语义 + E-17 补探针**（= T-3.10，detail v1.21）：① **缺陷**——§6.1 明写「兜底吸收现场（request ok + llm_call error）不产 L1/L2 候选（L3 二期）」，而 `analyzer/classify._collect_candidates` 对 `err_entries` 的迭代**无条件**、`decide()` 无前置守卫 → 请求成功的 trace 只要子节点有 llm 错误即产 L1 候选，回流建簇并触发 offline 回归**虚报「agent 有 bug」白烧 offline 资源**；**长期未发现的根因 = 检测盲区是结构性的**（既有用例 `root_status` 只取 error/timeout，**从未取 ok**）。② **可达性三重钉死（非理论构造）**——`root_ok` 语义 = 「root 已到达」非「root 成功」（`consumer/state.py` 到达即置 1、status 另存一列）∧「request ok + llm_call error」是 SDK 既定埋点形态（`record_llm` docstring：裸调用「先记 error 再抛」由 agent 包装点保证，§2.4 前提）∧ `judge_scan_job._facts_from_row` 零过滤透传；实测 `decide()` → `layer="L1"`（期望 `none`）。③ **修正**——门控落 `_collect_candidates` **局部**（非 `decide()` 前置：当前唯一生产调用方即 `decide()`，贴着迭代放，未来新增调用方自动继承该不变量），判据 **`root_status == "ok"` 单条件**，**刻意不叠加 `root_ok`**（`root_status` 全仓唯一写点 = `consumer/state.py` 的 `state.update(root_ok=1, …, root_status=event.status, …)`，两者同一条语句置位 → `root_status=="ok"` ⟹ `root_ok=1`、合取项恒真可证冗余；留单条件 = 只押「root 终态」一个语义，不与派生布尔标志的将来定义漂移耦合；用户 2026-09-10 裁定收敛）；**不用「`root_error_type` 为空」**（timeout root 按 step5 同样不带 error_type，会连带切掉 timeout 分支 = 未讨论的第二处行为变更）；残 trace `root_status` 恒 NULL 与门控天然不相交。④ **已知未决歧义显式标注、本批不触碰**——step5（timeout 事件自身不产候选）与 step4（L2 判定用「request 或子节点」）两条互斥原文未消解，对「timeout root 的子节点 error 是否参与」沉默 → **维持改动前既有行为**（仍参与），待真实流量再定。⑤ **E-17 补探针 C-15**（原**零覆盖**：`claim_probe` 全文件 grep `payload_id` = 0 命中，C-4 只覆盖前半段）——reopen 后改 fix_version 再 claim → 组装出**新 link + 新 payload_id**（非复用旧 link）→ 新版本回归 passed → fixed，且**旧 failed 时间线保留不覆写**；配套新助手 `_assemble_new_link`（**不复用 `_activate`**：其 `.one()` 无 pending 筛选，旧终态 link 在场会 `MultipleResultsFound`） | detail **v1.21**（修订记录 + §6.1 L826 兜底吸收条目补「门控已落地」实现约定 + 头注释实现约定条）；task.md T-3.10 条目 + T-3.9 闭环注；git 历史 | **landed（2026-09-10）**：backend 单测 **388 passed**（基线 385 + 3 新增，**既有 `decide()` 用例零调整**——已逐条核实无一构造 `root_status="ok"`）；**原缺陷复现点由 `L1` 归零为 `none`**；**claim_probe 集成探针容器内真库 15/15 全绿**（含 C-15 五段子校验），cluster 10/10 / assemble 8/8 / pull 13/13 零回归；本窗口新增后端 diff ruff 零新增（`classify.py` F401·E501 与 `test_analyzer_classify.py` I001 三条经 HEAD 基线比对确认为**存量**）；**已 commit = `c7169ea`（detail v1.21 + 本 register 此行 + task.md 同步，同笔）；commit 注记按 register 惯例于 2026-09-15 回填（回填时实测 `main == origin/main`、工作区干净）** |

| v1.22（**T-3.6 结清批**，纯口径修正、零行为改动，不占 P2-N 编号） | **T-3.6 结清：§1.3 周期表对齐实现 + 三项剩余逐 job 核查归零**（detail v1.22）：① 调研推翻 task.md 原「整批收口留后续批次」措辞——**单飞 CAS / 失败退避 / requeue 防抖 ≥5min 三项代码全部已在 P2-1~P2-4 落地、各有单测与探针**，缺的只是文档没跟上；② 改 §1.3 周期表 4 处漂移（`cluster` 1min→**15s** / `claim_ttl` 10min→**60s** / `recheck` 1~5min→**60s** / `reentry` 一栏**撤销**）+ 声明「**以 `worker/main.py:35-42` 常量为唯一事实源**」+ detail 修订记录 + `worker/__init__.py` 陈旧 docstring（原列 requeue/reentry 两个不存在的 job 修正）+ task.md T-3.6 结清注 + register 附录 A「阶段 3 剩余」更新（原列 T-3.9+T-3.6 已过时 → 仅剩 T-3.8）；③ **不计入 T-3.6 的三项** = 双实例单飞归 **T-4.5** / rollup 真实 ES 集成探针留集成阶段 / 「job 自监控计数」不在 §14.4 写侧判据内故不新编实现（worker 侧确无失败计数器/heartbeat/metrics，要补须另立 T 项并先定可见面挂靠） | detail **v1.22**（修订记录 + §1.3 周期表）；task.md T-3.6 结清注；`worker/__init__.py` docstring；register 附录 A「阶段 3 剩余」段 + 本行 | **landed（2026-09-10）**：末笔 = main **`bacd99b`**（4 files +11/−5 纯文档、零行为改动），**已 push、`origin/main == bacd99b`**；复核 438 passed / ruff 干净 / 五探针 15·22·13·10·13 全绿 |
| v1.23（**第 1~3 刀 = 平台间契约反转**，判定链由「online 主动拉 offline」反转为「offline 主动推 online」） | **① 第 1 刀（契约定义，纯文档，并入第 2 刀 commit）**：钉死新契约——offline 主动 `POST /backflow/regression-results`（fire-and-forget、5s 超时、3 次重试 1s/2s/4s、**online 不重试**）；判定数据源 = **本 link 已落的 `verify_run_record` 行集（载荷 `raw_json` 原样留档）**+ cluster 字段，**不再是 offline 只读面**；载荷 **删** `bound_version_first_seen`（强制但零读点）、**增必填** `prev_terminal_version`（值可空 = 无前次终态 run；堵「单值水位无法枚举中间版本 → 跨缺版丢推送 → 假累积 K → 静默误判为已修」）；「缺行中断」判据改 **agent 级**（`verify._agent_versions`，link→cluster 两跳）承载；`BACKFLOW_INBOUND_SECRET` 从未存在于代码，删除该空转设计。**② 第 2 刀 `b38a8fd`（接收面实装）**：`api/backflow.record_regression_result`（flush 后 commit 前**同事务**跑 `verify.judge_link`）+ 「结果未达」claim 判据（`claim_ttl_expire` conv 在 ∧ `fix_version` 非空 ∧ 现行 pending link 零 `verify_run_record`，含未超窗/过期记录须晚于 `assembled_ts`/空 `assembled_ts` 抑制集）+ 派生读字段 `result_gap_suspected`（零 DDL）+ `push_probe.py`（22 场景）+ `ci.yml` 挂 push_probe。**③ 第 3 刀 `f67719f`（判定内核切推送源 + 删净轮询链）**：`judge_link` 去掉 `client` 参数；**整删** `core/offline_client.py`、`worker/recheck_job.py`、`RECHECK_INTERVAL_S`（**job 6→5**）；**补回** `worker/rejudge_job.py`（60s **本地补判、零 outbound**：扫 `status=='claim'` ∧ 有现行 pending link 的簇、重放本 link 已落行；收口判据 =「本批零终态迁移」非「空批」）（**job 5→6**）；`offline_base_url` 字段保留仅为部署 .env 兼容（注释「已无消费方」）。**故 job 数 = 6（6→5→6，勿写成 5→6）**；**核心教训 = 删轮询链把「重试安全网」一并删了——老 recheck 的安全来自「循环」不是守卫本身**，照搬旧谓词到一次性调用点会让未认领簇的回推被永久丢弃 | detail **v1.23**（§1.3 周期表 + §7.6 判定语义块 + §7.3/§7.5 + §8.7/§8.8/§8.9 + §12.2 O-2 + §14.2 E-7/E-26 + §14.4 等）；solution **v3.5.10**（全篇「回查/轮询/只读面」→「结果推送 + 被动探测」）；task.md（T-3.4/T-3.6/T-4.7/阶段 4 出口）；**本 register**（R-4/R-5/R-16/R-17 行「v1.23 已作废/已改锚」后注 + 评审记录区语境注 + 附录 A P2-4 行 ⑤⑥ 与验证栏划改 + 本行） | **landed（2026-09-10）**：第 1 刀并入第 2 刀笔 **`b38a8fd`**，第 3 刀 **`f67719f`**；**两笔 commit 注记于 2026-09-15 按 register 惯例回填（回填时实测 `main == origin/main`、工作区干净；原记「local ahead 2；`origin/main` 仍 = `bacd99b`」系落地时点快照）**；复核 438 passed / ruff 干净 / 五探针 15·22·13·10·13 全绿 / 前端 vitest 130；三容器均已重启/重建、新码真机生效（`obs-worker` 6 loop；`obs-frontend` 新 bundle 含 gap 文案）；**~~唯一未闭合 = rejudge 的运行态 liveness 无证据~~ ⇒ **已于 2026-09-14 真机 tick 取证闭合**（`tests/integration/tick_rejudge_seed.py` seed→check→clean 三段）：seed 一个 claim 簇 + 一条 `pending` link（**脚本只造数据、不做判定**）→ 等 ≥1 个 live rejudge loop（60s）→ 该 link `verify_status` 由 `pending` 迁移为 **`failed`**，并落地 `conversion_record(action='reopen')` 行。**「非脚本触发」的对照 = 时间戳**：obs-worker 自 `00:25:24Z` 起持续存活、seed 于 `03:13:59Z`，窗口内**无重启**，故迁移只可能由 live 进程产生。**边界（勿读成更强）**：① 日志为空**不构成反证**——`rejudge_job.py:141-142` 仅在 `judged_total>0` 时打印，故主证据取 **DB 迁移**而非日志；② 同窗内该簇 `status` 兼由 `claim` 变 `open`，**此条仅记录观测、其因果链未单独取证**；③ 取证后数据已清（agent/cluster/link/run/conv 五表回 0）。 |
| **A-值域卫生**（非 R 批次；**四 agent 仓代码 + online 文档**，2026-09-15。**不占 P2-N 编号**——非回流实现批，是 agent 侧上报值域的契约对齐） | **LLM 级 error_type 值域对齐平台白名单 + §2.5 补强制值域条文**。**① 触发它的实测结论（先取证后改码，非推断）**：ES `dev.obs-event-*` 9001 doc + `trace_judge_state` 105 行（103 条真实 trace）实测——**真实流量中所有失败的 `llm_call` 其 `request.status` 均为 `ok`；所有非 2xx 的 `request` 均无 `llm_call` 子节点** ⇒ L1 的机制级判据 `llm_fact_ok=1 ∧ root_status=error` **交集为空**（实测 0 命中）⇒ **L1 当前零产出**。**四 agent 仓逐仓读码：均无「LLM 失败 → 透传非 2xx」入口**（gq 先 yield meta 后调 LLM；cs 规则引擎兜底 + `event: done`；sp 2 SSE + 3 处 service 层吞点、唯一 503 熔断在 LLM 调用**之前**故无 `llm_call` 子节点；cc 后台任务形态 + dry-run catch）。**按 `solution.md` 降级型异常归属条，兜底吸收现场属 L3、v1 不回流 —— 这是设计意图，不是缺陷**。**② 词表分叉的真实影响面**：四仓上报的 `TIMEOUT`/`NETWORK`/`HTTP_429`/`HTTP_5xx`/`AUTH_ERROR`/`ALL_KEYS_DOWN`/`QUEUE_TIMEOUT`/`LLM_ERROR` 等**均不在平台白名单**，而 `analyzer/classify._layer_for` 对白名单外值返 `None` → 调用点逐值跳过 ⇒ 该值**静默失去回流资格（不报错、不落标记；同 trace 内其他白名单值照常产候选，非整条作废）**。当前影响面 = **0**（L1 本就零产出，值域分叉不改变该事实）；**真风险在将来**——若某 agent 日后真出现透传现场（正是 L1 存在的理由），其上报名不在白名单 ⇒ **L1 回流静默失效且现场无任何提示**。**③ 本批显式不解锁 L1**：值域正确是**必要非充分**条件（还须过 root 终态/`backflow_allow` 等 §6.1 门控，且兜底吸收现场按规约不回流）⇒ 改完 L1 **仍零产出**。**改动**：四仓 `llm_error_type` 折叠函数重写为白名单值域（仅 429 单列 `llm_rate_limit`，其余状态码统一 `llm_other`，原始码值由 `error_msg` 保留不丢证据）+ 四仓 `test_obs_wiring.py` 断言/docstring 同步；online `solution_detail.md §2.5` 新增三条「值域约束」（LLM 级强制值域 / 白名单外值的后果 / 值域正确是必要非充分）。**刻意不做**：request 级 `error_type` 值域约束（仍开放，只出指标与 trace）、不新建文档、不加 SDK 常量。**已知语义粒度问题（用户 2026-09-15 裁定：维持 + 登记不修）**：LLM 级白名单七词**不含凭证类词**，401/403（key 失效/无权限）与其余非 429 状态码统一折叠为 `llm_other` ⇒ 若真出现凭证故障透传现场，它会与「provider 层其他失败」**聚为同一簇**（同因不同簇/同簇不同因）。不修的理由 = 白名单七词是平台既定语域（`analyzer/classify.LLM_ERR_TYPES` → offline 分类表 → 二期规划），增词须两侧同改、跨服务且超本批范围；本批目标仅「不上报白名单外的值」，语义粒度属平台侧既有问题。 | `solution_detail.md` §2.5 值域约束三条；`good-question/backend/services/llm_service.py` + `chat_service.py`；`customer-service/backend/app/infrastructure/deepseek_gateway.py`；`smart-procurement/app/obs.py`；`contract-check/backend/app/obs.py`；四仓 `tests/test_obs_wiring.py`（sp 为 `tests/unit/`） | **landed（2026-09-15）**：四仓各自一笔 = `good-question 2eb0f4f` / `customer-service befcc90` / `smart-procurement 79b0a85` / `contract-check 68dbff1`，online 本笔 `0eed31a`（本登记行 + `solution_detail.md` §2.5 同笔）。**均已 commit / 全部已 push（快进非 force）**。验证 = 四仓「绿 + 判别力对照」两证：gq 9→233 passed（对照：还原旧实现 **8 条断言红**）；cs 7→16 passed（对照 **13 红/3 绿**）；sp 29 passed（对照 **4 红/25 绿**）+ 全量 **364 passed**（sp pre-commit 的覆盖率门禁真跑，70.02%）；cc 17 passed（对照 **4 红/13 绿**）+ 全量 **409 passed / 1 failed**。**未验收项**：① cc 全量那 1 条失败 = `TestContractsManifest::test_manifest_fields`（manifest `endpoints` 少 `login`），**已实测在 HEAD 版本上同样红 ⇒ 既有缺陷，与本批无关，按「不碰无关代码」登记不修**；② 四仓 pre-commit 的「静态检查 + 覆盖率通过」**不可读作 lint 门禁**——该全局 hook 不含任何 lint，且覆盖率门禁对 `backend/` 布局的仓（gq/cs/cc）是死代码（`[ -d tests/unit ]` 判仓根），仅 sp（`tests/unit` 在仓根）真正执行；③ ruff **仍不可用**（本机 PATH/各仓 venv 均无），以「>100 列扫描、零新增」替代，**不是等价门禁**。 |
...
> **P2 编号边界（2026-09-10 补正）**：附录 A 的 P2-N 编号**止于 P2-6**（P2-1~P2-6 = T-3.1~T-3.7 回流实现批，一一对位）。detail v1.20 修订记录行末尾曾出现「P2-7 gate 门 / offline 配套轨另批」——系**自编号误引，已撤销**（"gate 门"出处 = solution §11 #5「judge 放行开关」，属**二期、v1 不实装**）。阶段 3 剩余 = **T-3.9 online 侧 P2 门**（**验收门，非实现批**，范围以 task.md T-3.9 条目为准）+ **T-3.8 offline 判定语义改造**（offline 仓，另一条腿）。**register 只登记 landed 事实，不为未落地项写预声明行**（P2-1~P2-6 均无预声明行，此非既有惯例）。**T-3.10 = T-3.9 验收挖出的阻塞项修正批**（兜底吸收误回流 + E-17 补探针），**不占 P2-N 编号**——该边界不因修正批而破；P2 编号是否续编留待阶段 3 收口时定。**（2026-09-10 更新）**：本行原列「阶段 3 剩余 = T-3.9 online 侧 P2 门 + T-3.8」**已过时**——**T-3.9 验收门已于 2026-09-10 闭环**（其挖出的阻塞项「兜底吸收误回流」由 **T-3.10** 修正并 landed）、**T-3.6 亦已结清**（逐 job 核查：单飞 CAS / 失败退避 / requeue 防抖 ≥5min 三项代码**全部已落地于 P2-1~P2-4**，双实例单飞归 T-4.5、rollup 真实 ES 集成探针留集成阶段；「job 自监控计数」不在 §14.4 写侧判据内故不计入；随收口修正 §1.3 周期口径漂移 = detail v1.22），故**阶段 3 剩余仅 T-3.8 一项**（在 **offline 仓**，另一条腿）。
