...
平台的重活不在请求线程里，而在 `obs-worker` 容器的一组常驻作业上（与 HTTP 进程分离部署，见第三章）：
...
平台最基本的数据结构是**一棵事件树**：`request` 为根，`llm_call` / `tool_call` / `retrieve` / `db` / `redis` 为子节点，全部携带同一个 `trace_id`，并以 `seq` 自增编号、`parent` 指回根节点：

```
trace_id = 7f3a…c21   agent = good-question   agent_version = 0.2.1
interface = POST /api/v1/chat

seq=0  request     parent=null  status=error  duration_ms=3120  error_type=connect_error
 ├─ seq=1  llm_call   parent=0   status=ok     duration_ms=1180  model=deepseek-chat
 ├─ seq=2  retrieve   parent=0   status=ok     duration_ms=86
 ├─ seq=3  tool_call  parent=0   status=error  duration_ms=1800  error_type=connect_error
 └─ seq=4  db         parent=0   status=ok     duration_ms=12
```

落到 ES 里的真实文档形状（根节点与 LLM 节点各一例，字段名即实际事件 schema）：
...
```json
{
  ...
  "trace_id": "7f3a…c21", "agent": "good-question", "agent_version": "0.2.1",
  "interface": "POST /api/v1/chat",
  "node": "request", "seq": 0, "parent": null, "ts": 1758420000000,
  "status": "error", "duration_ms": 3120,
  "error_type": "connect_error", "error_msg": "…",
  "input": { "已脱敏的入参快照": "…" }, "extra": {}
}
```
...
```json
{
  ...
  "trace_id": "7f3a…c21", "agent": "good-question", "agent_version": "0.2.1",
  "interface": "POST /api/v1/chat",
  "node": "llm_call", "seq": 1, "parent": 0, "ts": 1758420000180,
  "status": "ok", "duration_ms": 1180, "model": "deepseek-chat",
  "usage": { "prompt_tokens": 1240, "completion_tokens": 310, "total_tokens": 1550 },
  "extra": {}
}
```

几条字段约定（在 SDK 构造层写死，不靠业务自由拼接）：

- **`status` 三态**：`error` 必带 `error_type`，`ok` / `timeout` 必不带——错误类型是聚类与 LLM 失败页的入口键；
- **`input` 是显式传入才落的键**：它同时是聚类去重指纹（`root_input_hash`）的来源，**缺了则这一条不建簇**；
- **子节点字段面更窄**：`tool_call` / `db` / `retrieve` / `redis` 只有 `status` / `duration_ms` / `error_type`，不带 `input` / `output`（正文默认不采，控制存储成本）；
- **工具名不进事件**：具体调了哪个工具是 agent 侧的私有概念，平台的事件面只装平台要消费的字段。

同一棵树同时服务两件事：

- **指标**：每个节点的耗时与状态 → 接口维度与 LLM 维度的 P95、失败率、超时率（请求级 + LLM 调用级**双指标**）；
- **完整性判定**：这棵树是否**长齐**，决定它能不能进入聚类——半截的树不足以复现，必须等齐（见[六.2](#六技术闪光点)）。

> trace 详情页按节点列出该次请求的全部事件（日志行分页懒加载）——排障时沿列表往下看是哪一环断的，**不需要复现现场**。
...
本平台除了「看」，还往前走了一步：**让线上错误自己变成可复现、可回归的资产**。
...
### 2. 跨批 / 跨 SSE 累积的 trace 判定态

**难点**：「一个 trace 是否完整、是否算一次有效请求」**不能由任何单个事件自判断**——一次请求的事件可能分散在多批里、跨多次 SSE 推送、子节点分批到达。而只有判定为**完整**的 trace 才允许进入聚类：否则会把半截 trace 当成一次错误，造出一条**根本无法复现**的用例。

**做法**：

- **跨批累积态**：同一 trace 的事件无论何时、分几批到达，都归并到同一份判定态上，由**累积后的整体**判断完整性，而不是由单事件自报；
- **判定态持久化在 MySQL，不驻留内存**：消费组重平衡或进程重启后，从库内重建判定态——**不回读 ES 补状态**。ES 是**检索面**，不是**状态面**，把状态压在 ES 上会让「查得到」和「判得对」互相绑架；
- **迟到根节点的补判通道**：残 trace 已按子节点完成判定之后，根节点才到达的——**不重跑整个 trace**，只补一次根级候选并复用既有聚类结果，凭 CAS 标记保证补判幂等、不重复计入。

**收益**：完整性这个最容易出错的环节，被收敛成一个**可持久化、可重建、可幂等**的状态机——消费端可以随意重启、重放、扩缩，判定结果不变。

### 3. 幂等设计里的一条「静默数据损坏」防御

ES 的文档主键是 **`_id = sha256(agent | trace_id | seq)`**。键里那个 **`agent` 维度是专门加的**，堵的是一类**不报错的损坏**：

> `trace_id` 由**各 agent 自行生成、不保证全局唯一**——两个 agent 各自产生的 trace_id 完全可能撞上。一旦撞上，后写入的事件会按 `_id` **覆盖**先写入的，**且不产生任何错误**：两个 agent 的链路数据静默地互相污染，查询结果看起来完全正常，只有在对账时才会发现「这个 trace 怎么混进了别的 agent 的节点」。

其余幂等设计：

- **日志行占用 seq 编号**，与业务事件共用同一序号空间，因此同样需要 `_id` 幂等保护；
- **MySQL 侧**聚类开簇与 link 写入均为幂等操作——**消费重放不产生重复簇**。
...
6. **正文默认不采集**：内容型数据的可见性依赖采集策略（默认关 + 最小化）控制，而非事后掩码。这是隐私上的保守选择，代价是**部分「答案质量类」的排查场景缺原始正文**，需按接口逐 agent 评估后单独开启（该取舍的另一面见[六.5](#六技术闪光点)）。
