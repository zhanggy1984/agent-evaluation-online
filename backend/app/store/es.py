...
def build_trace_list_body(
    *,
    ...
    if status:
        ...
        # ⚠️ **口径警告（使用时必须知道）**：本列表按 trace_key **折叠去重**（一 trace 一行，
        # 见上文「折叠键」），故这里的 status 判的是**该 trace 最新命中行**的状态 ——
        # **不是**「trace 内存在 error 事件」。与 /metrics/interfaces 的 error
        # （filter agg 的 doc_count、**不折叠**）必然不等：一个 trace 内 3 个 error 事件
        # 在接口页计 3、在这里最多出 1 行；若最新行是 ok 则根本不出。
